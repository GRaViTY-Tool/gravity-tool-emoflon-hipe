...
public class MFlow_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MFlow>> ports;		
		
	public MFlow_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MFlow>(getSelf(), name2actor.get("Flow__Marker_CREATE__SRC__mFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFlow>(getSelf(), name2actor.get("IncomingFlow__Marker_CONTEXT__SRC__mFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFlow>(getSelf(), name2actor.get("MAbstractFlowElement_ownedFlows_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFlow>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__SRC__mFlow_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MFlow> message) {
		for(Port<org.gravity.modisco.MFlow> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MFlow> message) {
		for(Port<org.gravity.modisco.MFlow> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MFlow> message) {
		for(Port<?> port : ports) {
