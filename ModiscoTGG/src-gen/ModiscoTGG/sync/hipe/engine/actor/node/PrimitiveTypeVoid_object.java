...
public class PrimitiveTypeVoid_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.PrimitiveTypeVoid>> ports;		
		
	public PrimitiveTypeVoid_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeVoid>(getSelf(), name2actor.get("Model_orphanTypes_10_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeVoid>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_8_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeVoid>(getSelf(), name2actor.get("PrimitiveTypesIsVoid__Marker_CREATE__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeVoid>(getSelf(), name2actor.get("TypeToTAbstractType_source_17_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeVoid> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeVoid> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
		for(Port<?> port : ports) {
