...
public class MSignatureToTSignature_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSignatureToTSignature>> ports;		
		
	public MSignatureToTSignature_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("FieldSignature__Marker_CREATE__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MSignatureToTSignature_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MethodSignature__Marker_CREATE__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
	}
