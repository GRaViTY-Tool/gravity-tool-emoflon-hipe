...
public class UnresolvedTypeDeclaration_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.UnresolvedTypeDeclaration>> ports;		
		
	public UnresolvedTypeDeclaration_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.UnresolvedTypeDeclaration>(getSelf(), name2actor.get("MGravityModel_unresolvedItems_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.UnresolvedTypeDeclaration>(getSelf(), name2actor.get("TypeToTAbstractType_source_19_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.UnresolvedTypeDeclaration>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CREATE__SRC__unresolvedType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
		for(Port<org.eclipse.modisco.java.UnresolvedTypeDeclaration> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
		for(Port<org.eclipse.modisco.java.UnresolvedTypeDeclaration> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
		for(Port<?> port : ports) {
