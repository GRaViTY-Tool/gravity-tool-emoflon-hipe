...
public class ASTNodeToTAnnotatable_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ASTNodeToTAnnotatable>> ports;		
		
	public ASTNodeToTAnnotatable_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_target_4_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__Marker_CONTEXT__CORR__aSTNodeToTAnnotable_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("MethodDefinition__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
	}
