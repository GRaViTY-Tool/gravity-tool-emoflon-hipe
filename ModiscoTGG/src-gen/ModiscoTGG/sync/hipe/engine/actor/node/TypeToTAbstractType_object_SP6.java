...
public class TypeToTAbstractType_object_SP6 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypeToTAbstractType>> ports;		
		
	public TypeToTAbstractType_object_SP6() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("AnnotationTypeAccess__Marker_CONTEXT__CORR__AnnotationTypeToTAnnotationType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("EnumInAnonymous__Marker_CREATE__CORR__mNestedTypeToTNestedType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("PrimitiveTypeIsShort__Marker_CREATE__CORR__typeToTAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("TypeToTAbstractType_source_8_reference"), this::returnTrue));
	}
