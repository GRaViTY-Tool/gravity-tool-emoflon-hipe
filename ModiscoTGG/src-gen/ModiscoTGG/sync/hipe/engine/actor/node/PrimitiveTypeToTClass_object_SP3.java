...
public class PrimitiveTypeToTClass_object_SP3 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PrimitiveTypeToTClass>> ports;		
		
	public PrimitiveTypeToTClass_object_SP3() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsInt__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_3_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_8_reference"), this::returnTrue));
	}
