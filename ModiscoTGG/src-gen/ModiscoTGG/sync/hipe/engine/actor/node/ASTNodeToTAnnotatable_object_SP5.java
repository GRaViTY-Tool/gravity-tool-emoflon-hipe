...
public class ASTNodeToTAnnotatable_object_SP5 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ASTNodeToTAnnotatable>> ports;		
		
	public ASTNodeToTAnnotatable_object_SP5() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_source_3_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_target_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("InterfaceInAnonymous__Marker_CREATE__CORR__mbodyToTAnnotatable_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("NestedInterfaceInType__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
	}
