...
public class NestedEnum__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.NestedEnum__Marker>> ports;		
		
	public NestedEnum__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.NestedEnum__Marker>(getSelf(), name2actor.get("NestedEnum__Marker_CONTEXT__CORR__eOuterTypeToTOuterInterface_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.NestedEnum__Marker>(getSelf(), name2actor.get("NestedEnum__Marker_CONTEXT__TRG__typeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.NestedEnum__Marker>(getSelf(), name2actor.get("NestedEnum__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.NestedEnum__Marker>(getSelf(), name2actor.get("NestedEnum__Marker_CREATE__SRC__eNestedType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.NestedEnum__Marker> message) {
		for(Port<ModiscoTGG.NestedEnum__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.NestedEnum__Marker> message) {
		for(Port<ModiscoTGG.NestedEnum__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.NestedEnum__Marker> message) {
		for(Port<?> port : ports) {
