...
public class Package_object_SP2 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.Package>> ports;		
		
	public Package_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Package>(getSelf(), name2actor.get("AnnotationType__Marker_CONTEXT__SRC__mPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Package>(getSelf(), name2actor.get("Enum__Marker_CONTEXT__SRC__mPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Package>(getSelf(), name2actor.get("PackageToPGPackage__Marker_CREATE__SRC__mPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Package>(getSelf(), name2actor.get("SubPackageToPGPackage__Marker_CONTEXT__SRC__mParentPackage_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.Package> message) {
		for(Port<org.eclipse.modisco.java.Package> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.Package> message) {
		for(Port<org.eclipse.modisco.java.Package> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.Package> message) {
		for(Port<?> port : ports) {
