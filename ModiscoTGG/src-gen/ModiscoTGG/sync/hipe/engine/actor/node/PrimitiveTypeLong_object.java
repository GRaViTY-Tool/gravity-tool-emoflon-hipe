...
public class PrimitiveTypeLong_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.PrimitiveTypeLong>> ports;		
		
	public PrimitiveTypeLong_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeLong>(getSelf(), name2actor.get("Model_orphanTypes_8_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeLong>(getSelf(), name2actor.get("PrimitiveTypeIsLong__Marker_CREATE__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeLong>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_6_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeLong>(getSelf(), name2actor.get("TypeToTAbstractType_source_15_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.PrimitiveTypeLong> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeLong> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.PrimitiveTypeLong> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeLong> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.PrimitiveTypeLong> message) {
		for(Port<?> port : ports) {
