...
public class MethodParameterLinkFirst__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MethodParameterLinkFirst__Marker>> ports;		
		
	public MethodParameterLinkFirst__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLinkFirst__Marker>(getSelf(), name2actor.get("MethodParameterLinkFirst__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLinkFirst__Marker>(getSelf(), name2actor.get("MethodParameterLinkFirst__Marker_CONTEXT__SRC__mParameterList_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLinkFirst__Marker>(getSelf(), name2actor.get("MethodParameterLinkFirst__Marker_CONTEXT__TRG__tParameter_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		for(Port<ModiscoTGG.MethodParameterLinkFirst__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		for(Port<ModiscoTGG.MethodParameterLinkFirst__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		for(Port<?> port : ports) {
