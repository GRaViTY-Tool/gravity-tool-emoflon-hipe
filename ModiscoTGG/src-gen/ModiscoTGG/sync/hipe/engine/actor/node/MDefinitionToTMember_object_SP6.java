...
public class MDefinitionToTMember_object_SP6 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MDefinitionToTMember>> ports;		
		
	public MDefinitionToTMember_object_SP6() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("FieldAccessWrite__Marker_CONTEXT__CORR__mMethodToTMethod_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_source_2_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_source_4_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("SuperMethodInvocation__Marker_CONTEXT__CORR__mMemberToTMember_0_reference"), this::returnTrue));
	}
