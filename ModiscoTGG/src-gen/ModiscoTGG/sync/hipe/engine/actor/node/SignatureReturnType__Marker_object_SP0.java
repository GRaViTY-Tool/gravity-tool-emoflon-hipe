...
public class SignatureReturnType__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.SignatureReturnType__Marker>> ports;		
		
	public SignatureReturnType__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.SignatureReturnType__Marker>(getSelf(), name2actor.get("SignatureReturnType__Marker_CONTEXT__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.SignatureReturnType__Marker>(getSelf(), name2actor.get("SignatureReturnType__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.SignatureReturnType__Marker>(getSelf(), name2actor.get("SignatureReturnType__Marker_CONTEXT__TRG__tSignature_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.SignatureReturnType__Marker> message) {
		for(Port<ModiscoTGG.SignatureReturnType__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.SignatureReturnType__Marker> message) {
		for(Port<ModiscoTGG.SignatureReturnType__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.SignatureReturnType__Marker> message) {
		for(Port<?> port : ports) {
