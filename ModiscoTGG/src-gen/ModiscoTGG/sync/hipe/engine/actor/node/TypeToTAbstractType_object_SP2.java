...
public class TypeToTAbstractType_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypeToTAbstractType>> ports;		
		
	public TypeToTAbstractType_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("AnnotationMemberValueClassArray__Marker_CONTEXT__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("ClassInnerClassMember__Marker_CREATE__CORR__tInnerTypeToTInnerAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("NestedEnum__Marker_CREATE__CORR__eNestedTypeToTNestedType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("TypeToTAbstractType_target_9_reference"), this::returnTrue));
	}
