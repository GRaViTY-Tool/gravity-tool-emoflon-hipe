...
public class InterfaceInAnonymous__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.InterfaceInAnonymous__Marker>> ports;		
		
	public InterfaceInAnonymous__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.InterfaceInAnonymous__Marker>(getSelf(), name2actor.get("InterfaceInAnonymous__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.InterfaceInAnonymous__Marker>(getSelf(), name2actor.get("InterfaceInAnonymous__Marker_CONTEXT__TRG__pg_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.InterfaceInAnonymous__Marker>(getSelf(), name2actor.get("InterfaceInAnonymous__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.InterfaceInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.InterfaceInAnonymous__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.InterfaceInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.InterfaceInAnonymous__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.InterfaceInAnonymous__Marker> message) {
		for(Port<?> port : ports) {
