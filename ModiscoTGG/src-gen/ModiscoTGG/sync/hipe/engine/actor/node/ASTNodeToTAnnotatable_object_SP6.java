...
public class ASTNodeToTAnnotatable_object_SP6 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ASTNodeToTAnnotatable>> ports;		
		
	public ASTNodeToTAnnotatable_object_SP6() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_source_6_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("AnnotationTypeNested__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ConstructorDefinition__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("TypesOrphaned__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		for(Port<ModiscoTGG.ASTNodeToTAnnotatable> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		for(Port<ModiscoTGG.ASTNodeToTAnnotatable> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		for(Port<?> port : ports) {
