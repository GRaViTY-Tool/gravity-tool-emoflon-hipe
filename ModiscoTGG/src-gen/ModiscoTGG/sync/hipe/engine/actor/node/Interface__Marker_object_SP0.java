...
public class Interface__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.Interface__Marker>> ports;		
		
	public Interface__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.Interface__Marker>(getSelf(), name2actor.get("Interface__Marker_CONTEXT__TRG__tPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Interface__Marker>(getSelf(), name2actor.get("Interface__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Interface__Marker>(getSelf(), name2actor.get("Interface__Marker_CREATE__TRG__tType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.Interface__Marker> message) {
		for(Port<ModiscoTGG.Interface__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.Interface__Marker> message) {
		for(Port<ModiscoTGG.Interface__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.Interface__Marker> message) {
		for(Port<?> port : ports) {
