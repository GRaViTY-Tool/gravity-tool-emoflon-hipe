...
public class TypeParameter__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypeParameter__Marker>> ports;		
		
	public TypeParameter__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.TypeParameter__Marker>(getSelf(), name2actor.get("TypeParameter__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeParameter__Marker>(getSelf(), name2actor.get("TypeParameter__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeParameter__Marker>(getSelf(), name2actor.get("TypeParameter__Marker_CREATE__TRG__tClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.TypeParameter__Marker> message) {
		for(Port<ModiscoTGG.TypeParameter__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.TypeParameter__Marker> message) {
		for(Port<ModiscoTGG.TypeParameter__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.TypeParameter__Marker> message) {
		for(Port<?> port : ports) {
