...
public class MFieldNameToTField_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.MFieldNameToTField>> ports;		
		
	public MFieldNameToTField_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MFieldNameToTField>(getSelf(), name2actor.get("FieldName__Marker_CREATE__CORR__eFieldDeclarationToTField_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MFieldNameToTField>(getSelf(), name2actor.get("FieldSignature__Marker_CONTEXT__CORR__eFieldDeclarationToTField_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MFieldNameToTField>(getSelf(), name2actor.get("MFieldNameToTField_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MFieldNameToTField>(getSelf(), name2actor.get("MFieldNameToTField_target_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MFieldNameToTField> message) {
		for(Port<ModiscoTGG.MFieldNameToTField> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MFieldNameToTField> message) {
		for(Port<ModiscoTGG.MFieldNameToTField> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MFieldNameToTField> message) {
		for(Port<?> port : ports) {
