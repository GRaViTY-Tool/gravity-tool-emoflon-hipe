...
public class PrimitiveTypeChar_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.PrimitiveTypeChar>> ports;		
		
	public PrimitiveTypeChar_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeChar>(getSelf(), name2actor.get("Model_orphanTypes_4_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeChar>(getSelf(), name2actor.get("PrimitiveTypeIsChar__Marker_CREATE__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeChar>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeChar>(getSelf(), name2actor.get("TypeToTAbstractType_source_11_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.PrimitiveTypeChar> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeChar> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.PrimitiveTypeChar> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeChar> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.PrimitiveTypeChar> message) {
		for(Port<?> port : ports) {
