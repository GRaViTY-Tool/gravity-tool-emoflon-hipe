...
public class ParameterizedType_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.ParameterizedType>> ports;		
		
	public ParameterizedType_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ParameterizedType>(getSelf(), name2actor.get("Model_orphanTypes_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ParameterizedType>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CREATE__SRC__eParameterizedType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.ParameterizedType>(getSelf(), name2actor.get("ParameterizedType_type_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ParameterizedType>(getSelf(), name2actor.get("TypeToTAbstractType_source_8_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.ParameterizedType> message) {
		for(Port<org.eclipse.modisco.java.ParameterizedType> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.ParameterizedType> message) {
		for(Port<org.eclipse.modisco.java.ParameterizedType> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.ParameterizedType> message) {
		for(Port<?> port : ports) {
