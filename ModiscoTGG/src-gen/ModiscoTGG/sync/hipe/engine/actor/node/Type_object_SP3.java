...
public class Type_object_SP3 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.Type>> ports;		
		
	public Type_object_SP3() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Type>(getSelf(), name2actor.get("AnnotationMemberValueClass__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Type>(getSelf(), name2actor.get("FieldType__Marker_CONTEXT__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Type>(getSelf(), name2actor.get("MethodParameterType__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Type>(getSelf(), name2actor.get("SignatureReturnType__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.Type> message) {
		for(Port<org.eclipse.modisco.java.Type> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.Type> message) {
		for(Port<org.eclipse.modisco.java.Type> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.Type> message) {
		for(Port<?> port : ports) {
