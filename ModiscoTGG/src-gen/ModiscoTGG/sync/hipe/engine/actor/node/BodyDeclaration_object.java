...
public class BodyDeclaration_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.BodyDeclaration>> ports;		
		
	public BodyDeclaration_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.BodyDeclaration>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.BodyDeclaration>(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__Marker_CONTEXT__SRC__astNode_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.BodyDeclaration>(getSelf(), name2actor.get("BodyDeclaration_annotations_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.BodyDeclaration> message) {
		for(Port<org.eclipse.modisco.java.BodyDeclaration> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.BodyDeclaration> message) {
		for(Port<org.eclipse.modisco.java.BodyDeclaration> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.BodyDeclaration> message) {
		for(Port<?> port : ports) {
