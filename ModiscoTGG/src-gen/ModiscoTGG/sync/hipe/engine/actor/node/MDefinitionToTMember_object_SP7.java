...
public class MDefinitionToTMember_object_SP7 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MDefinitionToTMember>> ports;		
		
	public MDefinitionToTMember_object_SP7() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("FieldAccessWrite__Marker_CONTEXT__CORR__mMemberToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_target_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MethodInvocationRecursive__Marker_CONTEXT__CORR__mMemberToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("SyntethicMethod__Marker_CONTEXT__CORR__mDefinitionToTMember_0_reference"), this::returnTrue));
	}
