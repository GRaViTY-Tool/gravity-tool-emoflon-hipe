...
public class Annotation_object_SP1 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.Annotation>> ports;		
		
	public Annotation_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Annotation>(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__Marker_CREATE__SRC__mValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Annotation>(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__Marker_CREATE__SRC__mValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Annotation>(getSelf(), name2actor.get("AnnotationTypeAccess__Marker_CONTEXT__SRC__Annotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.Annotation>(getSelf(), name2actor.get("Annotation_values_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.Annotation> message) {
		for(Port<org.eclipse.modisco.java.Annotation> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.Annotation> message) {
		for(Port<org.eclipse.modisco.java.Annotation> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.Annotation> message) {
		for(Port<?> port : ports) {
