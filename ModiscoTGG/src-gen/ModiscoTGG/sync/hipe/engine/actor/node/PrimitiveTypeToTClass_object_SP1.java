...
public class PrimitiveTypeToTClass_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PrimitiveTypeToTClass>> ports;		
		
	public PrimitiveTypeToTClass_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsChar__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_7_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_5_reference"), this::returnTrue));
	}
