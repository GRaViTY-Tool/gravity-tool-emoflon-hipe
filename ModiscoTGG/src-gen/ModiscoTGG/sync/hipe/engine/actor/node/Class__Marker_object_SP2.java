...
public class Class__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.Class__Marker>> ports;		
		
	public Class__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.Class__Marker>(getSelf(), name2actor.get("Class__Marker_CONTEXT__CORR__mPackageToTPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Class__Marker>(getSelf(), name2actor.get("Class__Marker_CONTEXT__TRG__pg_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Class__Marker>(getSelf(), name2actor.get("Class__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.Class__Marker> message) {
		for(Port<ModiscoTGG.Class__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.Class__Marker> message) {
		for(Port<ModiscoTGG.Class__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.Class__Marker> message) {
		for(Port<?> port : ports) {
