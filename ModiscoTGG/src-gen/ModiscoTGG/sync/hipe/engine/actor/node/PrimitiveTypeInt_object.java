...
public class PrimitiveTypeInt_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.PrimitiveTypeInt>> ports;		
		
	public PrimitiveTypeInt_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeInt>(getSelf(), name2actor.get("Model_orphanTypes_7_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeInt>(getSelf(), name2actor.get("PrimitiveTypeIsInt__Marker_CREATE__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeInt>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_5_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeInt>(getSelf(), name2actor.get("TypeToTAbstractType_source_14_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.PrimitiveTypeInt> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeInt> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.PrimitiveTypeInt> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeInt> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.PrimitiveTypeInt> message) {
		for(Port<?> port : ports) {
