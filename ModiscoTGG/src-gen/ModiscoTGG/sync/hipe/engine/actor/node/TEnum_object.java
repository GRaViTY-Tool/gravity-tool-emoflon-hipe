...
public class TEnum_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TEnum>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TEnum>(getSelf(), name2actor.get("EnumInAnonymous__Marker_CREATE__TRG__tAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TEnum>(getSelf(), name2actor.get("Enum__Marker_CREATE__TRG__tAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TEnum>(getSelf(), name2actor.get("NestedEnum__Marker_CREATE__TRG__tAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TEnum>(getSelf(), name2actor.get("TClass_tAnnotation_2_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TEnum> message) {
		for(Port<org.gravity.typegraph.basic.TEnum> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TEnum> message) {
		for(Port<org.gravity.typegraph.basic.TEnum> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TEnum> message) {
		for(Port<?> port : ports) {
