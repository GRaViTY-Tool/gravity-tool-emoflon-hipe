...
public class TModifier_object_SP1 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TModifier>> ports;		
		
	public TModifier_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TModifier>(getSelf(), name2actor.get("InterfaceMember__Marker_CREATE__TRG__tModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TModifier>(getSelf(), name2actor.get("MemberInClassVisibility__Marker_CREATE__TRG__tModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TModifier>(getSelf(), name2actor.get("TAbstractType_tModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TModifier>(getSelf(), name2actor.get("TypesVisibility__Marker_CREATE__TRG__tModifier_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TModifier> message) {
		for(Port<org.gravity.typegraph.basic.TModifier> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TModifier> message) {
		for(Port<org.gravity.typegraph.basic.TModifier> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TModifier> message) {
		for(Port<?> port : ports) {
