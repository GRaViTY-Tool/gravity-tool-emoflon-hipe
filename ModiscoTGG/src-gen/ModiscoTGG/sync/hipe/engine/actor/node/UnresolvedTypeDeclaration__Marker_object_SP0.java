...
public class UnresolvedTypeDeclaration__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.UnresolvedTypeDeclaration__Marker>> ports;		
		
	public UnresolvedTypeDeclaration__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CREATE__TRG__tClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		for(Port<ModiscoTGG.UnresolvedTypeDeclaration__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		for(Port<ModiscoTGG.UnresolvedTypeDeclaration__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		for(Port<?> port : ports) {
