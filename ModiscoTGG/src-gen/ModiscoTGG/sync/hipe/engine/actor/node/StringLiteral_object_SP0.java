...
public class StringLiteral_object_SP0 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.StringLiteral>> ports;		
		
	public StringLiteral_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.StringLiteral>(getSelf(), name2actor.get("AnnotationMemberValueStringArray__Marker_CREATE__SRC__mValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.StringLiteral>(getSelf(), name2actor.get("AnnotationMemberValueString__Marker_CREATE__SRC__mValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.StringLiteral>(getSelf(), name2actor.get("ArrayInitializer_expressions_4_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.StringLiteral> message) {
		for(Port<org.eclipse.modisco.java.StringLiteral> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.StringLiteral> message) {
		for(Port<org.eclipse.modisco.java.StringLiteral> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.StringLiteral> message) {
		for(Port<?> port : ports) {
