...
public class ClassInAnonymous__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ClassInAnonymous__Marker>> ports;		
		
	public ClassInAnonymous__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ClassInAnonymous__Marker>(getSelf(), name2actor.get("ClassInAnonymous__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ClassInAnonymous__Marker>(getSelf(), name2actor.get("ClassInAnonymous__Marker_CONTEXT__TRG__pg_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ClassInAnonymous__Marker>(getSelf(), name2actor.get("ClassInAnonymous__Marker_CREATE__CORR__mClassDeclarationToTClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ClassInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.ClassInAnonymous__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ClassInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.ClassInAnonymous__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ClassInAnonymous__Marker> message) {
		for(Port<?> port : ports) {
