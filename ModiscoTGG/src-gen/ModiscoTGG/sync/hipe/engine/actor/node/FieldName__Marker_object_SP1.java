...
public class FieldName__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.FieldName__Marker>> ports;		
		
	public FieldName__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.FieldName__Marker>(getSelf(), name2actor.get("FieldName__Marker_CONTEXT__CORR__modelToTypeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.FieldName__Marker>(getSelf(), name2actor.get("FieldName__Marker_CONTEXT__TRG__typeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.FieldName__Marker>(getSelf(), name2actor.get("FieldName__Marker_CREATE__SRC__mFieldName_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.FieldName__Marker> message) {
		for(Port<ModiscoTGG.FieldName__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.FieldName__Marker> message) {
		for(Port<ModiscoTGG.FieldName__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.FieldName__Marker> message) {
		for(Port<?> port : ports) {
