...
public class ArrayType_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.ArrayType>> ports;		
		
	public ArrayType_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ArrayType>(getSelf(), name2actor.get("ArrayTypeToClass__Marker_CREATE__SRC__eArrayType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.ArrayType>(getSelf(), name2actor.get("ArrayType_elementType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ArrayType>(getSelf(), name2actor.get("Model_orphanTypes_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.ArrayType>(getSelf(), name2actor.get("TypeToTAbstractType_source_3_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.ArrayType> message) {
		for(Port<org.eclipse.modisco.java.ArrayType> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.ArrayType> message) {
		for(Port<org.eclipse.modisco.java.ArrayType> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.ArrayType> message) {
		for(Port<?> port : ports) {
