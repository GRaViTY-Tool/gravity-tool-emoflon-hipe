...
public class MSignatureToTSignature_object_SP3 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSignatureToTSignature>> ports;		
		
	public MSignatureToTSignature_object_SP3() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MSignatureToTSignature_source_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MSignatureToTSignature_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MethodParameterLinkFirst__Marker_CONTEXT__CORR__mmethodSignatureToTMethodSignature_0_reference"), this::returnTrue));
	}
