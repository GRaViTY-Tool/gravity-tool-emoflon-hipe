...
public class MFlowToTFlow_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MFlowToTFlow>> ports;		
		
	public MFlowToTFlow_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MFlowToTFlow>(getSelf(), name2actor.get("Flow__Marker_CREATE__CORR__mFlowToTFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MFlowToTFlow>(getSelf(), name2actor.get("MFlowToTFlow_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MFlowToTFlow>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__CORR__mFlowToTFlow_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MFlowToTFlow> message) {
		for(Port<ModiscoTGG.MFlowToTFlow> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MFlowToTFlow> message) {
		for(Port<ModiscoTGG.MFlowToTFlow> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MFlowToTFlow> message) {
		for(Port<?> port : ports) {
