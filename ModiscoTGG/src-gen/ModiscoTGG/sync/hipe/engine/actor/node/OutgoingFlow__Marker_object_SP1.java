...
public class OutgoingFlow__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.OutgoingFlow__Marker>> ports;		
		
	public OutgoingFlow__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.OutgoingFlow__Marker>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__CORR__mTargetToTTarget_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.OutgoingFlow__Marker>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__SRC__mFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.OutgoingFlow__Marker>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__TRG__tTarget_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.OutgoingFlow__Marker> message) {
		for(Port<ModiscoTGG.OutgoingFlow__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.OutgoingFlow__Marker> message) {
		for(Port<ModiscoTGG.OutgoingFlow__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.OutgoingFlow__Marker> message) {
		for(Port<?> port : ports) {
