...
public class ClassLiteralToClassNode_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.ClassLiteralToClassNode>> ports;		
		
	public ClassLiteralToClassNode_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.ClassLiteralToClassNode>(getSelf(), name2actor.get("AnnotationMemberValueClassArray__Marker_CREATE__CORR__mValueToTValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ClassLiteralToClassNode>(getSelf(), name2actor.get("AnnotationMemberValueClass__Marker_CREATE__CORR__mValueToTValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ClassLiteralToClassNode>(getSelf(), name2actor.get("ClassLiteralToClassNode_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ClassLiteralToClassNode>(getSelf(), name2actor.get("ClassLiteralToClassNode_target_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ClassLiteralToClassNode> message) {
		for(Port<ModiscoTGG.ClassLiteralToClassNode> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ClassLiteralToClassNode> message) {
		for(Port<ModiscoTGG.ClassLiteralToClassNode> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ClassLiteralToClassNode> message) {
		for(Port<?> port : ports) {
