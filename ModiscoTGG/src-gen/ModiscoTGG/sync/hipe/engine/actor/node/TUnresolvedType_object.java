...
public class TUnresolvedType_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TUnresolvedType>> ports;		
		
	public TUnresolvedType_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TUnresolvedType>(getSelf(), name2actor.get("TUnresolvedType_pg_15_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TUnresolvedType>(getSelf(), name2actor.get("TypeToTAbstractType_target_14_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TUnresolvedType>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CREATE__TRG__tClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TUnresolvedType> message) {
		for(Port<org.gravity.typegraph.basic.TUnresolvedType> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TUnresolvedType> message) {
		for(Port<org.gravity.typegraph.basic.TUnresolvedType> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TUnresolvedType> message) {
		for(Port<?> port : ports) {
