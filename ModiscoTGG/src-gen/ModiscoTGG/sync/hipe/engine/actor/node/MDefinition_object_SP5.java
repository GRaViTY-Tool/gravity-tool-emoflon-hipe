...
public class MDefinition_object_SP5 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MDefinition>> ports;		
		
	public MDefinition_object_SP5() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("InterfaceMember__Marker_CONTEXT__SRC__mDefinition_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("MDefinition_mAbstractFieldAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("MemberInClassVisibility__Marker_CONTEXT__SRC__mDefinition_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("TextAnnotations__Marker_CONTEXT__SRC__mMember_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MDefinition> message) {
		for(Port<org.gravity.modisco.MDefinition> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MDefinition> message) {
		for(Port<org.gravity.modisco.MDefinition> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MDefinition> message) {
		for(Port<?> port : ports) {
