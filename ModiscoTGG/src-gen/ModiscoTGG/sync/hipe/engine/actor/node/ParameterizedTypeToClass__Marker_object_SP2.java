...
public class ParameterizedTypeToClass__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ParameterizedTypeToClass__Marker>> ports;		
		
	public ParameterizedTypeToClass__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ParameterizedTypeToClass__Marker>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CONTEXT__CORR__eTypeToTAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ParameterizedTypeToClass__Marker>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ParameterizedTypeToClass__Marker>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CREATE__CORR__eParameterizedTypeToTAbstractType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ParameterizedTypeToClass__Marker> message) {
		for(Port<ModiscoTGG.ParameterizedTypeToClass__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ParameterizedTypeToClass__Marker> message) {
		for(Port<ModiscoTGG.ParameterizedTypeToClass__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ParameterizedTypeToClass__Marker> message) {
		for(Port<?> port : ports) {
