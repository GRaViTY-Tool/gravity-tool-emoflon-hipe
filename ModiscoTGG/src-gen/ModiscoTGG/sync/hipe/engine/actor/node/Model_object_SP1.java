...
public class Model_object_SP1 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.Model>> ports;		
		
	public Model_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Model>(getSelf(), name2actor.get("ModelToTypeGraph_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.Model>(getSelf(), name2actor.get("Model_orphanTypes_6_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Model>(getSelf(), name2actor.get("PrimitiveTypeIsBoolean__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Model>(getSelf(), name2actor.get("PrimitiveTypeIsShort__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.Model> message) {
		for(Port<org.eclipse.modisco.java.Model> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.Model> message) {
		for(Port<org.eclipse.modisco.java.Model> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.Model> message) {
		for(Port<?> port : ports) {
