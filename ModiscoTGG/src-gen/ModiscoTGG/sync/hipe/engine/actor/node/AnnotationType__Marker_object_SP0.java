...
public class AnnotationType__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.AnnotationType__Marker>> ports;		
		
	public AnnotationType__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationType__Marker>(getSelf(), name2actor.get("AnnotationType__Marker_CONTEXT__CORR__mPackageToTPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationType__Marker>(getSelf(), name2actor.get("AnnotationType__Marker_CONTEXT__TRG__tPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationType__Marker>(getSelf(), name2actor.get("AnnotationType__Marker_CREATE__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationType__Marker>(getSelf(), name2actor.get("AnnotationType__Marker_CREATE__SRC__mType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.AnnotationType__Marker> message) {
		for(Port<ModiscoTGG.AnnotationType__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.AnnotationType__Marker> message) {
		for(Port<ModiscoTGG.AnnotationType__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.AnnotationType__Marker> message) {
		for(Port<?> port : ports) {
