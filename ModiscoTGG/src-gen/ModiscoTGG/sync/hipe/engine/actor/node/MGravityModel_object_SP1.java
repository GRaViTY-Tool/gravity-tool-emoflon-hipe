...
public class MGravityModel_object_SP1 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MGravityModel>> ports;		
		
	public MGravityModel_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<org.gravity.modisco.MGravityModel>(getSelf(), name2actor.get("MGravityModel_mFieldNames_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MGravityModel>(getSelf(), name2actor.get("MGravityModel_typeParameters_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MGravityModel>(getSelf(), name2actor.get("MethodName__Marker_CONTEXT__SRC__mModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MGravityModel>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MGravityModel> message) {
		for(Port<org.gravity.modisco.MGravityModel> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MGravityModel> message) {
		for(Port<org.gravity.modisco.MGravityModel> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MGravityModel> message) {
		for(Port<?> port : ports) {
