...
public class TypeAccess_object_SP2 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.TypeAccess>> ports;		
		
	public TypeAccess_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeAccess>(getSelf(), name2actor.get("AnnotationMemberValueClassArray__Marker_CREATE__SRC__mAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeAccess>(getSelf(), name2actor.get("AnnotationTypeDeclaration_usagesInTypeAccess_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeAccess>(getSelf(), name2actor.get("ClassDeclaration_usagesInTypeAccess_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeAccess>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CREATE__SRC__eTypeAccess_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.TypeAccess> message) {
		for(Port<org.eclipse.modisco.java.TypeAccess> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.TypeAccess> message) {
		for(Port<org.eclipse.modisco.java.TypeAccess> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.TypeAccess> message) {
		for(Port<?> port : ports) {
