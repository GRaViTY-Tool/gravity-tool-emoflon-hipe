...
public class MMethodNameToTMethod_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.MMethodNameToTMethod>> ports;		
		
	public MMethodNameToTMethod_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MMethodNameToTMethod>(getSelf(), name2actor.get("MMethodNameToTMethod_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MMethodNameToTMethod>(getSelf(), name2actor.get("MMethodNameToTMethod_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MMethodNameToTMethod>(getSelf(), name2actor.get("MethodName__Marker_CREATE__CORR__mNameToTName_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MMethodNameToTMethod>(getSelf(), name2actor.get("MethodSignature__Marker_CONTEXT__CORR__mNameToTName_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MMethodNameToTMethod> message) {
		for(Port<ModiscoTGG.MMethodNameToTMethod> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MMethodNameToTMethod> message) {
		for(Port<ModiscoTGG.MMethodNameToTMethod> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MMethodNameToTMethod> message) {
		for(Port<?> port : ports) {
