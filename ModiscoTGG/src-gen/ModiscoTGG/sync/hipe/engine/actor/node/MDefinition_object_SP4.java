...
public class MDefinition_object_SP4 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MDefinition>> ports;		
		
	public MDefinition_object_SP4() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("ClassDeclaration_bodyDeclarations_6_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("ConstructorInvocation__Marker_CONTEXT__SRC__mMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("MDefinition_comments_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MDefinition>(getSelf(), name2actor.get("MDefinition_modifier_0_reference"), this::returnTrue));
	}
