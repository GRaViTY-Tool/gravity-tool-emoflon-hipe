...
public class LinkDefinitionToType__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.LinkDefinitionToType__Marker>> ports;		
		
	public LinkDefinitionToType__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.LinkDefinitionToType__Marker>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__CORR__mDefinitionToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.LinkDefinitionToType__Marker>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.LinkDefinitionToType__Marker>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__CORR__mTypeToTType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.LinkDefinitionToType__Marker> message) {
		for(Port<ModiscoTGG.LinkDefinitionToType__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.LinkDefinitionToType__Marker> message) {
		for(Port<ModiscoTGG.LinkDefinitionToType__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.LinkDefinitionToType__Marker> message) {
		for(Port<?> port : ports) {
