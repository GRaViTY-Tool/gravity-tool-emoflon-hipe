...
public class MDefinitionToTMember_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MDefinitionToTMember>> ports;		
		
	public MDefinitionToTMember_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("LinkDefinitionToAnonymous__Marker_CONTEXT__CORR__mDefinitionToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_source_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MethodOverriding__Marker_CONTEXT__CORR__mMethodToTMethod_0_reference"), this::returnTrue));
	}
