...
public class TCall_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TCall>> ports;		
		
	public TCall_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TCall>(getSelf(), name2actor.get("ClassInstanceCreation__Marker_CREATE__TRG__tAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TCall>(getSelf(), name2actor.get("MethodInvocationRecursive__Marker_CREATE__TRG__tAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TCall>(getSelf(), name2actor.get("SuperConstructorInvocation__Marker_CREATE__TRG__tAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TCall>(getSelf(), name2actor.get("TCall_tSource_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TCall> message) {
		for(Port<org.gravity.typegraph.basic.TCall> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TCall> message) {
		for(Port<org.gravity.typegraph.basic.TCall> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TCall> message) {
		for(Port<?> port : ports) {
