...
public class TypeParameter_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.TypeParameter>> ports;		
		
	public TypeParameter_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeParameter>(getSelf(), name2actor.get("MGravityModel_typeParameters_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeParameter>(getSelf(), name2actor.get("TypeParameter__Marker_CREATE__SRC__typeParameter_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeParameter>(getSelf(), name2actor.get("TypeToTAbstractType_source_18_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.TypeParameter> message) {
		for(Port<org.eclipse.modisco.java.TypeParameter> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.TypeParameter> message) {
		for(Port<org.eclipse.modisco.java.TypeParameter> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.TypeParameter> message) {
		for(Port<?> port : ports) {
