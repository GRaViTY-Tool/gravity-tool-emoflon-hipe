...
public class ConstructorDefinition__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ConstructorDefinition__Marker>> ports;		
		
	public ConstructorDefinition__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ConstructorDefinition__Marker>(getSelf(), name2actor.get("ConstructorDefinition__Marker_CONTEXT__CORR__mSignatureToTMethodSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ConstructorDefinition__Marker>(getSelf(), name2actor.get("ConstructorDefinition__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ConstructorDefinition__Marker>(getSelf(), name2actor.get("ConstructorDefinition__Marker_CREATE__SRC__mDefinition_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ConstructorDefinition__Marker> message) {
		for(Port<ModiscoTGG.ConstructorDefinition__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ConstructorDefinition__Marker> message) {
		for(Port<ModiscoTGG.ConstructorDefinition__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ConstructorDefinition__Marker> message) {
		for(Port<?> port : ports) {
