...
public class MemberInEnumVisibility__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MemberInEnumVisibility__Marker>> ports;		
		
	public MemberInEnumVisibility__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MemberInEnumVisibility__Marker>(getSelf(), name2actor.get("MemberInEnumVisibility__Marker_CONTEXT__CORR__defToMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MemberInEnumVisibility__Marker>(getSelf(), name2actor.get("MemberInEnumVisibility__Marker_CONTEXT__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MemberInEnumVisibility__Marker>(getSelf(), name2actor.get("MemberInEnumVisibility__Marker_CREATE__CORR__mModifierToTModifier_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MemberInEnumVisibility__Marker> message) {
		for(Port<ModiscoTGG.MemberInEnumVisibility__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MemberInEnumVisibility__Marker> message) {
		for(Port<ModiscoTGG.MemberInEnumVisibility__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MemberInEnumVisibility__Marker> message) {
		for(Port<?> port : ports) {
