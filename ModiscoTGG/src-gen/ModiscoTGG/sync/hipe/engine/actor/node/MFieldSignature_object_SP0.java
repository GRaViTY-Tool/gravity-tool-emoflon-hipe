...
public class MFieldSignature_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MFieldSignature>> ports;		
		
	public MFieldSignature_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldSignature>(getSelf(), name2actor.get("FieldDefinition__Marker_CONTEXT__SRC__mSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldSignature>(getSelf(), name2actor.get("MAbstractFlowElementToTAbstractFlowElement_source_7_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MFieldSignature>(getSelf(), name2actor.get("MFieldSignature_type_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldSignature>(getSelf(), name2actor.get("MSignatureToTSignature_source_1_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MFieldSignature> message) {
		for(Port<org.gravity.modisco.MFieldSignature> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MFieldSignature> message) {
		for(Port<org.gravity.modisco.MFieldSignature> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MFieldSignature> message) {
		for(Port<?> port : ports) {
