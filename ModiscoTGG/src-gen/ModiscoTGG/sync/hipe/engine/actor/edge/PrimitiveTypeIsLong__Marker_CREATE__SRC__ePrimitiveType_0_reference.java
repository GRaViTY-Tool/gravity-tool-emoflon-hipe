...
public class PrimitiveTypeIsLong__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeIsLong__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeIsLong__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeLong, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeLong, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeLong, Set<ModiscoTGG.PrimitiveTypeIsLong__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeIsLong__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypeIsLong__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeIsLong__Marker source, org.eclipse.modisco.java.PrimitiveTypeLong target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeIsLong__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeIsLong__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeLong) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeIsLong__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeIsLong__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeLong target = (org.eclipse.modisco.java.PrimitiveTypeLong) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeLong> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeIsLong__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeIsLong__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeIsLong__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeIsLong__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeLong> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeIsLong__Marker, org.eclipse.modisco.java.PrimitiveTypeLong> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeIsLong__Marker, org.eclipse.modisco.java.PrimitiveTypeLong> message) {
		Set<ModiscoTGG.PrimitiveTypeIsLong__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeIsLong__Marker) {
			leftChanged(message);
		}
		else {
