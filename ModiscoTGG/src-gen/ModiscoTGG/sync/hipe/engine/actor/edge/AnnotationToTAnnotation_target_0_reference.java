...
public class AnnotationToTAnnotation_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationToTAnnotation, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationToTAnnotation, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotation, Set<ModiscoTGG.AnnotationToTAnnotation>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationToTAnnotation_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberKey__BWD_3_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberKey__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberKey__FWD_37_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationTypeAccess__BWD_690_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationTypeAccess__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationTypeAccess__FWD_733_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationToTAnnotation source, org.gravity.typegraph.basic.annotations.TAnnotation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationToTAnnotation> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationToTAnnotation>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotation) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationToTAnnotation) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationToTAnnotation> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotation target = (org.gravity.typegraph.basic.annotations.TAnnotation) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotation> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationToTAnnotation> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationToTAnnotation source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationToTAnnotation> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationToTAnnotation> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationToTAnnotation, org.gravity.typegraph.basic.annotations.TAnnotation> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationToTAnnotation, org.gravity.typegraph.basic.annotations.TAnnotation> message) {
		Set<ModiscoTGG.AnnotationToTAnnotation> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationToTAnnotation) {
			leftChanged(message);
		}
		else {
