...
public class PrimitiveTypeIsBoolean__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeIsBoolean__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeIsBoolean__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeBoolean, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeBoolean, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeBoolean, Set<ModiscoTGG.PrimitiveTypeIsBoolean__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeIsBoolean__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypeIsBoolean__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeIsBoolean__Marker source, org.eclipse.modisco.java.PrimitiveTypeBoolean target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeIsBoolean__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeBoolean) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeIsBoolean__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeBoolean target = (org.eclipse.modisco.java.PrimitiveTypeBoolean) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeBoolean> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeIsBoolean__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeBoolean> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeIsBoolean__Marker, org.eclipse.modisco.java.PrimitiveTypeBoolean> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeIsBoolean__Marker, org.eclipse.modisco.java.PrimitiveTypeBoolean> message) {
		Set<ModiscoTGG.PrimitiveTypeIsBoolean__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeIsBoolean__Marker) {
			leftChanged(message);
		}
		else {
