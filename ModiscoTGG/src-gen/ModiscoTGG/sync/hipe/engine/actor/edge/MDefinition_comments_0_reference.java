...
public class MDefinition_comments_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.ASTNode, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.ASTNode, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.Comment, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.Comment, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.Comment, Set<org.eclipse.modisco.java.ASTNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MDefinition_comments_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TextAnnotations__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TextAnnotations__FWD_5871_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.ASTNode source, org.eclipse.modisco.java.Comment target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.ASTNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.ASTNode>();
			pending.put((org.eclipse.modisco.java.Comment) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.ASTNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.ASTNode> message) {
		...
		for(org.eclipse.modisco.java.Comment target : message.input.getComments()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Comment> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.ASTNode> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.ASTNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.ASTNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.ASTNode> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.Comment> message) {
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.ASTNode, org.eclipse.modisco.java.Comment> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.ASTNode, org.eclipse.modisco.java.Comment> message) {
		Set<org.eclipse.modisco.java.ASTNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.ASTNode) {
			leftChanged(message);
		}
		else {
