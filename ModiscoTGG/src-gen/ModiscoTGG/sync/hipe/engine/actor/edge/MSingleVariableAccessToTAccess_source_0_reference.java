...
public class MSingleVariableAccessToTAccess_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MSingleVariableAccessToTAccess, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MSingleVariableAccessToTAccess, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MSingleVariableAccess, Set<ModiscoTGG.MSingleVariableAccessToTAccess>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MSingleVariableAccessToTAccess_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessReadWrite__CONSISTENCY_triangle_13_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MSingleVariableAccessToTAccess source, org.gravity.modisco.MSingleVariableAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MSingleVariableAccessToTAccess> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MSingleVariableAccessToTAccess>();
			pending.put((org.gravity.modisco.MSingleVariableAccess) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MSingleVariableAccessToTAccess) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
		...
		org.gravity.modisco.MSingleVariableAccess target = (org.gravity.modisco.MSingleVariableAccess) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSingleVariableAccess> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MSingleVariableAccessToTAccess> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MSingleVariableAccessToTAccess source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MSingleVariableAccessToTAccess> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MSingleVariableAccessToTAccess, org.gravity.modisco.MSingleVariableAccess> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MSingleVariableAccessToTAccess, org.gravity.modisco.MSingleVariableAccess> message) {
		Set<ModiscoTGG.MSingleVariableAccessToTAccess> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MSingleVariableAccessToTAccess) {
			leftChanged(message);
		}
		else {
