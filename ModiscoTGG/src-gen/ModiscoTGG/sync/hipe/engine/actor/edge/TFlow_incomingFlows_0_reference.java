...
public class TFlow_incomingFlows_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TAbstractFlowElement, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TAbstractFlowElement, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TAbstractFlowElement, Set<org.gravity.typegraph.basic.TAbstractFlowElement>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TFlow_incomingFlows_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__BWD_2910_junction"), this::check_constraint_140));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_0_triangleJunction"), this::check_constraint_142));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TAbstractFlowElement source, org.gravity.typegraph.basic.TAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TAbstractFlowElement> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TAbstractFlowElement>();
			pending.put((org.gravity.typegraph.basic.TAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TAbstractFlowElement) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		...
		for(org.gravity.typegraph.basic.TAbstractFlowElement target : message.input.getIncomingFlows()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TAbstractFlowElement> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TAbstractFlowElement source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TAbstractFlowElement> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TAbstractFlowElement, org.gravity.typegraph.basic.TAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TAbstractFlowElement, org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		Set<org.gravity.typegraph.basic.TAbstractFlowElement> pendingSources = pending.get(message.target);
	...
	public boolean check_constraint_140(EdgeMatch edge) {
	...
	public boolean check_constraint_142(EdgeMatch edge) {
		org.gravity.typegraph.basic.TAbstractFlowElement tFlow = (org.gravity.typegraph.basic.TAbstractFlowElement) edge.source();
		org.gravity.typegraph.basic.TAbstractFlowElement tSource = (org.gravity.typegraph.basic.TAbstractFlowElement) edge.target();
		boolean predicate = !tFlow.equals(tSource);
		edge.setConstraintSatisfied(predicate);
