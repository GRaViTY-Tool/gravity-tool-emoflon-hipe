...
public class MEntryToTParameter_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MEntryToTParameter, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MEntryToTParameter, Set<EdgeMatch>>();
	private Map<org.gravity.modisco.MEntry, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MEntry, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MEntry, Set<ModiscoTGG.MEntryToTParameter>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MEntryToTParameter_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLinkFirst__BWD_4059_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLinkFirst__FWD_4099_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__BWD_4109_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__BWD_4114_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__FWD_4149_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterLink__FWD_4154_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameter__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MEntryToTParameter source, org.gravity.modisco.MEntry target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MEntryToTParameter> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MEntryToTParameter>();
			pending.put((org.gravity.modisco.MEntry) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MEntryToTParameter) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MEntryToTParameter> message) {
		...
		org.gravity.modisco.MEntry target = (org.gravity.modisco.MEntry) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MEntry> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MEntryToTParameter> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MEntryToTParameter source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MEntryToTParameter> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MEntryToTParameter> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MEntry> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MEntryToTParameter, org.gravity.modisco.MEntry> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MEntryToTParameter, org.gravity.modisco.MEntry> message) {
		Set<ModiscoTGG.MEntryToTParameter> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MEntryToTParameter) {
			leftChanged(message);
		}
		else {
