...
public class MAbstractMethodDefinition_usages_1_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.AbstractMethodDeclaration, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.AbstractMethodDeclaration, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AbstractMethodInvocation, Set<org.eclipse.modisco.java.AbstractMethodDeclaration>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MAbstractMethodDefinition_usages_1_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__CONSISTENCY_2052_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ConstructorInvocation__FWD_2097_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_5600_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SuperConstructorInvocation__FWD_5645_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.AbstractMethodDeclaration source, org.eclipse.modisco.java.AbstractMethodInvocation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.AbstractMethodDeclaration> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.AbstractMethodDeclaration>();
			pending.put((org.eclipse.modisco.java.AbstractMethodInvocation) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.AbstractMethodDeclaration) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.AbstractMethodDeclaration> message) {
		...
		for(org.eclipse.modisco.java.AbstractMethodInvocation target : message.input.getUsages()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractMethodInvocation> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.AbstractMethodDeclaration> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.AbstractMethodDeclaration source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.AbstractMethodDeclaration> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.AbstractMethodDeclaration> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.AbstractMethodDeclaration, org.eclipse.modisco.java.AbstractMethodInvocation> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.AbstractMethodDeclaration, org.eclipse.modisco.java.AbstractMethodInvocation> message) {
		Set<org.eclipse.modisco.java.AbstractMethodDeclaration> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.AbstractMethodDeclaration) {
			leftChanged(message);
		}
		else {
