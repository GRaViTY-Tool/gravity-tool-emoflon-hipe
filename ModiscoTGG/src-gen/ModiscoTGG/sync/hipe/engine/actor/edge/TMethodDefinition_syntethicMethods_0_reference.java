...
public class TMethodDefinition_syntethicMethods_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TMethodDefinition, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TMethodDefinition, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TSyntethicMethod, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TSyntethicMethod, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TSyntethicMethod, Set<org.gravity.typegraph.basic.TMethodDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TMethodDefinition_syntethicMethods_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SyntethicMethod__BWD_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod_tSyntethic_syntethicMethods_incoming_TRG__FILTER_NAC_5831_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod_tSyntethic_syntethicMethods_incoming_TRG__FILTER_NAC_5831_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TMethodDefinition source, org.gravity.typegraph.basic.TSyntethicMethod target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TMethodDefinition> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TMethodDefinition>();
			pending.put((org.gravity.typegraph.basic.TSyntethicMethod) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TMethodDefinition) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TMethodDefinition> message) {
		...
		for(org.gravity.typegraph.basic.TSyntethicMethod target : message.input.getSyntethicMethods()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TSyntethicMethod> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TMethodDefinition> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TMethodDefinition source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TMethodDefinition> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TMethodDefinition> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TSyntethicMethod> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TMethodDefinition, org.gravity.typegraph.basic.TSyntethicMethod> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TMethodDefinition, org.gravity.typegraph.basic.TSyntethicMethod> message) {
		Set<org.gravity.typegraph.basic.TMethodDefinition> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TMethodDefinition) {
			leftChanged(message);
		}
		else {
