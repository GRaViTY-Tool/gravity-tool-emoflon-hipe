...
public class MDefinition_mMethodInvocations_1_reference extends AbstractActor {
	...
	private Map<org.eclipse.modisco.java.AbstractMethodInvocation, Set<org.gravity.modisco.MDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MDefinition_mMethodInvocations_1_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__FWD_2097_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__FWD_5645_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MDefinition source, org.eclipse.modisco.java.AbstractMethodInvocation target) {
	...
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MDefinition> message) {
		...
		for(org.eclipse.modisco.java.AbstractMethodInvocation target : message.input.getMMethodInvocations()) {
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MDefinition, org.eclipse.modisco.java.AbstractMethodInvocation> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MDefinition, org.eclipse.modisco.java.AbstractMethodInvocation> message) {
