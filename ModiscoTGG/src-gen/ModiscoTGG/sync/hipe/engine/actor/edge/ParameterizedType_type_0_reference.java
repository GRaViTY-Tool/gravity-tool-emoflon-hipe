...
public class ParameterizedType_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.ParameterizedType, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.ParameterizedType, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.TypeAccess, Set<org.eclipse.modisco.java.ParameterizedType>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ParameterizedType_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ParameterizedTypeToClass__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ParameterizedTypeToClass__FWD_4837_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.ParameterizedType source, org.eclipse.modisco.java.TypeAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.ParameterizedType> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.ParameterizedType>();
			pending.put((org.eclipse.modisco.java.TypeAccess) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.ParameterizedType) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.ParameterizedType> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.TypeAccess> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.ParameterizedType> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.ParameterizedType source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.ParameterizedType> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.ParameterizedType> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.ParameterizedType, org.eclipse.modisco.java.TypeAccess> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.ParameterizedType, org.eclipse.modisco.java.TypeAccess> message) {
		Set<org.eclipse.modisco.java.ParameterizedType> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.ParameterizedType) {
			leftChanged(message);
		}
		else {
