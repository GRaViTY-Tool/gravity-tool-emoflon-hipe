...
public class InterfaceInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.InterfaceInAnonymous__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.InterfaceInAnonymous__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AnonymousClassDeclaration, Set<ModiscoTGG.InterfaceInAnonymous__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public InterfaceInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("InterfaceInAnonymous__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.InterfaceInAnonymous__Marker source, org.eclipse.modisco.java.AnonymousClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.InterfaceInAnonymous__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.InterfaceInAnonymous__Marker>();
			pending.put((org.eclipse.modisco.java.AnonymousClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.InterfaceInAnonymous__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.InterfaceInAnonymous__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.InterfaceInAnonymous__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.InterfaceInAnonymous__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.InterfaceInAnonymous__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.InterfaceInAnonymous__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.InterfaceInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.InterfaceInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		Set<ModiscoTGG.InterfaceInAnonymous__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.InterfaceInAnonymous__Marker) {
			leftChanged(message);
		}
		else {
