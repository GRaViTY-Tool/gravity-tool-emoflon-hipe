...
public class AnonymousClassTypeClass__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnonymousClassTypeClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnonymousClassTypeClass__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnonymousClassDeclarationToTClass, Set<ModiscoTGG.AnonymousClassTypeClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnonymousClassTypeClass__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnonymousClassTypeClass__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnonymousClassTypeClass__Marker source, ModiscoTGG.AnonymousClassDeclarationToTClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnonymousClassTypeClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnonymousClassTypeClass__Marker>();
			pending.put((ModiscoTGG.AnonymousClassDeclarationToTClass) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnonymousClassTypeClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnonymousClassTypeClass__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnonymousClassTypeClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnonymousClassTypeClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnonymousClassTypeClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnonymousClassTypeClass__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnonymousClassTypeClass__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnonymousClassTypeClass__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		Set<ModiscoTGG.AnonymousClassTypeClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnonymousClassTypeClass__Marker) {
			leftChanged(message);
		}
		else {
