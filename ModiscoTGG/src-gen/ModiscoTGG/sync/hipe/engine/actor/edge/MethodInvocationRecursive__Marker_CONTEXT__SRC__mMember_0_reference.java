...
public class MethodInvocationRecursive__Marker_CONTEXT__SRC__mMember_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MMethodDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MMethodDefinition, Set<ModiscoTGG.MethodInvocationRecursive__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodInvocationRecursive__Marker_CONTEXT__SRC__mMember_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodInvocationRecursive__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodInvocationRecursive__Marker source, org.gravity.modisco.MMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MMethodDefinition) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodInvocationRecursive__Marker> message) {
		...
		org.gravity.modisco.MMethodDefinition target = (org.gravity.modisco.MMethodDefinition) message.input.getCONTEXT__SRC__mMember();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MMethodDefinition> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MMethodDefinition> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodInvocationRecursive__Marker, org.gravity.modisco.MMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodInvocationRecursive__Marker, org.gravity.modisco.MMethodDefinition> message) {
