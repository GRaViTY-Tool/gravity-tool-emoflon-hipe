...
public class PackageToPGPackage__Marker_CREATE__TRG__tPackage_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PackageToPGPackage__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PackageToPGPackage__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TPackage, Set<ModiscoTGG.PackageToPGPackage__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PackageToPGPackage__Marker_CREATE__TRG__tPackage_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PackageToPGPackage__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PackageToPGPackage__Marker source, org.gravity.typegraph.basic.TPackage target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PackageToPGPackage__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PackageToPGPackage__Marker>();
			pending.put((org.gravity.typegraph.basic.TPackage) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PackageToPGPackage__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PackageToPGPackage__Marker> message) {
		...
		org.gravity.typegraph.basic.TPackage target = (org.gravity.typegraph.basic.TPackage) message.input.getCREATE__TRG__tPackage();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TPackage> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PackageToPGPackage__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PackageToPGPackage__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PackageToPGPackage__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PackageToPGPackage__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PackageToPGPackage__Marker, org.gravity.typegraph.basic.TPackage> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PackageToPGPackage__Marker, org.gravity.typegraph.basic.TPackage> message) {
		Set<ModiscoTGG.PackageToPGPackage__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PackageToPGPackage__Marker) {
			leftChanged(message);
		}
		else {
