...
public class FieldAccessReadWrite__Marker_CREATE__CORR__mAccessToTAccess_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MSingleVariableAccessToTAccess, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MSingleVariableAccessToTAccess, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MSingleVariableAccessToTAccess, Set<ModiscoTGG.FieldAccessReadWrite__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldAccessReadWrite__Marker_CREATE__CORR__mAccessToTAccess_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldAccessReadWrite__CONSISTENCY_triangle_13_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldAccessReadWrite__Marker source, ModiscoTGG.MSingleVariableAccessToTAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MSingleVariableAccessToTAccess) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldAccessReadWrite__Marker> message) {
		...
		ModiscoTGG.MSingleVariableAccessToTAccess target = (ModiscoTGG.MSingleVariableAccessToTAccess) message.input.getCREATE__CORR__mAccessToTAccess();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldAccessReadWrite__Marker, ModiscoTGG.MSingleVariableAccessToTAccess> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldAccessReadWrite__Marker, ModiscoTGG.MSingleVariableAccessToTAccess> message) {
