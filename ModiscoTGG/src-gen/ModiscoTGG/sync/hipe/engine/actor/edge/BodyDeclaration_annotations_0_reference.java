...
public class BodyDeclaration_annotations_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.BodyDeclaration, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.BodyDeclaration, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.Annotation, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.Annotation, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.Annotation, Set<org.eclipse.modisco.java.BodyDeclaration>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public BodyDeclaration_annotations_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__CONSISTENCY_6102_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__FWD_6104_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray_mValue_annotations_incoming_SRC__FILTER_NAC_production"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__CONSISTENCY_6106_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__FWD_6108_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationMemberValueAnnotation_mValue_annotations_incoming_SRC__FILTER_NAC_production"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__FWD_675_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.BodyDeclaration source, org.eclipse.modisco.java.Annotation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.BodyDeclaration> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.BodyDeclaration>();
			pending.put((org.eclipse.modisco.java.Annotation) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.BodyDeclaration) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.BodyDeclaration> message) {
		...
		for(org.eclipse.modisco.java.Annotation target : message.input.getAnnotations()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Annotation> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.BodyDeclaration> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.BodyDeclaration source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.BodyDeclaration> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.BodyDeclaration> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.Annotation> message) {
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.BodyDeclaration, org.eclipse.modisco.java.Annotation> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.BodyDeclaration, org.eclipse.modisco.java.Annotation> message) {
		Set<org.eclipse.modisco.java.BodyDeclaration> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.BodyDeclaration) {
			leftChanged(message);
		}
		else {
