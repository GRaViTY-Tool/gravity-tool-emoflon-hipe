...
public class MGravityModel_typeParameters_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MGravityModel, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MGravityModel, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.TypeParameter, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.TypeParameter, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.TypeParameter, Set<org.gravity.modisco.MGravityModel>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MGravityModel_typeParameters_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TypeParameter__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TypeParameter__FWD_5918_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MGravityModel source, org.eclipse.modisco.java.TypeParameter target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MGravityModel> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MGravityModel>();
			pending.put((org.eclipse.modisco.java.TypeParameter) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MGravityModel) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MGravityModel> message) {
		...
		for(org.eclipse.modisco.java.TypeParameter target : message.input.getTypeParameters()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.TypeParameter> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MGravityModel> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MGravityModel source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MGravityModel> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MGravityModel> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.TypeParameter> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MGravityModel, org.eclipse.modisco.java.TypeParameter> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MGravityModel, org.eclipse.modisco.java.TypeParameter> message) {
		Set<org.gravity.modisco.MGravityModel> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MGravityModel) {
			leftChanged(message);
		}
		else {
