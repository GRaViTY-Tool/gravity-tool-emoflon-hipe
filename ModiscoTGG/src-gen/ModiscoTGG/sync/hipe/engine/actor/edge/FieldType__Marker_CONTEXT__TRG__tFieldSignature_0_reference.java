...
public class FieldType__Marker_CONTEXT__TRG__tFieldSignature_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.FieldType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.FieldType__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TFieldSignature, Set<ModiscoTGG.FieldType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldType__Marker_CONTEXT__TRG__tFieldSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("FieldType__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldType__Marker source, org.gravity.typegraph.basic.TFieldSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.FieldType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.FieldType__Marker>();
			pending.put((org.gravity.typegraph.basic.TFieldSignature) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.FieldType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldType__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TFieldSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.FieldType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.FieldType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.FieldType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.FieldType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldType__Marker, org.gravity.typegraph.basic.TFieldSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldType__Marker, org.gravity.typegraph.basic.TFieldSignature> message) {
		Set<ModiscoTGG.FieldType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.FieldType__Marker) {
			leftChanged(message);
		}
		else {
