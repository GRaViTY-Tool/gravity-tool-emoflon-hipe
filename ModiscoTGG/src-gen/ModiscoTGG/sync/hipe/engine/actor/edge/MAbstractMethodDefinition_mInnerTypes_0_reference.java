...
public class MAbstractMethodDefinition_mInnerTypes_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MAbstractMethodDefinition, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MAbstractMethodDefinition, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<org.gravity.modisco.MAbstractMethodDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MAbstractMethodDefinition_mInnerTypes_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInnerAnonClassMember__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassInnerAnonClassMember__FWD_1674_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInnerClassMember__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassInnerClassMember__FWD_1784_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MAbstractMethodDefinition source, org.eclipse.modisco.java.AbstractTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MAbstractMethodDefinition> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MAbstractMethodDefinition>();
			pending.put((org.eclipse.modisco.java.AbstractTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MAbstractMethodDefinition) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MAbstractMethodDefinition> message) {
		...
		for(org.eclipse.modisco.java.AbstractTypeDeclaration target : message.input.getMInnerTypes()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MAbstractMethodDefinition> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MAbstractMethodDefinition source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MAbstractMethodDefinition> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MAbstractMethodDefinition> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MAbstractMethodDefinition, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MAbstractMethodDefinition, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		Set<org.gravity.modisco.MAbstractMethodDefinition> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MAbstractMethodDefinition) {
			leftChanged(message);
		}
		else {
