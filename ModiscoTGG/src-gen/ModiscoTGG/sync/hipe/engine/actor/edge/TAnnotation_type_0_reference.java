...
public class TAnnotation_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.annotations.TAnnotation, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.annotations.TAnnotation, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<org.gravity.typegraph.basic.annotations.TAnnotation>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TAnnotation_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationTypeAccess__BWD_686_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationTypeAccess__CONSISTENCY_699_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.annotations.TAnnotation source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.annotations.TAnnotation> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.annotations.TAnnotation>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.annotations.TAnnotation) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.annotations.TAnnotation> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotationType target = (org.gravity.typegraph.basic.annotations.TAnnotationType) message.input.getType();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.annotations.TAnnotation> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.annotations.TAnnotation source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.annotations.TAnnotation> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.annotations.TAnnotation> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.annotations.TAnnotation, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.annotations.TAnnotation, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<org.gravity.typegraph.basic.annotations.TAnnotation> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.annotations.TAnnotation) {
			leftChanged(message);
		}
		else {
