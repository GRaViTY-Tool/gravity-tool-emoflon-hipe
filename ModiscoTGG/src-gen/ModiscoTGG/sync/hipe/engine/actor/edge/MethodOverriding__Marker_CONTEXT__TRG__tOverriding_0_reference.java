...
public class MethodOverriding__Marker_CONTEXT__TRG__tOverriding_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodOverriding__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodOverriding__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMethodDefinition, Set<ModiscoTGG.MethodOverriding__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodOverriding__Marker_CONTEXT__TRG__tOverriding_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodOverriding__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodOverriding__Marker source, org.gravity.typegraph.basic.TMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodOverriding__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodOverriding__Marker>();
			pending.put((org.gravity.typegraph.basic.TMethodDefinition) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodOverriding__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodOverriding__Marker> message) {
		...
		org.gravity.typegraph.basic.TMethodDefinition target = (org.gravity.typegraph.basic.TMethodDefinition) message.input.getCONTEXT__TRG__tOverriding();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodOverriding__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodOverriding__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodOverriding__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodOverriding__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodOverriding__Marker, org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodOverriding__Marker, org.gravity.typegraph.basic.TMethodDefinition> message) {
		Set<ModiscoTGG.MethodOverriding__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodOverriding__Marker) {
			leftChanged(message);
		}
		else {
