...
public class MFlowToTFlow_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MFlowToTFlow, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MFlowToTFlow, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TFlow, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TFlow, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TFlow, Set<ModiscoTGG.MFlowToTFlow>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MFlowToTFlow_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Flow__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("IncomingFlow__BWD_2914_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("IncomingFlow__FWD_2954_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("OutgoingFlow__BWD_4691_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("OutgoingFlow__FWD_4731_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MFlowToTFlow source, org.gravity.typegraph.basic.TFlow target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MFlowToTFlow> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MFlowToTFlow>();
			pending.put((org.gravity.typegraph.basic.TFlow) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MFlowToTFlow) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MFlowToTFlow> message) {
		...
		org.gravity.typegraph.basic.TFlow target = (org.gravity.typegraph.basic.TFlow) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TFlow> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MFlowToTFlow> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MFlowToTFlow source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MFlowToTFlow> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MFlowToTFlow> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TFlow> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MFlowToTFlow, org.gravity.typegraph.basic.TFlow> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MFlowToTFlow, org.gravity.typegraph.basic.TFlow> message) {
		Set<ModiscoTGG.MFlowToTFlow> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MFlowToTFlow) {
			leftChanged(message);
		}
		else {
