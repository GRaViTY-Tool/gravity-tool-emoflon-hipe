...
public class VariableDeclarationFragment__Marker_CONTEXT__SRC__mDefinition_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFieldDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MFieldDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MFieldDefinition, Set<ModiscoTGG.VariableDeclarationFragment__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public VariableDeclarationFragment__Marker_CONTEXT__SRC__mDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("VariableDeclarationFragment__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.VariableDeclarationFragment__Marker source, org.gravity.modisco.MFieldDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MFieldDefinition) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.VariableDeclarationFragment__Marker> message) {
		...
		org.gravity.modisco.MFieldDefinition target = (org.gravity.modisco.MFieldDefinition) message.input.getCONTEXT__SRC__mDefinition();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MFieldDefinition> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MFieldDefinition> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.VariableDeclarationFragment__Marker, org.gravity.modisco.MFieldDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.VariableDeclarationFragment__Marker, org.gravity.modisco.MFieldDefinition> message) {
