...
public class NestedEnum__Marker_CONTEXT__SRC__eOuterType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.NestedEnum__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.NestedEnum__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<ModiscoTGG.NestedEnum__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public NestedEnum__Marker_CONTEXT__SRC__eOuterType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedEnum__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.NestedEnum__Marker source, org.eclipse.modisco.java.AbstractTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.NestedEnum__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.NestedEnum__Marker>();
			pending.put((org.eclipse.modisco.java.AbstractTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.NestedEnum__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.NestedEnum__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.NestedEnum__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.NestedEnum__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.NestedEnum__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.NestedEnum__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.NestedEnum__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.NestedEnum__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		Set<ModiscoTGG.NestedEnum__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.NestedEnum__Marker) {
			leftChanged(message);
		}
		else {
