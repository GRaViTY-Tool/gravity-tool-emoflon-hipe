...
public class VariableDeclarationFragment__Marker_CREATE__CORR__varDeclFragmentToTFieldDefinition_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.VariableDeclarationFragment__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.VariableDeclarationFragment__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, Set<ModiscoTGG.VariableDeclarationFragment__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public VariableDeclarationFragment__Marker_CREATE__CORR__varDeclFragmentToTFieldDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("VariableDeclarationFragment__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.VariableDeclarationFragment__Marker source, ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.VariableDeclarationFragment__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.VariableDeclarationFragment__Marker>();
			pending.put((ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.VariableDeclarationFragment__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.VariableDeclarationFragment__Marker> message) {
		...
		ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition target = (ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition) message.input.getCREATE__CORR__varDeclFragmentToTFieldDefinition();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.VariableDeclarationFragment__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.VariableDeclarationFragment__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.VariableDeclarationFragment__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.VariableDeclarationFragment__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.VariableDeclarationFragment__Marker, ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.VariableDeclarationFragment__Marker, ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
		Set<ModiscoTGG.VariableDeclarationFragment__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.VariableDeclarationFragment__Marker) {
			leftChanged(message);
		}
		else {
