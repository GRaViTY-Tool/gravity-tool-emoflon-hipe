...
public class MethodParameterLinkFirst__Marker_CONTEXT__TRG__tMethodSignature_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodParameterLinkFirst__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodParameterLinkFirst__Marker, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TMethodSignature, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TMethodSignature, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TMethodSignature, Set<ModiscoTGG.MethodParameterLinkFirst__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLinkFirst__Marker_CONTEXT__TRG__tMethodSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLinkFirst__Marker source, org.gravity.typegraph.basic.TMethodSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodParameterLinkFirst__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodParameterLinkFirst__Marker>();
			pending.put((org.gravity.typegraph.basic.TMethodSignature) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodParameterLinkFirst__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		org.gravity.typegraph.basic.TMethodSignature target = (org.gravity.typegraph.basic.TMethodSignature) message.input.getCONTEXT__TRG__tMethodSignature();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodParameterLinkFirst__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodParameterLinkFirst__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodParameterLinkFirst__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TMethodSignature> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.typegraph.basic.TMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.typegraph.basic.TMethodSignature> message) {
		Set<ModiscoTGG.MethodParameterLinkFirst__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodParameterLinkFirst__Marker) {
			leftChanged(message);
		}
		else {
