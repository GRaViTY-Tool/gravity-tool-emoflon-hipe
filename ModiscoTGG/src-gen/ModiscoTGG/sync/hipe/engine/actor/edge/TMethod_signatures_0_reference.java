...
public class TMethod_signatures_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TMethod, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TMethod, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMethodSignature, Set<org.gravity.typegraph.basic.TMethod>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TMethod_signatures_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__BWD_4298_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_4309_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TMethod source, org.gravity.typegraph.basic.TMethodSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TMethod> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TMethod>();
			pending.put((org.gravity.typegraph.basic.TMethodSignature) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TMethod) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TMethod> message) {
		...
		for(org.gravity.typegraph.basic.TMethodSignature target : message.input.getSignatures()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TMethod> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TMethod source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TMethod> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TMethod> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TMethod, org.gravity.typegraph.basic.TMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TMethod, org.gravity.typegraph.basic.TMethodSignature> message) {
		Set<org.gravity.typegraph.basic.TMethod> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TMethod) {
			leftChanged(message);
		}
		else {
