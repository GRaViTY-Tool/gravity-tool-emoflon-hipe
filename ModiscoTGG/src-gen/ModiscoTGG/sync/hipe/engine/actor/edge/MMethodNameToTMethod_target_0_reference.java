...
public class MMethodNameToTMethod_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MMethodNameToTMethod, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MMethodNameToTMethod, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TMethod, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TMethod, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TMethod, Set<ModiscoTGG.MMethodNameToTMethod>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodNameToTMethod_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodName__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodSignature__BWD_4300_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodSignature__FWD_4340_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MMethodNameToTMethod source, org.gravity.typegraph.basic.TMethod target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MMethodNameToTMethod> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MMethodNameToTMethod>();
			pending.put((org.gravity.typegraph.basic.TMethod) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MMethodNameToTMethod) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MMethodNameToTMethod> message) {
		...
		org.gravity.typegraph.basic.TMethod target = (org.gravity.typegraph.basic.TMethod) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethod> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MMethodNameToTMethod> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MMethodNameToTMethod source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MMethodNameToTMethod> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MMethodNameToTMethod> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TMethod> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MMethodNameToTMethod, org.gravity.typegraph.basic.TMethod> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MMethodNameToTMethod, org.gravity.typegraph.basic.TMethod> message) {
		Set<ModiscoTGG.MMethodNameToTMethod> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MMethodNameToTMethod) {
			leftChanged(message);
		}
		else {
