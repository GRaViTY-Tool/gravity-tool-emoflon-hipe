...
public class FieldAccessWrite__Marker_CREATE__TRG__tAccess_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.TWrite, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TWrite, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TWrite, Set<ModiscoTGG.FieldAccessWrite__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldAccessWrite__Marker_CREATE__TRG__tAccess_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("FieldAccessWrite__CONSISTENCY_triangle_12_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldAccessWrite__Marker source, org.gravity.typegraph.basic.TWrite target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.typegraph.basic.TWrite) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldAccessWrite__Marker> message) {
		...
		org.gravity.typegraph.basic.TWrite target = (org.gravity.typegraph.basic.TWrite) message.input.getCREATE__TRG__tAccess();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TWrite> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TWrite> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldAccessWrite__Marker, org.gravity.typegraph.basic.TWrite> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldAccessWrite__Marker, org.gravity.typegraph.basic.TWrite> message) {
