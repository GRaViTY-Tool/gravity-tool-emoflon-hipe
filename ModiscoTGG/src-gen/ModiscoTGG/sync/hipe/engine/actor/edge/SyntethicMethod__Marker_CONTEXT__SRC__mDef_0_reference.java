...
public class SyntethicMethod__Marker_CONTEXT__SRC__mDef_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodDefinition, Set<ModiscoTGG.SyntethicMethod__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SyntethicMethod__Marker_CONTEXT__SRC__mDef_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SyntethicMethod__Marker source, org.gravity.modisco.MMethodDefinition target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SyntethicMethod__Marker> message) {
		...
		org.gravity.modisco.MMethodDefinition target = (org.gravity.modisco.MMethodDefinition) message.input.getCONTEXT__SRC__mDef();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SyntethicMethod__Marker, org.gravity.modisco.MMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SyntethicMethod__Marker, org.gravity.modisco.MMethodDefinition> message) {
