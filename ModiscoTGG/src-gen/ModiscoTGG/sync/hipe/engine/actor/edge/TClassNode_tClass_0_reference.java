...
public class TClassNode_tClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.annotations.TClassNode, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.annotations.TClassNode, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TAbstractType, Set<org.gravity.typegraph.basic.annotations.TClassNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TClassNode_tClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueClassArray__BWD_248_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClassArray__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueClass__BWD_358_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClass__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.annotations.TClassNode source, org.gravity.typegraph.basic.TAbstractType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.annotations.TClassNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.annotations.TClassNode>();
			pending.put((org.gravity.typegraph.basic.TAbstractType) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.annotations.TClassNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.annotations.TClassNode> message) {
		...
		org.gravity.typegraph.basic.TAbstractType target = (org.gravity.typegraph.basic.TAbstractType) message.input.getTClass();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAbstractType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.annotations.TClassNode> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.annotations.TClassNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.annotations.TClassNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.annotations.TClassNode> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.annotations.TClassNode, org.gravity.typegraph.basic.TAbstractType> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.annotations.TClassNode, org.gravity.typegraph.basic.TAbstractType> message) {
		Set<org.gravity.typegraph.basic.annotations.TClassNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.annotations.TClassNode) {
			leftChanged(message);
		}
		else {
