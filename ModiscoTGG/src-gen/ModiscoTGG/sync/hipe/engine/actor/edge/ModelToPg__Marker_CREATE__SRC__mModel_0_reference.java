...
public class ModelToPg__Marker_CREATE__SRC__mModel_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ModelToPg__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ModelToPg__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MGravityModel, Set<ModiscoTGG.ModelToPg__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ModelToPg__Marker_CREATE__SRC__mModel_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ModelToPg__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ModelToPg__Marker source, org.gravity.modisco.MGravityModel target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ModelToPg__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ModelToPg__Marker>();
			pending.put((org.gravity.modisco.MGravityModel) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ModelToPg__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ModelToPg__Marker> message) {
		...
		org.gravity.modisco.MGravityModel target = (org.gravity.modisco.MGravityModel) message.input.getCREATE__SRC__mModel();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MGravityModel> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ModelToPg__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ModelToPg__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ModelToPg__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ModelToPg__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ModelToPg__Marker, org.gravity.modisco.MGravityModel> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ModelToPg__Marker, org.gravity.modisco.MGravityModel> message) {
		Set<ModiscoTGG.ModelToPg__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ModelToPg__Marker) {
			leftChanged(message);
		}
		else {
