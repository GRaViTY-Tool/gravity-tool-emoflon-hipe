...
public class MMethodDefinition_syntheticMethodDefinitions_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MMethodDefinition, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MMethodDefinition, Set<EdgeMatch>>();
	private Map<org.gravity.modisco.MSyntheticMethodDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MSyntheticMethodDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MSyntheticMethodDefinition, Set<org.gravity.modisco.MMethodDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodDefinition_syntheticMethodDefinitions_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod__FWD_5810_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod_mSyntethic_syntheticMethodDefinitions_incoming_SRC__FILTER_NAC_5825_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod_mSyntethic_syntheticMethodDefinitions_incoming_SRC__FILTER_NAC_5825_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MMethodDefinition source, org.gravity.modisco.MSyntheticMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MMethodDefinition> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MMethodDefinition>();
			pending.put((org.gravity.modisco.MSyntheticMethodDefinition) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MMethodDefinition) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MMethodDefinition> message) {
		...
		for(org.gravity.modisco.MSyntheticMethodDefinition target : message.input.getSyntheticMethodDefinitions()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSyntheticMethodDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MMethodDefinition> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MMethodDefinition source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MMethodDefinition> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MMethodDefinition> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MSyntheticMethodDefinition> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MMethodDefinition, org.gravity.modisco.MSyntheticMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MMethodDefinition, org.gravity.modisco.MSyntheticMethodDefinition> message) {
		Set<org.gravity.modisco.MMethodDefinition> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MMethodDefinition) {
			leftChanged(message);
		}
		else {
