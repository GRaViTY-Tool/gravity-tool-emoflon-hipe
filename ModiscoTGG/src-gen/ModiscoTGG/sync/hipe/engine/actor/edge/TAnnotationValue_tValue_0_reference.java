...
public class TAnnotationValue_tValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.annotations.TAnnotationValue, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.annotations.TAnnotationValue, Set<EdgeMatch>>();
	private Map<org.eclipse.emf.ecore.EObject, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.emf.ecore.EObject, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.emf.ecore.EObject, Set<org.gravity.typegraph.basic.annotations.TAnnotationValue>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TAnnotationValue_tValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__BWD_44_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__CONSISTENCY_53_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__BWD_100_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotation__CONSISTENCY_109_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.annotations.TAnnotationValue source, org.eclipse.emf.ecore.EObject target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.annotations.TAnnotationValue> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.annotations.TAnnotationValue>();
			pending.put((org.eclipse.emf.ecore.EObject) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.annotations.TAnnotationValue) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.annotations.TAnnotationValue> message) {
		...
		for(org.eclipse.emf.ecore.EObject target : message.input.getTValue()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.emf.ecore.EObject> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.annotations.TAnnotationValue> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.annotations.TAnnotationValue source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.annotations.TAnnotationValue> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.annotations.TAnnotationValue> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.emf.ecore.EObject> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.annotations.TAnnotationValue, org.eclipse.emf.ecore.EObject> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.annotations.TAnnotationValue, org.eclipse.emf.ecore.EObject> message) {
		Set<org.gravity.typegraph.basic.annotations.TAnnotationValue> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.annotations.TAnnotationValue) {
			leftChanged(message);
		}
		else {
