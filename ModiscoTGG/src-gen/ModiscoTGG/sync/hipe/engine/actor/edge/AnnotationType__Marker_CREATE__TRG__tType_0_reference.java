...
public class AnnotationType__Marker_CREATE__TRG__tType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationType__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<ModiscoTGG.AnnotationType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationType__Marker_CREATE__TRG__tType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationType__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationType__Marker source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationType__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationType__Marker> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotationType target = (org.gravity.typegraph.basic.annotations.TAnnotationType) message.input.getCREATE__TRG__tType();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationType__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationType__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<ModiscoTGG.AnnotationType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationType__Marker) {
			leftChanged(message);
		}
		else {
