...
public class TMember_tModifier_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TMember, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TMember, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TModifier, Set<org.gravity.typegraph.basic.TMember>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TMember_tModifier_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("InterfaceMember__BWD_3121_junction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("InterfaceMember__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInAnnotationVisibility__BWD_3466_junction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInAnnotationVisibility__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInClassVisibility__BWD_3547_junction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInClassVisibility__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInEnumVisibility__BWD_3628_junction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInEnumVisibility__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("TypesVisibility__BWD_6427_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("TypesVisibility__CONSISTENCY_6428_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("TypesVisibility_tModifier_tModifier_incoming_TRG__FILTER_NAC_production"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TMember source, org.gravity.typegraph.basic.TModifier target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TMember> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TMember>();
			pending.put((org.gravity.typegraph.basic.TModifier) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TMember) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TMember> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TModifier> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TMember> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TMember source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TMember> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TMember> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TMember, org.gravity.typegraph.basic.TModifier> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TMember, org.gravity.typegraph.basic.TModifier> message) {
		Set<org.gravity.typegraph.basic.TMember> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TMember) {
			leftChanged(message);
		}
		else {
