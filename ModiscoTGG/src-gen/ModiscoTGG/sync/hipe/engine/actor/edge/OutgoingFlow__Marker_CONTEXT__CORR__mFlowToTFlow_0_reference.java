...
public class OutgoingFlow__Marker_CONTEXT__CORR__mFlowToTFlow_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MFlowToTFlow, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MFlowToTFlow, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MFlowToTFlow, Set<ModiscoTGG.OutgoingFlow__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public OutgoingFlow__Marker_CONTEXT__CORR__mFlowToTFlow_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.OutgoingFlow__Marker source, ModiscoTGG.MFlowToTFlow target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MFlowToTFlow) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.OutgoingFlow__Marker> message) {
		...
		ModiscoTGG.MFlowToTFlow target = (ModiscoTGG.MFlowToTFlow) message.input.getCONTEXT__CORR__mFlowToTFlow();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MFlowToTFlow> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MFlowToTFlow> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.OutgoingFlow__Marker, ModiscoTGG.MFlowToTFlow> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.OutgoingFlow__Marker, ModiscoTGG.MFlowToTFlow> message) {
