...
public class EnumInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.EnumInAnonymous__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.EnumInAnonymous__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AnonymousClassDeclaration, Set<ModiscoTGG.EnumInAnonymous__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public EnumInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("EnumInAnonymous__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.EnumInAnonymous__Marker source, org.eclipse.modisco.java.AnonymousClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.EnumInAnonymous__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.EnumInAnonymous__Marker>();
			pending.put((org.eclipse.modisco.java.AnonymousClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.EnumInAnonymous__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.EnumInAnonymous__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.EnumInAnonymous__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.EnumInAnonymous__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.EnumInAnonymous__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.EnumInAnonymous__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.EnumInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.EnumInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		Set<ModiscoTGG.EnumInAnonymous__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.EnumInAnonymous__Marker) {
			leftChanged(message);
		}
		else {
