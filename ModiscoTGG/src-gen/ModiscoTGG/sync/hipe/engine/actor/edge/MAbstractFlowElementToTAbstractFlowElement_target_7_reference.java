...
public class MAbstractFlowElementToTAbstractFlowElement_target_7_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TAbstractFlowElement, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TAbstractFlowElement, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TAbstractFlowElement, Set<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MAbstractFlowElementToTAbstractFlowElement_target_7_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("Flow__BWD_2867_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Flow__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("Flow__FWD_2901_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("IncomingFlow__BWD_2909_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("IncomingFlow__FWD_2949_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("OutgoingFlow__BWD_4686_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("OutgoingFlow__FWD_4726_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement source, org.gravity.typegraph.basic.TAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement>();
			pending.put((org.gravity.typegraph.basic.TAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		org.gravity.typegraph.basic.TAbstractFlowElement target = (org.gravity.typegraph.basic.TAbstractFlowElement) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, org.gravity.typegraph.basic.TAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		Set<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) {
			leftChanged(message);
		}
		else {
