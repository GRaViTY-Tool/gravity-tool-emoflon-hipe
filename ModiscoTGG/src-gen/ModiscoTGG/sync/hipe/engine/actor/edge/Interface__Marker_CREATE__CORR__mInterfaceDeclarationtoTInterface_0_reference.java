...
public class Interface__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.Interface__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.Interface__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.InterfaceDeclarationToTInterface, Set<ModiscoTGG.Interface__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Interface__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Interface__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.Interface__Marker source, ModiscoTGG.InterfaceDeclarationToTInterface target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.Interface__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.Interface__Marker>();
			pending.put((ModiscoTGG.InterfaceDeclarationToTInterface) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.Interface__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.Interface__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.Interface__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.Interface__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.Interface__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.Interface__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.Interface__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.Interface__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		Set<ModiscoTGG.Interface__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.Interface__Marker) {
			leftChanged(message);
		}
		else {
