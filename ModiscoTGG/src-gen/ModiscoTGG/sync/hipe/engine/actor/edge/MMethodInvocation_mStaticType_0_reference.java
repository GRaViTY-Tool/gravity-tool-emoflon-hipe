...
public class MMethodInvocation_mStaticType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MMethodInvocation, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MMethodInvocation, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Type, Set<org.gravity.modisco.MMethodInvocation>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodInvocation_mStaticType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocationStaticType__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocationStaticType__FWD_3861_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MMethodInvocation source, org.eclipse.modisco.java.Type target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MMethodInvocation> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MMethodInvocation>();
			pending.put((org.eclipse.modisco.java.Type) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MMethodInvocation) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MMethodInvocation> message) {
		...
		org.eclipse.modisco.java.Type target = (org.eclipse.modisco.java.Type) message.input.getMStaticType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Type> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MMethodInvocation> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MMethodInvocation source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MMethodInvocation> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MMethodInvocation> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MMethodInvocation, org.eclipse.modisco.java.Type> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MMethodInvocation, org.eclipse.modisco.java.Type> message) {
		Set<org.gravity.modisco.MMethodInvocation> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MMethodInvocation) {
			leftChanged(message);
		}
		else {
