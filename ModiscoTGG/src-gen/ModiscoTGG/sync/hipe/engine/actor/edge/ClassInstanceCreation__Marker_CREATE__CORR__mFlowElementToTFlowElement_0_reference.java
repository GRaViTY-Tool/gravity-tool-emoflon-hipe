...
public class ClassInstanceCreation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassInstanceCreation__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassInstanceCreation__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.ClassInstanceCreation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassInstanceCreation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInstanceCreation__CONSISTENCY_1826_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassInstanceCreation__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassInstanceCreation__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassInstanceCreation__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassInstanceCreation__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassInstanceCreation__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassInstanceCreation__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassInstanceCreation__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassInstanceCreation__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassInstanceCreation__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassInstanceCreation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassInstanceCreation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.ClassInstanceCreation__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassInstanceCreation__Marker) {
			leftChanged(message);
		}
		else {
