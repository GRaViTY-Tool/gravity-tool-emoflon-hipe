...
public class LinkDefinitionToType__Marker_CONTEXT__SRC__mSignature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MSignature, Set<ModiscoTGG.LinkDefinitionToType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public LinkDefinitionToType__Marker_CONTEXT__SRC__mSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("LinkDefinitionToType__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.LinkDefinitionToType__Marker source, org.gravity.modisco.MSignature target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.LinkDefinitionToType__Marker> message) {
		...
		org.gravity.modisco.MSignature target = (org.gravity.modisco.MSignature) message.input.getCONTEXT__SRC__mSignature();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.LinkDefinitionToType__Marker, org.gravity.modisco.MSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.LinkDefinitionToType__Marker, org.gravity.modisco.MSignature> message) {
