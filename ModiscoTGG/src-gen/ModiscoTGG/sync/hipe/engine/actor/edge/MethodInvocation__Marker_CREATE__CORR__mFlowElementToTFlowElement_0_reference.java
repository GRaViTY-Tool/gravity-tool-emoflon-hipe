...
public class MethodInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodInvocation__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodInvocation__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.MethodInvocation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocation__CONSISTENCY_3892_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodInvocation__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodInvocation__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodInvocation__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodInvocation__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodInvocation__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodInvocation__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodInvocation__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodInvocation__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodInvocation__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.MethodInvocation__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodInvocation__Marker) {
			leftChanged(message);
		}
		else {
