...
public class PrimitiveTypeToTClass_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeToTClass, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeToTClass, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveType, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveType, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveType, Set<ModiscoTGG.PrimitiveTypeToTClass>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeToTClass_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("PrimitiveTypeIsBoolean__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeToTClass source, org.eclipse.modisco.java.PrimitiveType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeToTClass> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeToTClass>();
			pending.put((org.eclipse.modisco.java.PrimitiveType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeToTClass) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeToTClass> message) {
		...
		org.eclipse.modisco.java.PrimitiveType target = (org.eclipse.modisco.java.PrimitiveType) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeToTClass> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeToTClass source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeToTClass> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeToTClass> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveType> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeToTClass, org.eclipse.modisco.java.PrimitiveType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeToTClass, org.eclipse.modisco.java.PrimitiveType> message) {
		Set<ModiscoTGG.PrimitiveTypeToTClass> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeToTClass) {
			leftChanged(message);
		}
		else {
