...
public class InterfaceMember__Marker_CONTEXT__SRC__mInterface_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.InterfaceMember__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.InterfaceMember__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.InterfaceDeclaration, Set<ModiscoTGG.InterfaceMember__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public InterfaceMember__Marker_CONTEXT__SRC__mInterface_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("InterfaceMember__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.InterfaceMember__Marker source, org.eclipse.modisco.java.InterfaceDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.InterfaceMember__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.InterfaceMember__Marker>();
			pending.put((org.eclipse.modisco.java.InterfaceDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.InterfaceMember__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.InterfaceMember__Marker> message) {
		...
		org.eclipse.modisco.java.InterfaceDeclaration target = (org.eclipse.modisco.java.InterfaceDeclaration) message.input.getCONTEXT__SRC__mInterface();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.InterfaceDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.InterfaceMember__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.InterfaceMember__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.InterfaceMember__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.InterfaceMember__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.InterfaceMember__Marker, org.eclipse.modisco.java.InterfaceDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.InterfaceMember__Marker, org.eclipse.modisco.java.InterfaceDeclaration> message) {
		Set<ModiscoTGG.InterfaceMember__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.InterfaceMember__Marker) {
			leftChanged(message);
		}
		else {
