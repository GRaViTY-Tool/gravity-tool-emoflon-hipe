...
public class MethodSignature__Marker_CREATE__SRC__mSignature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodSignature, Set<ModiscoTGG.MethodSignature__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodSignature__Marker_CREATE__SRC__mSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodSignature__Marker source, org.gravity.modisco.MMethodSignature target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodSignature__Marker> message) {
		...
		org.gravity.modisco.MMethodSignature target = (org.gravity.modisco.MMethodSignature) message.input.getCREATE__SRC__mSignature();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodSignature__Marker, org.gravity.modisco.MMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodSignature__Marker, org.gravity.modisco.MMethodSignature> message) {
