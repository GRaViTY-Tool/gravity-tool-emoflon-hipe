...
public class TypeLiteral_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.TypeLiteral, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.TypeLiteral, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.TypeAccess, Set<org.eclipse.modisco.java.TypeLiteral>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TypeLiteral_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClassArray__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueClassArray__FWD_313_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClass__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueClass__FWD_419_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.TypeLiteral source, org.eclipse.modisco.java.TypeAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.TypeLiteral> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.TypeLiteral>();
			pending.put((org.eclipse.modisco.java.TypeAccess) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.TypeLiteral) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.TypeLiteral> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.TypeAccess> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.TypeLiteral> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.TypeLiteral source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.TypeLiteral> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.TypeLiteral> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.TypeLiteral, org.eclipse.modisco.java.TypeAccess> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.TypeLiteral, org.eclipse.modisco.java.TypeAccess> message) {
		Set<org.eclipse.modisco.java.TypeLiteral> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.TypeLiteral) {
			leftChanged(message);
		}
		else {
