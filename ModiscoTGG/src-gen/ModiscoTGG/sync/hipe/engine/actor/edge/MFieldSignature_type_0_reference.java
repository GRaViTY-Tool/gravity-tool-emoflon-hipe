...
public class MFieldSignature_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MFieldSignature, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MFieldSignature, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Type, Set<org.gravity.modisco.MFieldSignature>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MFieldSignature_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldType__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldType__FWD_2852_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MFieldSignature source, org.eclipse.modisco.java.Type target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MFieldSignature> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MFieldSignature>();
			pending.put((org.eclipse.modisco.java.Type) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MFieldSignature) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MFieldSignature> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Type> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MFieldSignature> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MFieldSignature source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MFieldSignature> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MFieldSignature> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MFieldSignature, org.eclipse.modisco.java.Type> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MFieldSignature, org.eclipse.modisco.java.Type> message) {
		Set<org.gravity.modisco.MFieldSignature> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MFieldSignature) {
			leftChanged(message);
		}
		else {
