...
public class MMethodNameToTMethod_source_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodName, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MMethodName, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MMethodName, Set<ModiscoTGG.MMethodNameToTMethod>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodNameToTMethod_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodName__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__BWD_4300_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__FWD_4340_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MMethodNameToTMethod source, org.gravity.modisco.MMethodName target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MMethodName) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MMethodNameToTMethod> message) {
		...
		org.gravity.modisco.MMethodName target = (org.gravity.modisco.MMethodName) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MMethodName> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MMethodName> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MMethodNameToTMethod, org.gravity.modisco.MMethodName> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MMethodNameToTMethod, org.gravity.modisco.MMethodName> message) {
