...
public class TField_signatures_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TField, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TField, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TFieldSignature, Set<org.gravity.typegraph.basic.TField>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TField_signatures_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__BWD_2761_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__CONSISTENCY_2772_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TField source, org.gravity.typegraph.basic.TFieldSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TField> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TField>();
			pending.put((org.gravity.typegraph.basic.TFieldSignature) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TField) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TField> message) {
		...
		for(org.gravity.typegraph.basic.TFieldSignature target : message.input.getSignatures()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TFieldSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TField> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TField source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TField> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TField> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TField, org.gravity.typegraph.basic.TFieldSignature> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TField, org.gravity.typegraph.basic.TFieldSignature> message) {
		Set<org.gravity.typegraph.basic.TField> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TField) {
			leftChanged(message);
		}
		else {
