...
public class MMethodSignature_mEntrys_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MParameterList, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MParameterList, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MEntry, Set<org.gravity.modisco.MParameterList>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodSignature_mEntrys_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameterType__FWD_4231_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameter__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameter__FWD_4283_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MParameterList source, org.gravity.modisco.MEntry target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MParameterList> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MParameterList>();
			pending.put((org.gravity.modisco.MEntry) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MParameterList) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MParameterList> message) {
		...
		for(org.gravity.modisco.MEntry target : message.input.getMEntrys()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MEntry> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MParameterList> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MParameterList source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MParameterList> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MParameterList> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MParameterList, org.gravity.modisco.MEntry> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MParameterList, org.gravity.modisco.MEntry> message) {
		Set<org.gravity.modisco.MParameterList> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MParameterList) {
			leftChanged(message);
		}
		else {
