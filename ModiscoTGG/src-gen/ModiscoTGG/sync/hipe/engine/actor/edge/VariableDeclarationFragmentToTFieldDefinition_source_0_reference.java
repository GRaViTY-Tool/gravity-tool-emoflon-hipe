...
public class VariableDeclarationFragmentToTFieldDefinition_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.VariableDeclarationFragment, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.VariableDeclarationFragment, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.VariableDeclarationFragment, Set<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public VariableDeclarationFragmentToTFieldDefinition_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("VariableDeclarationFragment__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition source, org.eclipse.modisco.java.VariableDeclarationFragment target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition>();
			pending.put((org.eclipse.modisco.java.VariableDeclarationFragment) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
		...
		org.eclipse.modisco.java.VariableDeclarationFragment target = (org.eclipse.modisco.java.VariableDeclarationFragment) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.VariableDeclarationFragment> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.VariableDeclarationFragment> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, org.eclipse.modisco.java.VariableDeclarationFragment> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition, org.eclipse.modisco.java.VariableDeclarationFragment> message) {
		Set<ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.VariableDeclarationFragmentToTFieldDefinition) {
			leftChanged(message);
		}
		else {
