...
public class MModifierToTModifier_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MModifierToTModifier, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MModifierToTModifier, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TModifier, Set<ModiscoTGG.MModifierToTModifier>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MModifierToTModifier_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("InterfaceMember__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInAnnotationVisibility__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInClassVisibility__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MemberInEnumVisibility__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TypesVisibility__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MModifierToTModifier source, org.gravity.typegraph.basic.TModifier target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MModifierToTModifier> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MModifierToTModifier>();
			pending.put((org.gravity.typegraph.basic.TModifier) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MModifierToTModifier) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MModifierToTModifier> message) {
		...
		org.gravity.typegraph.basic.TModifier target = (org.gravity.typegraph.basic.TModifier) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TModifier> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MModifierToTModifier> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MModifierToTModifier source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MModifierToTModifier> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MModifierToTModifier> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MModifierToTModifier, org.gravity.typegraph.basic.TModifier> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MModifierToTModifier, org.gravity.typegraph.basic.TModifier> message) {
		Set<ModiscoTGG.MModifierToTModifier> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MModifierToTModifier) {
			leftChanged(message);
		}
		else {
