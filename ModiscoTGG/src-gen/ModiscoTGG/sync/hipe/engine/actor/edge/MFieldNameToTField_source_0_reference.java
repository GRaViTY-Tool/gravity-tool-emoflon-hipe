...
public class MFieldNameToTField_source_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFieldName, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MFieldName, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MFieldName, Set<ModiscoTGG.MFieldNameToTField>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MFieldNameToTField_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldName__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__BWD_2763_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__FWD_2803_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MFieldNameToTField source, org.gravity.modisco.MFieldName target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MFieldName) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MFieldNameToTField> message) {
		...
		org.gravity.modisco.MFieldName target = (org.gravity.modisco.MFieldName) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MFieldName> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MFieldName> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MFieldNameToTField, org.gravity.modisco.MFieldName> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MFieldNameToTField, org.gravity.modisco.MFieldName> message) {
