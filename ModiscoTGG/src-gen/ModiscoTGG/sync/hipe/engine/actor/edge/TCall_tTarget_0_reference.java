...
public class TCall_tTarget_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TAccess, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TAccess, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMember, Set<org.gravity.typegraph.basic.TAccess>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TCall_tTarget_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassInstanceCreation__BWD_1814_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInstanceCreation__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ConstructorInvocation__BWD_2034_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodInvocation__BWD_3880_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocation__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SuperConstructorInvocation__BWD_5582_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SuperMethodInvocation__BWD_5663_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperMethodInvocation__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TAccess source, org.gravity.typegraph.basic.TMember target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TAccess> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TAccess>();
			pending.put((org.gravity.typegraph.basic.TMember) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TAccess) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TAccess> message) {
		...
		org.gravity.typegraph.basic.TMember target = (org.gravity.typegraph.basic.TMember) message.input.getTTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMember> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TAccess> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TAccess source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TAccess> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TAccess> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TAccess, org.gravity.typegraph.basic.TMember> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TAccess, org.gravity.typegraph.basic.TMember> message) {
		Set<org.gravity.typegraph.basic.TAccess> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TAccess) {
			leftChanged(message);
		}
		else {
