...
public class MSingleVariableAccess_variable_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.SingleVariableAccess, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.SingleVariableAccess, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.VariableDeclaration, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.VariableDeclaration, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.VariableDeclaration, Set<org.eclipse.modisco.java.SingleVariableAccess>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MSingleVariableAccess_variable_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessReadWrite__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessReadWrite__FWD_2366_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.SingleVariableAccess source, org.eclipse.modisco.java.VariableDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.SingleVariableAccess> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.SingleVariableAccess>();
			pending.put((org.eclipse.modisco.java.VariableDeclaration) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.SingleVariableAccess) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.SingleVariableAccess> message) {
		...
		org.eclipse.modisco.java.VariableDeclaration target = (org.eclipse.modisco.java.VariableDeclaration) message.input.getVariable();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.VariableDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.SingleVariableAccess> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.SingleVariableAccess source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.SingleVariableAccess> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.SingleVariableAccess> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.VariableDeclaration> message) {
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.SingleVariableAccess, org.eclipse.modisco.java.VariableDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.SingleVariableAccess, org.eclipse.modisco.java.VariableDeclaration> message) {
		Set<org.eclipse.modisco.java.SingleVariableAccess> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.SingleVariableAccess) {
			leftChanged(message);
		}
		else {
