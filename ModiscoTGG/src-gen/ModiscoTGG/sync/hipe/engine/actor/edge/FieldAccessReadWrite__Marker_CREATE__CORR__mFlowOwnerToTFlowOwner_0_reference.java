...
public class FieldAccessReadWrite__Marker_CREATE__CORR__mFlowOwnerToTFlowOwner_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.FieldAccessReadWrite__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.FieldAccessReadWrite__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.FieldAccessReadWrite__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldAccessReadWrite__Marker_CREATE__CORR__mFlowOwnerToTFlowOwner_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessReadWrite__CONSISTENCY_2300_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldAccessReadWrite__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.FieldAccessReadWrite__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.FieldAccessReadWrite__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.FieldAccessReadWrite__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldAccessReadWrite__Marker> message) {
		...
		ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target = (ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) message.input.getCREATE__CORR__mFlowOwnerToTFlowOwner();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.FieldAccessReadWrite__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.FieldAccessReadWrite__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.FieldAccessReadWrite__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.FieldAccessReadWrite__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldAccessReadWrite__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldAccessReadWrite__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.FieldAccessReadWrite__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.FieldAccessReadWrite__Marker) {
			leftChanged(message);
		}
		else {
