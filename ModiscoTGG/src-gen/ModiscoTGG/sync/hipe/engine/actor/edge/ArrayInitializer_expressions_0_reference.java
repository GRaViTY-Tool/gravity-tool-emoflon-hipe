...
public class ArrayInitializer_expressions_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.ArrayInitializer, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.ArrayInitializer, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Expression, Set<org.eclipse.modisco.java.ArrayInitializer>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ArrayInitializer_expressions_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueAnnotationArray__FWD_84_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.ArrayInitializer source, org.eclipse.modisco.java.Expression target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.ArrayInitializer> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.ArrayInitializer>();
			pending.put((org.eclipse.modisco.java.Expression) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.ArrayInitializer) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.ArrayInitializer> message) {
		...
		for(org.eclipse.modisco.java.Expression target : message.input.getExpressions()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Expression> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.ArrayInitializer> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.ArrayInitializer source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.ArrayInitializer> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.ArrayInitializer> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.ArrayInitializer, org.eclipse.modisco.java.Expression> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.ArrayInitializer, org.eclipse.modisco.java.Expression> message) {
		Set<org.eclipse.modisco.java.ArrayInitializer> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.ArrayInitializer) {
			leftChanged(message);
		}
		else {
