...
public class ASTNodeToTAnnotatable_target_2_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ASTNodeToTAnnotatable, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ASTNodeToTAnnotatable, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.annotations.TAnnotatable, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.annotations.TAnnotatable, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.annotations.TAnnotatable, Set<ModiscoTGG.ASTNodeToTAnnotatable>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ASTNodeToTAnnotatable_target_2_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassInAnonymous__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Class__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("EnumInAnonymous__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Enum__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedClassInType__CONSISTENCY_triangle_11_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedEnum__CONSISTENCY_triangle_10_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TypesOrphaned__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ASTNodeToTAnnotatable source, org.gravity.typegraph.basic.annotations.TAnnotatable target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ASTNodeToTAnnotatable> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ASTNodeToTAnnotatable>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotatable) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ASTNodeToTAnnotatable) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotatable target = (org.gravity.typegraph.basic.annotations.TAnnotatable) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ASTNodeToTAnnotatable> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ASTNodeToTAnnotatable source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ASTNodeToTAnnotatable> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ASTNodeToTAnnotatable, org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ASTNodeToTAnnotatable, org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		Set<ModiscoTGG.ASTNodeToTAnnotatable> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ASTNodeToTAnnotatable) {
			leftChanged(message);
		}
		else {
