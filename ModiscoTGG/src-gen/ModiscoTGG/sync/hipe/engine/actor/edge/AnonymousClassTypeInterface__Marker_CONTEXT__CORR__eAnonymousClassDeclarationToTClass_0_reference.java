...
public class AnonymousClassTypeInterface__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnonymousClassTypeInterface__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnonymousClassTypeInterface__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.AnonymousClassDeclarationToTClass, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.AnonymousClassDeclarationToTClass, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.AnonymousClassDeclarationToTClass, Set<ModiscoTGG.AnonymousClassTypeInterface__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnonymousClassTypeInterface__Marker_CONTEXT__CORR__eAnonymousClassDeclarationToTClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnonymousClassTypeInterface__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnonymousClassTypeInterface__Marker source, ModiscoTGG.AnonymousClassDeclarationToTClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnonymousClassTypeInterface__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnonymousClassTypeInterface__Marker>();
			pending.put((ModiscoTGG.AnonymousClassDeclarationToTClass) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnonymousClassTypeInterface__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnonymousClassTypeInterface__Marker> message) {
		...
		ModiscoTGG.AnonymousClassDeclarationToTClass target = (ModiscoTGG.AnonymousClassDeclarationToTClass) message.input.getCONTEXT__CORR__eAnonymousClassDeclarationToTClass();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnonymousClassTypeInterface__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnonymousClassTypeInterface__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnonymousClassTypeInterface__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnonymousClassTypeInterface__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnonymousClassTypeInterface__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnonymousClassTypeInterface__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		Set<ModiscoTGG.AnonymousClassTypeInterface__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnonymousClassTypeInterface__Marker) {
			leftChanged(message);
		}
		else {
