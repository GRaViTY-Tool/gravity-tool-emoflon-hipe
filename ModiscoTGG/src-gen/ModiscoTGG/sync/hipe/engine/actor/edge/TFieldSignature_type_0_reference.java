...
public class TFieldSignature_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TFieldSignature, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TFieldSignature, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TAbstractType, Set<org.gravity.typegraph.basic.TFieldSignature>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TFieldSignature_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldType__BWD_2812_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldType__CONSISTENCY_2825_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TFieldSignature source, org.gravity.typegraph.basic.TAbstractType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TFieldSignature> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TFieldSignature>();
			pending.put((org.gravity.typegraph.basic.TAbstractType) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TFieldSignature) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TFieldSignature> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAbstractType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TFieldSignature> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TFieldSignature source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TFieldSignature> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TFieldSignature> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TFieldSignature, org.gravity.typegraph.basic.TAbstractType> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TFieldSignature, org.gravity.typegraph.basic.TAbstractType> message) {
		Set<org.gravity.typegraph.basic.TFieldSignature> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TFieldSignature) {
			leftChanged(message);
		}
		else {
