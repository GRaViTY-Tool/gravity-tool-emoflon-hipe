...
public class MethodParameterLinkFirst__Marker_CONTEXT__SRC__mParameterList_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodSignature, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MMethodSignature, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MMethodSignature, Set<ModiscoTGG.MethodParameterLinkFirst__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLinkFirst__Marker_CONTEXT__SRC__mParameterList_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLinkFirst__Marker source, org.gravity.modisco.MMethodSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MMethodSignature) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		org.gravity.modisco.MMethodSignature target = (org.gravity.modisco.MMethodSignature) message.input.getCONTEXT__SRC__mParameterList();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MMethodSignature> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MMethodSignature> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.modisco.MMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.modisco.MMethodSignature> message) {
