...
public class Class__Marker_CREATE__CORR__mClassDeclarationToTClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.Class__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.Class__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.ClassDeclarationToTClass, Set<ModiscoTGG.Class__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Class__Marker_CREATE__CORR__mClassDeclarationToTClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Class__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.Class__Marker source, ModiscoTGG.ClassDeclarationToTClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.Class__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.Class__Marker>();
			pending.put((ModiscoTGG.ClassDeclarationToTClass) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.Class__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.Class__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.ClassDeclarationToTClass> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.Class__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.Class__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.Class__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.Class__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.Class__Marker, ModiscoTGG.ClassDeclarationToTClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.Class__Marker, ModiscoTGG.ClassDeclarationToTClass> message) {
		Set<ModiscoTGG.Class__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.Class__Marker) {
			leftChanged(message);
		}
		else {
