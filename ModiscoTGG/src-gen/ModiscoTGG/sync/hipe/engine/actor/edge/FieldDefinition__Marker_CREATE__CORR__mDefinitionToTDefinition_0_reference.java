...
public class FieldDefinition__Marker_CREATE__CORR__mDefinitionToTDefinition_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.FieldDefinition__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.FieldDefinition__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.FieldDefinition__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldDefinition__Marker_CREATE__CORR__mDefinitionToTDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldDefinition__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldDefinition__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.FieldDefinition__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.FieldDefinition__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.FieldDefinition__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldDefinition__Marker> message) {
		...
		ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target = (ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) message.input.getCREATE__CORR__mDefinitionToTDefinition();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.FieldDefinition__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.FieldDefinition__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.FieldDefinition__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.FieldDefinition__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldDefinition__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldDefinition__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.FieldDefinition__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.FieldDefinition__Marker) {
			leftChanged(message);
		}
		else {
