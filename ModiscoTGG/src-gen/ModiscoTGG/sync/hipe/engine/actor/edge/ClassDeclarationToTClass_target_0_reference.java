...
public class ClassDeclarationToTClass_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassDeclarationToTClass, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassDeclarationToTClass, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TClass, Set<ModiscoTGG.ClassDeclarationToTClass>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassDeclarationToTClass_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassExtendsClass__BWD_1352_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassExtendsClass__BWD_1357_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassExtendsClass__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassExtendsClass__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassExtendsClass__FWD_1396_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassExtendsClass__FWD_1400_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassImplementsInterface__BWD_1437_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassImplementsInterface__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ClassImplementsInterface__FWD_1480_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInAnonymous__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInnerAnonClassMember__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInnerClassMember__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("Class__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedClassInType__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod__BWD_5742_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod__FWD_5809_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassDeclarationToTClass source, org.gravity.typegraph.basic.TClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassDeclarationToTClass> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassDeclarationToTClass>();
			pending.put((org.gravity.typegraph.basic.TClass) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassDeclarationToTClass) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassDeclarationToTClass> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TClass> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassDeclarationToTClass> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassDeclarationToTClass source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassDeclarationToTClass> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassDeclarationToTClass> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassDeclarationToTClass, org.gravity.typegraph.basic.TClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassDeclarationToTClass, org.gravity.typegraph.basic.TClass> message) {
		Set<ModiscoTGG.ClassDeclarationToTClass> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassDeclarationToTClass) {
			leftChanged(message);
		}
		else {
