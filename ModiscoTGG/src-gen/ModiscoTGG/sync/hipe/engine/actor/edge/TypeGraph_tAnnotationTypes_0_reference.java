...
public class TypeGraph_tAnnotationTypes_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<org.gravity.typegraph.basic.TypeGraph>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TypeGraph_tAnnotationTypes_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationTypeNested__BWD_triangle_2_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationTypeNested__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationType__BWD_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationType__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TypeGraph source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TypeGraph> message) {
		...
		for(org.gravity.typegraph.basic.annotations.TAnnotationType target : message.input.getTAnnotationTypes()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TypeGraph, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TypeGraph, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
