...
public class AbstractMethodInvocationToTAccess_target_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.TAccess, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TAccess, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TAccess, Set<ModiscoTGG.AbstractMethodInvocationToTAccess>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AbstractMethodInvocationToTAccess_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ClassInstanceCreation__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocationRecursive__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodInvocationStaticType__BWD_3825_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocationStaticType__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodInvocationStaticType__FWD_3865_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodInvocation__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperMethodInvocation__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AbstractMethodInvocationToTAccess source, org.gravity.typegraph.basic.TAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.typegraph.basic.TAccess) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AbstractMethodInvocationToTAccess> message) {
		...
		org.gravity.typegraph.basic.TAccess target = (org.gravity.typegraph.basic.TAccess) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAccess> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TAccess> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AbstractMethodInvocationToTAccess, org.gravity.typegraph.basic.TAccess> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AbstractMethodInvocationToTAccess, org.gravity.typegraph.basic.TAccess> message) {
