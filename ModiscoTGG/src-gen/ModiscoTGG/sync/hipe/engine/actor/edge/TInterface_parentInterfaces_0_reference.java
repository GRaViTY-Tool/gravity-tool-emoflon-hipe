...
public class TInterface_parentInterfaces_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TInterface, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TInterface, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TInterface, Set<org.gravity.typegraph.basic.TInterface>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TInterface_parentInterfaces_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("InterfaceExtendsInterface__BWD_2965_junction"), this::check_constraint_147));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("InterfaceExtendsInterface__CONSISTENCY_triangle_0_triangleJunction"), this::check_constraint_149));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TInterface source, org.gravity.typegraph.basic.TInterface target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TInterface> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TInterface>();
			pending.put((org.gravity.typegraph.basic.TInterface) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TInterface) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TInterface> message) {
		...
		for(org.gravity.typegraph.basic.TInterface target : message.input.getParentInterfaces()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TInterface> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TInterface> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TInterface source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TInterface> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TInterface> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TInterface, org.gravity.typegraph.basic.TInterface> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TInterface, org.gravity.typegraph.basic.TInterface> message) {
		Set<org.gravity.typegraph.basic.TInterface> pendingSources = pending.get(message.target);
