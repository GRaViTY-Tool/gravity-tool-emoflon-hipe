...
public class ClassLiteralToClassNode_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassLiteralToClassNode, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassLiteralToClassNode, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.TypeLiteral, Set<ModiscoTGG.ClassLiteralToClassNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassLiteralToClassNode_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClassArray__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueClass__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassLiteralToClassNode source, org.eclipse.modisco.java.TypeLiteral target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassLiteralToClassNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassLiteralToClassNode>();
			pending.put((org.eclipse.modisco.java.TypeLiteral) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassLiteralToClassNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassLiteralToClassNode> message) {
		...
		org.eclipse.modisco.java.TypeLiteral target = (org.eclipse.modisco.java.TypeLiteral) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.TypeLiteral> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassLiteralToClassNode> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassLiteralToClassNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassLiteralToClassNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassLiteralToClassNode> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassLiteralToClassNode, org.eclipse.modisco.java.TypeLiteral> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassLiteralToClassNode, org.eclipse.modisco.java.TypeLiteral> message) {
		Set<ModiscoTGG.ClassLiteralToClassNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassLiteralToClassNode) {
			leftChanged(message);
		}
		else {
