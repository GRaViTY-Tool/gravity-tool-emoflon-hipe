...
public class TClass_implements_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TClass, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TClass, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TInterface, Set<org.gravity.typegraph.basic.TClass>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TClass_implements_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnonymousClassTypeInterface__BWD_1084_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnonymousClassTypeInterface__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TClass source, org.gravity.typegraph.basic.TInterface target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TClass> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TClass>();
			pending.put((org.gravity.typegraph.basic.TInterface) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TClass) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TClass> message) {
		...
		for(org.gravity.typegraph.basic.TInterface target : message.input.getImplements()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TInterface> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TClass> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TClass source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TClass> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TClass> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TClass, org.gravity.typegraph.basic.TInterface> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TClass, org.gravity.typegraph.basic.TInterface> message) {
		Set<org.gravity.typegraph.basic.TClass> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TClass) {
			leftChanged(message);
		}
		else {
