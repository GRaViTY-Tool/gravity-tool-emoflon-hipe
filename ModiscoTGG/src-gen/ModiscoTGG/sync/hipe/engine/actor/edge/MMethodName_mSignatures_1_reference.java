...
public class MMethodName_mSignatures_1_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MName, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MName, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MSignature, Set<org.gravity.modisco.MName>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodName_mSignatures_1_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodSignature__FWD_4338_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MName source, org.gravity.modisco.MSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MName> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MName>();
			pending.put((org.gravity.modisco.MSignature) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MName) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MName> message) {
		...
		for(org.gravity.modisco.MSignature target : message.input.getMSignatures()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MName> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MName source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MName> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MName> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MName, org.gravity.modisco.MSignature> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MName, org.gravity.modisco.MSignature> message) {
		Set<org.gravity.modisco.MName> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MName) {
			leftChanged(message);
		}
		else {
