...
public class NestedClassInType__Marker_CONTEXT__SRC__eOuterType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.NestedClassInType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.NestedClassInType__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<ModiscoTGG.NestedClassInType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public NestedClassInType__Marker_CONTEXT__SRC__eOuterType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedClassInType__CONSISTENCY_triangle_10_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.NestedClassInType__Marker source, org.eclipse.modisco.java.AbstractTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.NestedClassInType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.NestedClassInType__Marker>();
			pending.put((org.eclipse.modisco.java.AbstractTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.NestedClassInType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.NestedClassInType__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.NestedClassInType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.NestedClassInType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.NestedClassInType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.NestedClassInType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.NestedClassInType__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.NestedClassInType__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		Set<ModiscoTGG.NestedClassInType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.NestedClassInType__Marker) {
			leftChanged(message);
		}
		else {
