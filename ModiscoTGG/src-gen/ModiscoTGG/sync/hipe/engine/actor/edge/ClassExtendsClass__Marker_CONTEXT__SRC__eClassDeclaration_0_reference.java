...
public class ClassExtendsClass__Marker_CONTEXT__SRC__eClassDeclaration_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassExtendsClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassExtendsClass__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.ClassDeclaration, Set<ModiscoTGG.ClassExtendsClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassExtendsClass__Marker_CONTEXT__SRC__eClassDeclaration_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassExtendsClass__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassExtendsClass__Marker source, org.eclipse.modisco.java.ClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassExtendsClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassExtendsClass__Marker>();
			pending.put((org.eclipse.modisco.java.ClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassExtendsClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassExtendsClass__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.ClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassExtendsClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassExtendsClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassExtendsClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassExtendsClass__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassExtendsClass__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassExtendsClass__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
		Set<ModiscoTGG.ClassExtendsClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassExtendsClass__Marker) {
			leftChanged(message);
		}
		else {
