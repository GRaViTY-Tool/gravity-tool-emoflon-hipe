...
public class ConstructorInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ConstructorInvocation__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ConstructorInvocation__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.ConstructorInvocation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ConstructorInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorInvocation__CONSISTENCY_2046_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ConstructorInvocation__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ConstructorInvocation__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ConstructorInvocation__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ConstructorInvocation__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ConstructorInvocation__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ConstructorInvocation__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ConstructorInvocation__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ConstructorInvocation__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ConstructorInvocation__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ConstructorInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ConstructorInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.ConstructorInvocation__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ConstructorInvocation__Marker) {
			leftChanged(message);
		}
		else {
