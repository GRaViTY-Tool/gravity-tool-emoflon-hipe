...
public class AnnotationMemberValueNumberArray__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationMemberValueNumberArray__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationMemberValueNumberArray__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue, Set<ModiscoTGG.AnnotationMemberValueNumberArray__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationMemberValueNumberArray__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueNumberArray__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationMemberValueNumberArray__Marker source, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationMemberValueNumberArray__Marker>();
			pending.put((ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationMemberValueNumberArray__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationMemberValueNumberArray__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationMemberValueNumberArray__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationMemberValueNumberArray__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		Set<ModiscoTGG.AnnotationMemberValueNumberArray__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationMemberValueNumberArray__Marker) {
			leftChanged(message);
		}
		else {
