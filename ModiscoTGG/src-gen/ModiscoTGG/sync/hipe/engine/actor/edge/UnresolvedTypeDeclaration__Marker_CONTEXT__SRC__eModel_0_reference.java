...
public class UnresolvedTypeDeclaration__Marker_CONTEXT__SRC__eModel_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MGravityModel, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MGravityModel, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MGravityModel, Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public UnresolvedTypeDeclaration__Marker_CONTEXT__SRC__eModel_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("UnresolvedTypeDeclaration__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.UnresolvedTypeDeclaration__Marker source, org.gravity.modisco.MGravityModel target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MGravityModel) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		...
		org.gravity.modisco.MGravityModel target = (org.gravity.modisco.MGravityModel) message.input.getCONTEXT__SRC__eModel();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MGravityModel> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MGravityModel> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.UnresolvedTypeDeclaration__Marker, org.gravity.modisco.MGravityModel> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.UnresolvedTypeDeclaration__Marker, org.gravity.modisco.MGravityModel> message) {
