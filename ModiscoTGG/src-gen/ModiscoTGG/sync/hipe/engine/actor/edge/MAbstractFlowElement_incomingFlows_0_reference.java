...
public class MAbstractFlowElement_incomingFlows_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MAbstractFlowElement, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MAbstractFlowElement, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MFlow, Set<org.gravity.modisco.MAbstractFlowElement>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MAbstractFlowElement_incomingFlows_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_4700_junction"), this::check_constraint_215));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__FWD_4727_junction"), this::check_constraint_217));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MAbstractFlowElement source, org.gravity.modisco.MFlow target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MAbstractFlowElement> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MAbstractFlowElement>();
			pending.put((org.gravity.modisco.MFlow) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MAbstractFlowElement) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MAbstractFlowElement> message) {
		...
		for(org.gravity.modisco.MFlow target : message.input.getIncomingFlows()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MFlow> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MAbstractFlowElement> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MAbstractFlowElement source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MAbstractFlowElement> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MAbstractFlowElement> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MAbstractFlowElement, org.gravity.modisco.MFlow> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MAbstractFlowElement, org.gravity.modisco.MFlow> message) {
		Set<org.gravity.modisco.MAbstractFlowElement> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MAbstractFlowElement) {
			leftChanged(message);
		}
		else {
	...
	public boolean check_constraint_215(EdgeMatch edge) {
	...
	public boolean check_constraint_217(EdgeMatch edge) {
		org.gravity.modisco.MAbstractFlowElement mTarget = (org.gravity.modisco.MAbstractFlowElement) edge.source();
		org.gravity.modisco.MFlow mFlow = (org.gravity.modisco.MFlow) edge.target();
		boolean predicate = !mFlow.equals(mTarget);
		edge.setConstraintSatisfied(predicate);
