...
public class ClassInnerAnonClassMember__Marker_CREATE__SRC__mInnerClass_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MClass, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MClass, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MClass, Set<ModiscoTGG.ClassInnerAnonClassMember__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassInnerAnonClassMember__Marker_CREATE__SRC__mInnerClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassInnerAnonClassMember__CONSISTENCY_triangle_11_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassInnerAnonClassMember__Marker source, org.gravity.modisco.MClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MClass) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassInnerAnonClassMember__Marker> message) {
		...
		org.gravity.modisco.MClass target = (org.gravity.modisco.MClass) message.input.getCREATE__SRC__mInnerClass();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MClass> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MClass> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassInnerAnonClassMember__Marker, org.gravity.modisco.MClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassInnerAnonClassMember__Marker, org.gravity.modisco.MClass> message) {
