...
public class TAbstractFlowElement_ownedFlows_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.TFlow, Set<org.gravity.typegraph.basic.TAbstractFlowElement>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TAbstractFlowElement_ownedFlows_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Flow__BWD_2865_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Flow__CONSISTENCY_2874_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TAbstractFlowElement source, org.gravity.typegraph.basic.TFlow target) {
	...
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TAbstractFlowElement> message) {
		...
		for(org.gravity.typegraph.basic.TFlow target : message.input.getOwnedFlows()) {
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TAbstractFlowElement, org.gravity.typegraph.basic.TFlow> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TAbstractFlowElement, org.gravity.typegraph.basic.TFlow> message) {
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TAbstractFlowElement) {
			leftChanged(message);
		}
		else {
