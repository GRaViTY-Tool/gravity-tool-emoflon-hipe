...
public class UnresolvedTypeDeclaration__Marker_CREATE__SRC__unresolvedType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.UnresolvedTypeDeclaration__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.UnresolvedTypeDeclaration__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.UnresolvedTypeDeclaration, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.UnresolvedTypeDeclaration, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.UnresolvedTypeDeclaration, Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public UnresolvedTypeDeclaration__Marker_CREATE__SRC__unresolvedType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("UnresolvedTypeDeclaration__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.UnresolvedTypeDeclaration__Marker source, org.eclipse.modisco.java.UnresolvedTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.UnresolvedTypeDeclaration__Marker>();
			pending.put((org.eclipse.modisco.java.UnresolvedTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.UnresolvedTypeDeclaration__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		...
		org.eclipse.modisco.java.UnresolvedTypeDeclaration target = (org.eclipse.modisco.java.UnresolvedTypeDeclaration) message.input.getCREATE__SRC__unresolvedType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.UnresolvedTypeDeclaration__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.UnresolvedTypeDeclaration__Marker, org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.UnresolvedTypeDeclaration__Marker, org.eclipse.modisco.java.UnresolvedTypeDeclaration> message) {
		Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.UnresolvedTypeDeclaration__Marker) {
			leftChanged(message);
		}
		else {
