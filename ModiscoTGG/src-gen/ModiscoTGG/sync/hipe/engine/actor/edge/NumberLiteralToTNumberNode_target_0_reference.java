...
public class NumberLiteralToTNumberNode_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.NumberLiteralToTNumberNode, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.NumberLiteralToTNumberNode, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TNumberNode, Set<ModiscoTGG.NumberLiteralToTNumberNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public NumberLiteralToTNumberNode_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueNumberArray__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueNumber__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.NumberLiteralToTNumberNode source, org.gravity.typegraph.basic.annotations.TNumberNode target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.NumberLiteralToTNumberNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.NumberLiteralToTNumberNode>();
			pending.put((org.gravity.typegraph.basic.annotations.TNumberNode) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.NumberLiteralToTNumberNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.NumberLiteralToTNumberNode> message) {
		...
		org.gravity.typegraph.basic.annotations.TNumberNode target = (org.gravity.typegraph.basic.annotations.TNumberNode) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TNumberNode> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.NumberLiteralToTNumberNode> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.NumberLiteralToTNumberNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.NumberLiteralToTNumberNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.NumberLiteralToTNumberNode> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.NumberLiteralToTNumberNode, org.gravity.typegraph.basic.annotations.TNumberNode> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.NumberLiteralToTNumberNode, org.gravity.typegraph.basic.annotations.TNumberNode> message) {
		Set<ModiscoTGG.NumberLiteralToTNumberNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.NumberLiteralToTNumberNode) {
			leftChanged(message);
		}
		else {
