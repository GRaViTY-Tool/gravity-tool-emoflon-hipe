...
public class MFieldNameToTField_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MFieldNameToTField, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MFieldNameToTField, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.TField, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TField, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TField, Set<ModiscoTGG.MFieldNameToTField>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MFieldNameToTField_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldName__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldSignature__BWD_2763_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldSignature__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldSignature__FWD_2803_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MFieldNameToTField source, org.gravity.typegraph.basic.TField target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MFieldNameToTField> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MFieldNameToTField>();
			pending.put((org.gravity.typegraph.basic.TField) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MFieldNameToTField) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MFieldNameToTField> message) {
		...
		org.gravity.typegraph.basic.TField target = (org.gravity.typegraph.basic.TField) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TField> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MFieldNameToTField> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MFieldNameToTField source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MFieldNameToTField> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MFieldNameToTField> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TField> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MFieldNameToTField, org.gravity.typegraph.basic.TField> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MFieldNameToTField, org.gravity.typegraph.basic.TField> message) {
		Set<ModiscoTGG.MFieldNameToTField> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MFieldNameToTField) {
			leftChanged(message);
		}
		else {
