...
public class AnnotationMemberValueBooleanArray__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue, Set<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationMemberValueBooleanArray__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueBooleanArray__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationMemberValueBooleanArray__Marker source, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker>();
			pending.put((ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationMemberValueBooleanArray__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationMemberValueBooleanArray__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		Set<ModiscoTGG.AnnotationMemberValueBooleanArray__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationMemberValueBooleanArray__Marker) {
			leftChanged(message);
		}
		else {
