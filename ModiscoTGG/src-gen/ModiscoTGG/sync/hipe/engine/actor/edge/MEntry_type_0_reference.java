...
public class MEntry_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MEntry, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MEntry, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Type, Set<org.gravity.modisco.MEntry>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MEntry_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__FWD_4231_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameter__CONSISTENCY_6318_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameter__FWD_6320_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameter_mEntry_type_outgoing_SRC__FILTER_NAC_production"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MEntry source, org.eclipse.modisco.java.Type target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MEntry> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MEntry>();
			pending.put((org.eclipse.modisco.java.Type) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MEntry) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MEntry> message) {
		...
		org.eclipse.modisco.java.Type target = (org.eclipse.modisco.java.Type) message.input.getType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Type> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MEntry> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MEntry source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MEntry> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MEntry> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MEntry, org.eclipse.modisco.java.Type> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MEntry, org.eclipse.modisco.java.Type> message) {
		Set<org.gravity.modisco.MEntry> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MEntry) {
			leftChanged(message);
		}
		else {
