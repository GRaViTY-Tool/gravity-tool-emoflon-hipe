...
public class Model_ownedElements_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.Model, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.Model, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Package, Set<org.eclipse.modisco.java.Model>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Model_ownedElements_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("PackageToPGPackage__CONSISTENCY_4753_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("PackageToPGPackage__FWD_4778_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SubPackageToPGPackage__CONSISTENCY_6412_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SubPackageToPGPackage__FWD_6414_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("SubPackageToPGPackage_mPackage_ownedElements_incoming_SRC__FILTER_NAC_production"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.Model source, org.eclipse.modisco.java.Package target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.Model> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.Model>();
			pending.put((org.eclipse.modisco.java.Package) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.Model) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.Model> message) {
		...
		for(org.eclipse.modisco.java.Package target : message.input.getOwnedElements()) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Package> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.Model> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.Model source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.Model> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.Model> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.Model, org.eclipse.modisco.java.Package> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.Model, org.eclipse.modisco.java.Package> message) {
		Set<org.eclipse.modisco.java.Model> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.eclipse.modisco.java.Model) {
			leftChanged(message);
		}
		else {
