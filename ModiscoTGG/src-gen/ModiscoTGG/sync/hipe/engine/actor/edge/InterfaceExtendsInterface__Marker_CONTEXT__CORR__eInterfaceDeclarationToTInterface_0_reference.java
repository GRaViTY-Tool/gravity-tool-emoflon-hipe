...
public class InterfaceExtendsInterface__Marker_CONTEXT__CORR__eInterfaceDeclarationToTInterface_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.InterfaceExtendsInterface__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.InterfaceExtendsInterface__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.InterfaceDeclarationToTInterface, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.InterfaceDeclarationToTInterface, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.InterfaceDeclarationToTInterface, Set<ModiscoTGG.InterfaceExtendsInterface__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public InterfaceExtendsInterface__Marker_CONTEXT__CORR__eInterfaceDeclarationToTInterface_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("InterfaceExtendsInterface__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.InterfaceExtendsInterface__Marker source, ModiscoTGG.InterfaceDeclarationToTInterface target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.InterfaceExtendsInterface__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.InterfaceExtendsInterface__Marker>();
			pending.put((ModiscoTGG.InterfaceDeclarationToTInterface) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.InterfaceExtendsInterface__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.InterfaceExtendsInterface__Marker> message) {
		...
		ModiscoTGG.InterfaceDeclarationToTInterface target = (ModiscoTGG.InterfaceDeclarationToTInterface) message.input.getCONTEXT__CORR__eInterfaceDeclarationToTInterface();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.InterfaceExtendsInterface__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.InterfaceExtendsInterface__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.InterfaceExtendsInterface__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.InterfaceExtendsInterface__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.InterfaceDeclarationToTInterface> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.InterfaceExtendsInterface__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.InterfaceExtendsInterface__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		Set<ModiscoTGG.InterfaceExtendsInterface__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.InterfaceExtendsInterface__Marker) {
			leftChanged(message);
		}
		else {
