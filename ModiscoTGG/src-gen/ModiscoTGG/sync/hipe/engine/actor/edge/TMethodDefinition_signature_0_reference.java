...
public class TMethodDefinition_signature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.TSignature, Set<org.gravity.typegraph.basic.TMember>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TMethodDefinition_signature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorDefinition__BWD_1969_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("ConstructorDefinition__CONSISTENCY_1979_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodDefinition__BWD_3708_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodDefinition__CONSISTENCY_3717_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__BWD_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__FWD_5805_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TMember source, org.gravity.typegraph.basic.TSignature target) {
	...
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TMember> message) {
		...
		org.gravity.typegraph.basic.TSignature target = (org.gravity.typegraph.basic.TSignature) message.input.getSignature();
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TMember, org.gravity.typegraph.basic.TSignature> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TMember, org.gravity.typegraph.basic.TSignature> message) {
