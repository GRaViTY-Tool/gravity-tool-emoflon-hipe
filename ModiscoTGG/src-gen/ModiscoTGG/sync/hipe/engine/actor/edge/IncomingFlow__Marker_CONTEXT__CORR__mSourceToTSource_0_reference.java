...
public class IncomingFlow__Marker_CONTEXT__CORR__mSourceToTSource_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.IncomingFlow__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.IncomingFlow__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.IncomingFlow__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public IncomingFlow__Marker_CONTEXT__CORR__mSourceToTSource_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.IncomingFlow__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.IncomingFlow__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.IncomingFlow__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.IncomingFlow__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.IncomingFlow__Marker> message) {
		...
		ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target = (ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) message.input.getCONTEXT__CORR__mSourceToTSource();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.IncomingFlow__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.IncomingFlow__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.IncomingFlow__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.IncomingFlow__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.IncomingFlow__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.IncomingFlow__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.IncomingFlow__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.IncomingFlow__Marker) {
			leftChanged(message);
		}
		else {
