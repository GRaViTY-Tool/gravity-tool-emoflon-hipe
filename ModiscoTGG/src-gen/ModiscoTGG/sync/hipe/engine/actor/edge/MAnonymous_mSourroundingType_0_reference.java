...
public class MAnonymous_mSourroundingType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MAnonymous, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MAnonymous, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<org.gravity.modisco.MAnonymous>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MAnonymous_mSourroundingType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnonymousClass__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnonymousClass__FWD_1239_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MAnonymous source, org.eclipse.modisco.java.AbstractTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MAnonymous> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MAnonymous>();
			pending.put((org.eclipse.modisco.java.AbstractTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MAnonymous) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MAnonymous> message) {
		...
		org.eclipse.modisco.java.AbstractTypeDeclaration target = (org.eclipse.modisco.java.AbstractTypeDeclaration) message.input.getMSourroundingType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MAnonymous> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MAnonymous source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MAnonymous> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MAnonymous> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MAnonymous, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MAnonymous, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		Set<org.gravity.modisco.MAnonymous> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MAnonymous) {
			leftChanged(message);
		}
		else {
