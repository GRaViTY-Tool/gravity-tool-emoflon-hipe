...
public class MDefinition_mAbstractFieldAccess_1_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MDefinition, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MDefinition, Set<EdgeMatch>>();
	private Map<org.gravity.modisco.MSingleVariableAccess, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MSingleVariableAccess, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MSingleVariableAccess, Set<org.gravity.modisco.MDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MDefinition_mAbstractFieldAccess_1_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessReadWrite_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2394_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldAccessReadWrite_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2394_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessRead_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2526_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldAccessRead_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2526_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessWrite_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2658_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldAccessWrite_mAccess_mAbstractFieldAccess_incoming_SRC__FILTER_NAC_2658_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MDefinition source, org.gravity.modisco.MSingleVariableAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MDefinition> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MDefinition>();
			pending.put((org.gravity.modisco.MSingleVariableAccess) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MDefinition) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MDefinition> message) {
		...
		for(org.gravity.modisco.MSingleVariableAccess target : message.input.getMAbstractFieldAccess()) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSingleVariableAccess> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MDefinition> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MDefinition source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MDefinition> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MDefinition> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MSingleVariableAccess> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MDefinition, org.gravity.modisco.MSingleVariableAccess> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MDefinition, org.gravity.modisco.MSingleVariableAccess> message) {
		Set<org.gravity.modisco.MDefinition> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MDefinition) {
			leftChanged(message);
		}
		else {
