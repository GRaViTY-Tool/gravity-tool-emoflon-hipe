...
public class MMethodSignature_returnType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.modisco.MMethodSignature, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.modisco.MMethodSignature, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Type, Set<org.gravity.modisco.MMethodSignature>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MMethodSignature_returnType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SignatureReturnType__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SignatureReturnType__FWD_5502_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.modisco.MMethodSignature source, org.eclipse.modisco.java.Type target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.modisco.MMethodSignature> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.modisco.MMethodSignature>();
			pending.put((org.eclipse.modisco.java.Type) target, sourcePending);
		}
		sourcePending.add((org.gravity.modisco.MMethodSignature) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.modisco.MMethodSignature> message) {
		...
		org.eclipse.modisco.java.Type target = (org.eclipse.modisco.java.Type) message.input.getReturnType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Type> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.modisco.MMethodSignature> pendingsSources = pending.get(message.input);
			for(org.gravity.modisco.MMethodSignature source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.modisco.MMethodSignature> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.modisco.MMethodSignature> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.modisco.MMethodSignature, org.eclipse.modisco.java.Type> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.modisco.MMethodSignature, org.eclipse.modisco.java.Type> message) {
		Set<org.gravity.modisco.MMethodSignature> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.modisco.MMethodSignature) {
			leftChanged(message);
		}
		else {
