...
public class LineCommentToTTextAnnotation_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.LineCommentToTTextAnnotation, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.LineCommentToTTextAnnotation, Set<EdgeMatch>>();
	private Map<org.gravity.typegraph.basic.annotations.TTextAnnotation, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.annotations.TTextAnnotation, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.annotations.TTextAnnotation, Set<ModiscoTGG.LineCommentToTTextAnnotation>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public LineCommentToTTextAnnotation_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("TextAnnotations__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.LineCommentToTTextAnnotation source, org.gravity.typegraph.basic.annotations.TTextAnnotation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.LineCommentToTTextAnnotation> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.LineCommentToTTextAnnotation>();
			pending.put((org.gravity.typegraph.basic.annotations.TTextAnnotation) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.LineCommentToTTextAnnotation) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.LineCommentToTTextAnnotation> message) {
		...
		org.gravity.typegraph.basic.annotations.TTextAnnotation target = (org.gravity.typegraph.basic.annotations.TTextAnnotation) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.LineCommentToTTextAnnotation> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.LineCommentToTTextAnnotation source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.LineCommentToTTextAnnotation> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.LineCommentToTTextAnnotation> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.LineCommentToTTextAnnotation, org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.LineCommentToTTextAnnotation, org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
		Set<ModiscoTGG.LineCommentToTTextAnnotation> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.LineCommentToTTextAnnotation) {
			leftChanged(message);
		}
		else {
