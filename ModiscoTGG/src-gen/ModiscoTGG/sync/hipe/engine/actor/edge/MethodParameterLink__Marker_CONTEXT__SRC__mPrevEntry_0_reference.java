...
public class MethodParameterLink__Marker_CONTEXT__SRC__mPrevEntry_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MEntry, Set<ModiscoTGG.MethodParameterLink__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLink__Marker_CONTEXT__SRC__mPrevEntry_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameterLink__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLink__Marker source, org.gravity.modisco.MEntry target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLink__Marker> message) {
		...
		org.gravity.modisco.MEntry target = (org.gravity.modisco.MEntry) message.input.getCONTEXT__SRC__mPrevEntry();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLink__Marker, org.gravity.modisco.MEntry> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLink__Marker, org.gravity.modisco.MEntry> message) {
