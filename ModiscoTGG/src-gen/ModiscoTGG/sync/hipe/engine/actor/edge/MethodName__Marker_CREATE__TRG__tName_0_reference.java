...
public class MethodName__Marker_CREATE__TRG__tName_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodName__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodName__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMethod, Set<ModiscoTGG.MethodName__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodName__Marker_CREATE__TRG__tName_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodName__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodName__Marker source, org.gravity.typegraph.basic.TMethod target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodName__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodName__Marker>();
			pending.put((org.gravity.typegraph.basic.TMethod) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodName__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodName__Marker> message) {
		...
		org.gravity.typegraph.basic.TMethod target = (org.gravity.typegraph.basic.TMethod) message.input.getCREATE__TRG__tName();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethod> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodName__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodName__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodName__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodName__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodName__Marker, org.gravity.typegraph.basic.TMethod> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodName__Marker, org.gravity.typegraph.basic.TMethod> message) {
		Set<ModiscoTGG.MethodName__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodName__Marker) {
			leftChanged(message);
		}
		else {
