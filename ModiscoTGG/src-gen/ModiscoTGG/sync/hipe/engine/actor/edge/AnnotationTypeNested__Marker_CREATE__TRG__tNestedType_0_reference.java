...
public class AnnotationTypeNested__Marker_CREATE__TRG__tNestedType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationTypeNested__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationTypeNested__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<ModiscoTGG.AnnotationTypeNested__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationTypeNested__Marker_CREATE__TRG__tNestedType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationTypeNested__CONSISTENCY_triangle_10_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationTypeNested__Marker source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationTypeNested__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationTypeNested__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationTypeNested__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationTypeNested__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationTypeNested__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationTypeNested__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationTypeNested__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationTypeNested__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationTypeNested__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationTypeNested__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<ModiscoTGG.AnnotationTypeNested__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationTypeNested__Marker) {
			leftChanged(message);
		}
		else {
