...
public class MFlowToTFlow_source_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFlow, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MFlow, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MFlow, Set<ModiscoTGG.MFlowToTFlow>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MFlowToTFlow_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("Flow__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__BWD_2914_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("IncomingFlow__FWD_2954_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__BWD_4691_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("OutgoingFlow__FWD_4731_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MFlowToTFlow source, org.gravity.modisco.MFlow target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MFlow) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MFlowToTFlow> message) {
		...
		org.gravity.modisco.MFlow target = (org.gravity.modisco.MFlow) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MFlow> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MFlow> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MFlowToTFlow, org.gravity.modisco.MFlow> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MFlowToTFlow, org.gravity.modisco.MFlow> message) {
