...
public class SubPackageToPGPackage__Marker_CONTEXT__TRG__tParent_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.SubPackageToPGPackage__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.SubPackageToPGPackage__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TPackage, Set<ModiscoTGG.SubPackageToPGPackage__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SubPackageToPGPackage__Marker_CONTEXT__TRG__tParent_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SubPackageToPGPackage__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SubPackageToPGPackage__Marker source, org.gravity.typegraph.basic.TPackage target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.SubPackageToPGPackage__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.SubPackageToPGPackage__Marker>();
			pending.put((org.gravity.typegraph.basic.TPackage) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.SubPackageToPGPackage__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SubPackageToPGPackage__Marker> message) {
		...
		org.gravity.typegraph.basic.TPackage target = (org.gravity.typegraph.basic.TPackage) message.input.getCONTEXT__TRG__tParent();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TPackage> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.SubPackageToPGPackage__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.SubPackageToPGPackage__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.SubPackageToPGPackage__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.SubPackageToPGPackage__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SubPackageToPGPackage__Marker, org.gravity.typegraph.basic.TPackage> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SubPackageToPGPackage__Marker, org.gravity.typegraph.basic.TPackage> message) {
		Set<ModiscoTGG.SubPackageToPGPackage__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.SubPackageToPGPackage__Marker) {
			leftChanged(message);
		}
		else {
