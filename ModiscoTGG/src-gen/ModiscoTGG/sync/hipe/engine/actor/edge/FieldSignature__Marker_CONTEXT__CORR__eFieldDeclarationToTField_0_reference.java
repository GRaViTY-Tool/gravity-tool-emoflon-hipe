...
public class FieldSignature__Marker_CONTEXT__CORR__eFieldDeclarationToTField_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MFieldNameToTField, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MFieldNameToTField, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MFieldNameToTField, Set<ModiscoTGG.FieldSignature__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldSignature__Marker_CONTEXT__CORR__eFieldDeclarationToTField_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldSignature__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldSignature__Marker source, ModiscoTGG.MFieldNameToTField target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MFieldNameToTField) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldSignature__Marker> message) {
		...
		ModiscoTGG.MFieldNameToTField target = (ModiscoTGG.MFieldNameToTField) message.input.getCONTEXT__CORR__eFieldDeclarationToTField();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MFieldNameToTField> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MFieldNameToTField> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldSignature__Marker, ModiscoTGG.MFieldNameToTField> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldSignature__Marker, ModiscoTGG.MFieldNameToTField> message) {
