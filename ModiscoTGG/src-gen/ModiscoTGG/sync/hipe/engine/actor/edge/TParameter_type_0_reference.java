...
public class TParameter_type_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.gravity.typegraph.basic.TParameter, Set<EdgeMatch>> sourceElements = new HashMap<org.gravity.typegraph.basic.TParameter, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TAbstractType, Set<org.gravity.typegraph.basic.TParameter>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TParameter_type_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__BWD_4169_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodParameterType__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameter__BWD_6317_nacjunction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameter__CONSISTENCY_6319_nacjunction"), this::returnTrue));
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameter_tParameter_type_outgoing_TRG__FILTER_NAC_production"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TParameter source, org.gravity.typegraph.basic.TAbstractType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.gravity.typegraph.basic.TParameter> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.gravity.typegraph.basic.TParameter>();
			pending.put((org.gravity.typegraph.basic.TAbstractType) target, sourcePending);
		}
		sourcePending.add((org.gravity.typegraph.basic.TParameter) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TParameter> message) {
		...
		org.gravity.typegraph.basic.TAbstractType target = (org.gravity.typegraph.basic.TAbstractType) message.input.getType();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TAbstractType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.gravity.typegraph.basic.TParameter> pendingsSources = pending.get(message.input);
			for(org.gravity.typegraph.basic.TParameter source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.gravity.typegraph.basic.TParameter> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.gravity.typegraph.basic.TParameter> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TParameter, org.gravity.typegraph.basic.TAbstractType> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TParameter, org.gravity.typegraph.basic.TAbstractType> message) {
		Set<org.gravity.typegraph.basic.TParameter> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof org.gravity.typegraph.basic.TParameter) {
			leftChanged(message);
		}
		else {
