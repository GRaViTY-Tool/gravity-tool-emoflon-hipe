...
public class Package_package_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<org.eclipse.modisco.java.Package, Set<EdgeMatch>> sourceElements = new HashMap<org.eclipse.modisco.java.Package, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.Package, Set<org.eclipse.modisco.java.Package>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Package_package_0_reference() {
	}
	...
	private void addMatch(InputMessage message, org.eclipse.modisco.java.Package source, org.eclipse.modisco.java.Package target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<org.eclipse.modisco.java.Package> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<org.eclipse.modisco.java.Package>();
			pending.put((org.eclipse.modisco.java.Package) target, sourcePending);
		}
		sourcePending.add((org.eclipse.modisco.java.Package) source);
	}
	
	private void leftAdded(NodeAddedLeft<org.eclipse.modisco.java.Package> message) {
		...
		org.eclipse.modisco.java.Package target = (org.eclipse.modisco.java.Package) message.input.getPackage();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.Package> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<org.eclipse.modisco.java.Package> pendingsSources = pending.get(message.input);
			for(org.eclipse.modisco.java.Package source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<org.eclipse.modisco.java.Package> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<org.eclipse.modisco.java.Package> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<org.eclipse.modisco.java.Package, org.eclipse.modisco.java.Package> message) {
	...
	private void removeReference(ReferenceDeleted<org.eclipse.modisco.java.Package, org.eclipse.modisco.java.Package> message) {
		Set<org.eclipse.modisco.java.Package> pendingSources = pending.get(message.target);
