...
public class NestedEnum__CO_1932_junction extends GenericJunctionActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, AbstractJunctionNode node) {
		...
		ports.add(new PortJunctionRight(getSelf(), name2actor.get("NestedEnum__CO_1930_junction"), this::check_constraint_133));
	}
	
	@Override
	protected void registerMatchForAttributeChanges(HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> tPackage_6_Matches = tPackageAttrMap.get(matchObjects[6]);
		if(tPackage_6_Matches == null) {
			tPackage_6_Matches = new LinkedList<>();
			tPackageAttrMap.put(matchObjects[6], tPackage_6_Matches);
		}
		
		tPackage_6_Matches.add(match);
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = tPackageAttrMap.get(matchObjects[6]);
		if(matches_0 != null) {
	...
	public boolean check_constraint_133(HMatch match) {
		...
		csp_45.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, "$", "java.lang.String"));
		csp_45.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, tNestedPackage.getTName(), "java.lang.String"));
		csp_45.solve();
		...
		boolean predicate = csp_45.isSatisfied() && csp_46.isSatisfied() && csp_47.isSatisfied();
