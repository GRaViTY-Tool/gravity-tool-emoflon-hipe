...
public class AnnotationMemberKey__CO_7_junction extends GenericJunctionActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, JunctionNode node) {
		...
		ports.add(new PortJunctionRight(getSelf(), name2actor.get("AnnotationMemberKey__CO_3_junction"), this::check_constraint_1));
	}
	
	@Override
	protected void registerMatchForAttributeChanges(HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> tAnnotationValue_2_Matches = tAnnotationValueAttrMap.get(matchObjects[2]);
		if(tAnnotationValue_2_Matches == null) {
			tAnnotationValue_2_Matches = new LinkedList<>();
			tAnnotationValueAttrMap.put(matchObjects[2], tAnnotationValue_2_Matches);
		}
		
		tAnnotationValue_2_Matches.add(match);
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = tAnnotationValueAttrMap.get(matchObjects[2]);
		if(matches_0 != null) {
	...
	public boolean check_constraint_1(HMatch match) {
		org.gravity.typegraph.basic.annotations.TAnnotationValue tAnnotationValue = (org.gravity.typegraph.basic.annotations.TAnnotationValue) match.getNodes()[2];
		org.eclipse.modisco.java.AnnotationMemberValuePair mValue = (org.eclipse.modisco.java.AnnotationMemberValuePair) match.getNodes()[1];
