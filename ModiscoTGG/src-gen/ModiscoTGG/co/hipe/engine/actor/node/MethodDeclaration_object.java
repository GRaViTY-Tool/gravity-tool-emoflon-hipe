...
public class MethodDeclaration_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.MethodDeclaration>> ports;		
		
	public MethodDeclaration_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.MethodDeclaration>(getSelf(), name2actor.get("MethodDeclaration_returnType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.MethodDeclaration> message) {
		for(Port<org.eclipse.modisco.java.MethodDeclaration> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.MethodDeclaration> message) {
		for(Port<org.eclipse.modisco.java.MethodDeclaration> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.MethodDeclaration> message) {
		for(Port<?> port : ports) {
