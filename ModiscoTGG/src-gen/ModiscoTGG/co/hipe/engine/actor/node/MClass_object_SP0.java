...
public class MClass_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MClass>> ports;		
		
	public MClass_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MClass>(getSelf(), name2actor.get("AbstractTypeDeclaration_bodyDeclarations_8_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MClass>(getSelf(), name2actor.get("AnonymousClassDeclaration_bodyDeclarations_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MClass>(getSelf(), name2actor.get("MAbstractMethodDefinition_mInnerTypes_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MClass>(getSelf(), name2actor.get("TypeToTAbstractType_source_4_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MClass> message) {
		for(Port<org.gravity.modisco.MClass> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MClass> message) {
		for(Port<org.gravity.modisco.MClass> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MClass> message) {
		for(Port<?> port : ports) {
