...
public class TSyntethicMethod_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TSyntethicMethod>> ports;		
		
	public TSyntethicMethod_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSyntethicMethod>(getSelf(), name2actor.get("MSyntethicMethodToTSyntethicMethod_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSyntethicMethod>(getSelf(), name2actor.get("TClass_defines_5_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSyntethicMethod>(getSelf(), name2actor.get("TMethodDefinition_syntethicMethods_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TSyntethicMethod>(getSelf(), name2actor.get("TSyntethicMethod_signature_3_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TSyntethicMethod> message) {
		for(Port<org.gravity.typegraph.basic.TSyntethicMethod> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TSyntethicMethod> message) {
		for(Port<org.gravity.typegraph.basic.TSyntethicMethod> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TSyntethicMethod> message) {
		for(Port<?> port : ports) {
