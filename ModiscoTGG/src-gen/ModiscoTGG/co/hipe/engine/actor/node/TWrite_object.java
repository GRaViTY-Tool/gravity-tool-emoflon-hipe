...
public class TWrite_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TWrite>> ports;		
		
	public TWrite_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TWrite>(getSelf(), name2actor.get("MAbstractFlowElementToTAbstractFlowElement_target_4_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TWrite>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TWrite>(getSelf(), name2actor.get("TWrite_tSource_3_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TWrite>(getSelf(), name2actor.get("TWrite_tTarget_3_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TWrite> message) {
		for(Port<org.gravity.typegraph.basic.TWrite> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TWrite> message) {
		for(Port<org.gravity.typegraph.basic.TWrite> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TWrite> message) {
		for(Port<?> port : ports) {
