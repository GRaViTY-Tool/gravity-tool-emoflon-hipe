...
public class TypeLiteral_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.TypeLiteral>> ports;		
		
	public TypeLiteral_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeLiteral>(getSelf(), name2actor.get("AnnotationMemberValuePair_value_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeLiteral>(getSelf(), name2actor.get("ArrayInitializer_expressions_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.TypeLiteral>(getSelf(), name2actor.get("ClassLiteralToClassNode_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.eclipse.modisco.java.TypeLiteral>(getSelf(), name2actor.get("TypeLiteral_type_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.TypeLiteral> message) {
		for(Port<org.eclipse.modisco.java.TypeLiteral> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.TypeLiteral> message) {
		for(Port<org.eclipse.modisco.java.TypeLiteral> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.TypeLiteral> message) {
		for(Port<?> port : ports) {
