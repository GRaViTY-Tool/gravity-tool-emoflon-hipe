...
public class TRead_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TRead>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TRead>(getSelf(), name2actor.get("MAbstractFlowElementToTAbstractFlowElement_target_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TRead>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_target_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TRead>(getSelf(), name2actor.get("TRead_tSource_2_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TRead>(getSelf(), name2actor.get("TRead_tTarget_2_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TRead> message) {
		for(Port<org.gravity.typegraph.basic.TRead> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TRead> message) {
		for(Port<org.gravity.typegraph.basic.TRead> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TRead> message) {
		for(Port<?> port : ports) {
