...
public class TSignature_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TSignature>> ports;		
		
	public TSignature_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSignature>(getSelf(), name2actor.get("MSignatureToTSignature_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSignature>(getSelf(), name2actor.get("TAbstractType_signature_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSignature>(getSelf(), name2actor.get("TClass_signature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TSignature>(getSelf(), name2actor.get("TMember_signature_2_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TSignature> message) {
		for(Port<org.gravity.typegraph.basic.TSignature> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TSignature> message) {
		for(Port<org.gravity.typegraph.basic.TSignature> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TSignature> message) {
		for(Port<?> port : ports) {
