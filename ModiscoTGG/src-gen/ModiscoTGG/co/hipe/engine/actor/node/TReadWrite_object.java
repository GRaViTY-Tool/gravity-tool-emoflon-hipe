...
public class TReadWrite_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TReadWrite>> ports;		
		
	public TReadWrite_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TReadWrite>(getSelf(), name2actor.get("MAbstractFlowElementToTAbstractFlowElement_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TReadWrite>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TReadWrite>(getSelf(), name2actor.get("TReadWrite_tSource_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TReadWrite>(getSelf(), name2actor.get("TReadWrite_tTarget_1_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TReadWrite> message) {
		for(Port<org.gravity.typegraph.basic.TReadWrite> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TReadWrite> message) {
		for(Port<org.gravity.typegraph.basic.TReadWrite> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TReadWrite> message) {
		for(Port<?> port : ports) {
