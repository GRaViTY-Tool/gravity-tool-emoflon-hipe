...
public class ReturnType__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ReturnType__Marker>> ports;		
		
	public ReturnType__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ReturnType__Marker>(getSelf(), name2actor.get("ReturnType__Marker_CONTEXT__CORR__eReturnTypeToTAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ReturnType__Marker>(getSelf(), name2actor.get("ReturnType__Marker_CONTEXT__CORR__mMethodDefinitionToTMethodDefinition_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ReturnType__Marker>(getSelf(), name2actor.get("ReturnType__Marker_CONTEXT__TRG__tAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ReturnType__Marker>(getSelf(), name2actor.get("ReturnType__Marker_CONTEXT__TRG__tMethodDefinition_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ReturnType__Marker> message) {
		for(Port<ModiscoTGG.ReturnType__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ReturnType__Marker> message) {
		for(Port<ModiscoTGG.ReturnType__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ReturnType__Marker> message) {
		for(Port<?> port : ports) {
