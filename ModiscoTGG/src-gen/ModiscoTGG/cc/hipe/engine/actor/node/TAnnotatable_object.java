...
public class TAnnotatable_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.annotations.TAnnotatable>> ports;		
		
	public TAnnotatable_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TAnnotatable>(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__Marker_CONTEXT__TRG__tAnnotable_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.annotations.TAnnotatable>(getSelf(), name2actor.get("TAnnotatable_tAnnotation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TAnnotatable> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TAnnotatable> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		for(Port<?> port : ports) {
