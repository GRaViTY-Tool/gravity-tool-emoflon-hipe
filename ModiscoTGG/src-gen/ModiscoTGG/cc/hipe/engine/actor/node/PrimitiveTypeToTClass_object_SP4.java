...
public class PrimitiveTypeToTClass_object_SP4 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PrimitiveTypeToTClass>> ports;		
		
	public PrimitiveTypeToTClass_object_SP4() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsDouble__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_8_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_6_reference"), this::returnTrue));
	}
