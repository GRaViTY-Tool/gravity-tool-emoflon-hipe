...
public class TPackage_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TPackage>> ports;		
		
	public TPackage_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TPackage>(getSelf(), name2actor.get("AnnotationType__Marker_CONTEXT__TRG__tPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TPackage>(getSelf(), name2actor.get("ClassInnerClassMember__Marker_CREATE__TRG__tInnerPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TPackage>(getSelf(), name2actor.get("NestedEnum__Marker_CREATE__TRG__tNestedPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TPackage>(getSelf(), name2actor.get("TPackage_subpackage_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TPackage> message) {
		for(Port<org.gravity.typegraph.basic.TPackage> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TPackage> message) {
		for(Port<org.gravity.typegraph.basic.TPackage> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TPackage> message) {
		for(Port<?> port : ports) {
