...
public class MDefinitionToTMember_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MDefinitionToTMember>> ports;		
		
	public MDefinitionToTMember_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__CORR__mDefinitionToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MDefinitionToTMember_target_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MDefinitionToTMember>(getSelf(), name2actor.get("MethodOverriding__Marker_CONTEXT__CORR__mOverridingToTOverriding_0_reference"), this::returnTrue));
	}
