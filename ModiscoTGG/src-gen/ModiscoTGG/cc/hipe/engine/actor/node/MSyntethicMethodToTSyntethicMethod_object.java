...
public class MSyntethicMethodToTSyntethicMethod_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>> ports;		
		
	public MSyntethicMethodToTSyntethicMethod_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>(getSelf(), name2actor.get("MSyntethicMethodToTSyntethicMethod_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>(getSelf(), name2actor.get("MSyntethicMethodToTSyntethicMethod_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>(getSelf(), name2actor.get("SyntethicMethod__Marker_CREATE__CORR__mSyntToTSynt_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		for(Port<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		for(Port<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		for(Port<?> port : ports) {
