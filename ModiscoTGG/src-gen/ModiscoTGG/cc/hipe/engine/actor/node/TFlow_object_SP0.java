...
public class TFlow_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TFlow>> ports;		
		
	public TFlow_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TFlow>(getSelf(), name2actor.get("Flow__Marker_CREATE__TRG__tFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TFlow>(getSelf(), name2actor.get("IncomingFlow__Marker_CONTEXT__TRG__tFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TFlow>(getSelf(), name2actor.get("OutgoingFlow__Marker_CONTEXT__TRG__tFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TFlow>(getSelf(), name2actor.get("TAbstractFlowElement_ownedFlows_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TFlow> message) {
		for(Port<org.gravity.typegraph.basic.TFlow> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TFlow> message) {
		for(Port<org.gravity.typegraph.basic.TFlow> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TFlow> message) {
		for(Port<?> port : ports) {
