...
public class TMethod_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TMethod>> ports;		
		
	public TMethod_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMethod>(getSelf(), name2actor.get("MethodName__Marker_CREATE__TRG__tName_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMethod>(getSelf(), name2actor.get("MethodSignature__Marker_CONTEXT__TRG__tName_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TMethod>(getSelf(), name2actor.get("TMethod_pg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TMethod> message) {
		for(Port<org.gravity.typegraph.basic.TMethod> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TMethod> message) {
		for(Port<org.gravity.typegraph.basic.TMethod> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TMethod> message) {
		for(Port<?> port : ports) {
