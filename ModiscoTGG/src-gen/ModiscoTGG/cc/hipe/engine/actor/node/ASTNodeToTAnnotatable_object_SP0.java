...
public class ASTNodeToTAnnotatable_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ASTNodeToTAnnotatable>> ports;		
		
	public ASTNodeToTAnnotatable_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ASTNodeToTAnnotatable_source_4_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("ClassInAnonymous__Marker_CREATE__CORR__mbodyToTAnnotatable_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ASTNodeToTAnnotatable>(getSelf(), name2actor.get("Interface__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
	}
