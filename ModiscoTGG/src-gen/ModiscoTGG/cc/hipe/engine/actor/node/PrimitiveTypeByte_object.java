...
public class PrimitiveTypeByte_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.PrimitiveTypeByte>> ports;		
		
	public PrimitiveTypeByte_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeByte>(getSelf(), name2actor.get("Model_orphanTypes_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeByte>(getSelf(), name2actor.get("PrimitiveTypeIsByte__Marker_CREATE__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeByte>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.PrimitiveTypeByte>(getSelf(), name2actor.get("TypeToTAbstractType_source_10_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.PrimitiveTypeByte> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeByte> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.PrimitiveTypeByte> message) {
		for(Port<org.eclipse.modisco.java.PrimitiveTypeByte> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.PrimitiveTypeByte> message) {
		for(Port<?> port : ports) {
