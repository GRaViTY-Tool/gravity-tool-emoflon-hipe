...
public class MethodOverriding__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MethodOverriding__Marker>> ports;		
		
	public MethodOverriding__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MethodOverriding__Marker>(getSelf(), name2actor.get("MethodOverriding__Marker_CONTEXT__CORR__mOverridingToTOverriding_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodOverriding__Marker>(getSelf(), name2actor.get("MethodOverriding__Marker_CONTEXT__SRC__mMethod_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodOverriding__Marker>(getSelf(), name2actor.get("MethodOverriding__Marker_CONTEXT__TRG__tOverriding_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MethodOverriding__Marker> message) {
		for(Port<ModiscoTGG.MethodOverriding__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MethodOverriding__Marker> message) {
		for(Port<ModiscoTGG.MethodOverriding__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MethodOverriding__Marker> message) {
		for(Port<?> port : ports) {
