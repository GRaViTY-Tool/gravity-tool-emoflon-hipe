...
public class PackageToTPackage_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PackageToTPackage>> ports;		
		
	public PackageToTPackage_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PackageToTPackage>(getSelf(), name2actor.get("AnnotationType__Marker_CONTEXT__CORR__mPackageToTPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.PackageToTPackage>(getSelf(), name2actor.get("Interface__Marker_CONTEXT__CORR__mPackageToTPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.PackageToTPackage>(getSelf(), name2actor.get("SubPackageToPGPackage__Marker_CONTEXT__CORR__mParentPackageToTParent_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.PackageToTPackage> message) {
		for(Port<ModiscoTGG.PackageToTPackage> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.PackageToTPackage> message) {
		for(Port<ModiscoTGG.PackageToTPackage> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.PackageToTPackage> message) {
		for(Port<?> port : ports) {
