...
public class MMethodName_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MMethodName>> ports;		
		
	public MMethodName_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MMethodName>(getSelf(), name2actor.get("MGravityModel_mMethodNames_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MMethodName>(getSelf(), name2actor.get("MethodName__Marker_CREATE__SRC__mName_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MMethodName>(getSelf(), name2actor.get("MethodSignature__Marker_CONTEXT__SRC__mName_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MMethodName> message) {
		for(Port<org.gravity.modisco.MMethodName> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MMethodName> message) {
		for(Port<org.gravity.modisco.MMethodName> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MMethodName> message) {
		for(Port<?> port : ports) {
