...
public class LineCommentToTTextAnnotation_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.LineCommentToTTextAnnotation>> ports;		
		
	public LineCommentToTTextAnnotation_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.LineCommentToTTextAnnotation>(getSelf(), name2actor.get("LineCommentToTTextAnnotation_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.LineCommentToTTextAnnotation>(getSelf(), name2actor.get("LineCommentToTTextAnnotation_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.LineCommentToTTextAnnotation>(getSelf(), name2actor.get("TextAnnotations__Marker_CREATE__CORR__mCommentToTText_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.LineCommentToTTextAnnotation> message) {
		for(Port<ModiscoTGG.LineCommentToTTextAnnotation> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.LineCommentToTTextAnnotation> message) {
		for(Port<ModiscoTGG.LineCommentToTTextAnnotation> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.LineCommentToTTextAnnotation> message) {
		for(Port<?> port : ports) {
