...
public class ModelToTypeGraph_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ModelToTypeGraph>> ports;		
		
	public ModelToTypeGraph_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.ModelToTypeGraph>(getSelf(), name2actor.get("ArrayTypeToClass__Marker_CONTEXT__CORR__eModelToTypeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ModelToTypeGraph>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CONTEXT__CORR__eModelToPg_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ModelToTypeGraph>(getSelf(), name2actor.get("PrimitiveTypeIsInt__Marker_CONTEXT__CORR__eModelToTypeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.ModelToTypeGraph>(getSelf(), name2actor.get("UnresolvedTypeDeclaration__Marker_CONTEXT__CORR__eModelToPg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ModelToTypeGraph> message) {
		for(Port<ModiscoTGG.ModelToTypeGraph> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ModelToTypeGraph> message) {
		for(Port<ModiscoTGG.ModelToTypeGraph> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ModelToTypeGraph> message) {
		for(Port<?> port : ports) {
