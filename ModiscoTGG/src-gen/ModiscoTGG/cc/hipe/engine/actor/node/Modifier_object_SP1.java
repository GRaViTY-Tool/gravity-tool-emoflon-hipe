...
public class Modifier_object_SP1 extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.Modifier>> ports;		
		
	public Modifier_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Modifier>(getSelf(), name2actor.get("MModifierToTModifier_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Modifier>(getSelf(), name2actor.get("MemberInAnnotationVisibility__Marker_CREATE__SRC__mModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Modifier>(getSelf(), name2actor.get("MemberInEnumVisibility__Marker_CREATE__SRC__mModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.Modifier>(getSelf(), name2actor.get("TypesVisibility__Marker_CREATE__SRC__mModifier_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.Modifier> message) {
		for(Port<org.eclipse.modisco.java.Modifier> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.Modifier> message) {
		for(Port<org.eclipse.modisco.java.Modifier> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.Modifier> message) {
		for(Port<?> port : ports) {
