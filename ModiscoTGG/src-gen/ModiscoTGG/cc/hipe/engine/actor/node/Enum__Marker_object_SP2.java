...
public class Enum__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.Enum__Marker>> ports;		
		
	public Enum__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.Enum__Marker>(getSelf(), name2actor.get("Enum__Marker_CONTEXT__TRG__tPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Enum__Marker>(getSelf(), name2actor.get("Enum__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Enum__Marker>(getSelf(), name2actor.get("Enum__Marker_CREATE__TRG__tAnnotation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.Enum__Marker> message) {
		for(Port<ModiscoTGG.Enum__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.Enum__Marker> message) {
		for(Port<ModiscoTGG.Enum__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.Enum__Marker> message) {
		for(Port<?> port : ports) {
