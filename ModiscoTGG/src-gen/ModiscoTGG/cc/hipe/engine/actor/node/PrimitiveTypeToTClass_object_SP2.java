...
public class PrimitiveTypeToTClass_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PrimitiveTypeToTClass>> ports;		
		
	public PrimitiveTypeToTClass_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsBoolean__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsShort__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_5_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_3_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.PrimitiveTypeToTClass> message) {
		for(Port<ModiscoTGG.PrimitiveTypeToTClass> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.PrimitiveTypeToTClass> message) {
		for(Port<ModiscoTGG.PrimitiveTypeToTClass> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.PrimitiveTypeToTClass> message) {
		for(Port<?> port : ports) {
