...
public class MSignature_object extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MSignature>> ports;		
		
	public MSignature_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MSignature>(getSelf(), name2actor.get("LinkDefinitionToAnonymous__Marker_CONTEXT__SRC__mSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MSignature>(getSelf(), name2actor.get("LinkDefinitionToType__Marker_CONTEXT__SRC__mSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MSignature>(getSelf(), name2actor.get("MDefinition_mSignature_2_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MSignature>(getSelf(), name2actor.get("MSignatureToTSignature_source_2_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MSignature> message) {
		for(Port<org.gravity.modisco.MSignature> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MSignature> message) {
		for(Port<org.gravity.modisco.MSignature> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MSignature> message) {
		for(Port<?> port : ports) {
