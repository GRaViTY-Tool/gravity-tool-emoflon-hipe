...
public class TMember_object_SP3 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TMember>> ports;		
		
	public TMember_object_SP3() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMember>(getSelf(), name2actor.get("ClassInstanceCreation__Marker_CONTEXT__TRG__tMethod_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMember>(getSelf(), name2actor.get("FieldAccessWrite__Marker_CONTEXT__TRG__tMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMember>(getSelf(), name2actor.get("SuperMethodInvocation__Marker_CONTEXT__TRG__tMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TMember>(getSelf(), name2actor.get("TAnnotationType_defines_4_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TMember> message) {
		for(Port<org.gravity.typegraph.basic.TMember> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TMember> message) {
		for(Port<org.gravity.typegraph.basic.TMember> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TMember> message) {
		for(Port<?> port : ports) {
