...
public class IncomingFlow__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.IncomingFlow__Marker>> ports;		
		
	public IncomingFlow__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.IncomingFlow__Marker>(getSelf(), name2actor.get("IncomingFlow__Marker_CONTEXT__CORR__mSourceToTSource_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.IncomingFlow__Marker>(getSelf(), name2actor.get("IncomingFlow__Marker_CONTEXT__SRC__mFlow_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.IncomingFlow__Marker>(getSelf(), name2actor.get("IncomingFlow__Marker_CONTEXT__TRG__tSource_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.IncomingFlow__Marker> message) {
		for(Port<ModiscoTGG.IncomingFlow__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.IncomingFlow__Marker> message) {
		for(Port<ModiscoTGG.IncomingFlow__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.IncomingFlow__Marker> message) {
		for(Port<?> port : ports) {
