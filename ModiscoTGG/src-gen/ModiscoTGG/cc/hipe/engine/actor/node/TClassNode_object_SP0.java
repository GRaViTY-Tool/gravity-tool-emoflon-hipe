...
public class TClassNode_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.annotations.TClassNode>> ports;		
		
	public TClassNode_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TClassNode>(getSelf(), name2actor.get("AnnotationMemberValueClass__Marker_CREATE__TRG__tValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TClassNode>(getSelf(), name2actor.get("ClassLiteralToClassNode_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TClassNode>(getSelf(), name2actor.get("TAnnotationValue_tValue_2_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.annotations.TClassNode> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TClassNode> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.annotations.TClassNode> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TClassNode> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.annotations.TClassNode> message) {
		for(Port<?> port : ports) {
