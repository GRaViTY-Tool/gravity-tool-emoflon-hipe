...
public class MAnonymous_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MAnonymous>> ports;		
		
	public MAnonymous_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("AnonymousClassDeclarationToTClass_source_1_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("AnonymousClassTypeClass__Marker_CONTEXT__SRC__mAnonymous_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("MAnonymous_bodyDeclarations_5_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("MAnonymous_mSourroundingType_0_reference"), this::returnTrue));
	}
