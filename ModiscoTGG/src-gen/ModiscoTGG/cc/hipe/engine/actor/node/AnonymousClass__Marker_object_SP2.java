...
public class AnonymousClass__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.AnonymousClass__Marker>> ports;		
		
	public AnonymousClass__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.AnonymousClass__Marker>(getSelf(), name2actor.get("AnonymousClass__Marker_CONTEXT__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnonymousClass__Marker>(getSelf(), name2actor.get("AnonymousClass__Marker_CONTEXT__TRG__tPackage_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnonymousClass__Marker>(getSelf(), name2actor.get("AnonymousClass__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.AnonymousClass__Marker> message) {
		for(Port<ModiscoTGG.AnonymousClass__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.AnonymousClass__Marker> message) {
		for(Port<ModiscoTGG.AnonymousClass__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.AnonymousClass__Marker> message) {
		for(Port<?> port : ports) {
