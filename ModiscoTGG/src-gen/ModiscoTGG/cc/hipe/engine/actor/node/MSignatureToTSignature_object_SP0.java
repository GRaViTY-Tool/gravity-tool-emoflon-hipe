...
public class MSignatureToTSignature_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSignatureToTSignature>> ports;		
		
	public MSignatureToTSignature_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("FieldDefinition__Marker_CONTEXT__CORR__mSignatureToTFieldSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("LinkDefinitionToAnonymous__Marker_CONTEXT__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MSignatureToTSignature_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSignatureToTSignature>(getSelf(), name2actor.get("MethodParameter__Marker_CONTEXT__CORR__mSignatureToTSignature_0_reference"), this::returnTrue));
	}
