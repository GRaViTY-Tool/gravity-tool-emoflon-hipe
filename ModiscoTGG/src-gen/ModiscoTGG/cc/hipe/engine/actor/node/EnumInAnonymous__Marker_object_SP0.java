...
public class EnumInAnonymous__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.EnumInAnonymous__Marker>> ports;		
		
	public EnumInAnonymous__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.EnumInAnonymous__Marker>(getSelf(), name2actor.get("EnumInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.EnumInAnonymous__Marker>(getSelf(), name2actor.get("EnumInAnonymous__Marker_CREATE__CORR__mbodyToTAnnotatable_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.EnumInAnonymous__Marker>(getSelf(), name2actor.get("EnumInAnonymous__Marker_CREATE__TRG__tNestedType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.EnumInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.EnumInAnonymous__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.EnumInAnonymous__Marker> message) {
		for(Port<ModiscoTGG.EnumInAnonymous__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.EnumInAnonymous__Marker> message) {
		for(Port<?> port : ports) {
