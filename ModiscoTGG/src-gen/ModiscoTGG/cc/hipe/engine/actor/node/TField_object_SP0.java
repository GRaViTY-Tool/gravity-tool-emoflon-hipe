...
public class TField_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TField>> ports;		
		
	public TField_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TField>(getSelf(), name2actor.get("FieldName__Marker_CREATE__TRG__tField_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TField>(getSelf(), name2actor.get("FieldSignature__Marker_CONTEXT__TRG__tField_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.typegraph.basic.TField>(getSelf(), name2actor.get("TField_pg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TField> message) {
		for(Port<org.gravity.typegraph.basic.TField> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TField> message) {
		for(Port<org.gravity.typegraph.basic.TField> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TField> message) {
		for(Port<?> port : ports) {
