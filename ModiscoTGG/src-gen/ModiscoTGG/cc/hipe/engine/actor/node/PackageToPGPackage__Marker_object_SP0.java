...
public class PackageToPGPackage__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PackageToPGPackage__Marker>> ports;		
		
	public PackageToPGPackage__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.PackageToPGPackage__Marker>(getSelf(), name2actor.get("PackageToPGPackage__Marker_CONTEXT__CORR__mModelToTypeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PackageToPGPackage__Marker>(getSelf(), name2actor.get("PackageToPGPackage__Marker_CONTEXT__TRG__typeGraph_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PackageToPGPackage__Marker>(getSelf(), name2actor.get("PackageToPGPackage__Marker_CREATE__SRC__mPackage_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.PackageToPGPackage__Marker> message) {
		for(Port<ModiscoTGG.PackageToPGPackage__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.PackageToPGPackage__Marker> message) {
		for(Port<ModiscoTGG.PackageToPGPackage__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.PackageToPGPackage__Marker> message) {
		for(Port<?> port : ports) {
