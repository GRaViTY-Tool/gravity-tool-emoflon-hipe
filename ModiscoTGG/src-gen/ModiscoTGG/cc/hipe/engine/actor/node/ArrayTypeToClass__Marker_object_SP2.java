...
public class ArrayTypeToClass__Marker_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.ArrayTypeToClass__Marker>> ports;		
		
	public ArrayTypeToClass__Marker_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ArrayTypeToClass__Marker>(getSelf(), name2actor.get("ArrayTypeToClass__Marker_CONTEXT__CORR__eTypeToTArrayClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ArrayTypeToClass__Marker>(getSelf(), name2actor.get("ArrayTypeToClass__Marker_CONTEXT__SRC__eModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ArrayTypeToClass__Marker>(getSelf(), name2actor.get("ArrayTypeToClass__Marker_CREATE__CORR__eArrayTypeToTArrayClass_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ArrayTypeToClass__Marker> message) {
		for(Port<ModiscoTGG.ArrayTypeToClass__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ArrayTypeToClass__Marker> message) {
		for(Port<ModiscoTGG.ArrayTypeToClass__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ArrayTypeToClass__Marker> message) {
		for(Port<?> port : ports) {
