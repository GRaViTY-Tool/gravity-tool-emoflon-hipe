...
public class FieldType__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.FieldType__Marker>> ports;		
		
	public FieldType__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.FieldType__Marker>(getSelf(), name2actor.get("FieldType__Marker_CONTEXT__CORR__mSignatureToTFieldSignature_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.FieldType__Marker>(getSelf(), name2actor.get("FieldType__Marker_CONTEXT__SRC__ePrimitiveType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.FieldType__Marker>(getSelf(), name2actor.get("FieldType__Marker_CONTEXT__TRG__tFieldSignature_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.FieldType__Marker> message) {
		for(Port<ModiscoTGG.FieldType__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.FieldType__Marker> message) {
		for(Port<ModiscoTGG.FieldType__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.FieldType__Marker> message) {
		for(Port<?> port : ports) {
