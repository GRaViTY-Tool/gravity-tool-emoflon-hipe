...
public class TTextAnnotation_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.annotations.TTextAnnotation>> ports;		
		
	public TTextAnnotation_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TTextAnnotation>(getSelf(), name2actor.get("LineCommentToTTextAnnotation_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TTextAnnotation>(getSelf(), name2actor.get("TMember_tAnnotation_3_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.annotations.TTextAnnotation>(getSelf(), name2actor.get("TextAnnotations__Marker_CREATE__TRG__tText_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TTextAnnotation> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
		for(Port<org.gravity.typegraph.basic.annotations.TTextAnnotation> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.annotations.TTextAnnotation> message) {
		for(Port<?> port : ports) {
