...
public class TypesOrphaned__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypesOrphaned__Marker>> ports;		
		
	public TypesOrphaned__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.TypesOrphaned__Marker>(getSelf(), name2actor.get("TypesOrphaned__Marker_CONTEXT__SRC__mModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypesOrphaned__Marker>(getSelf(), name2actor.get("TypesOrphaned__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypesOrphaned__Marker>(getSelf(), name2actor.get("TypesOrphaned__Marker_CREATE__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypesOrphaned__Marker>(getSelf(), name2actor.get("TypesOrphaned__Marker_CREATE__TRG__tType_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.TypesOrphaned__Marker> message) {
		for(Port<ModiscoTGG.TypesOrphaned__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.TypesOrphaned__Marker> message) {
		for(Port<ModiscoTGG.TypesOrphaned__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.TypesOrphaned__Marker> message) {
		for(Port<?> port : ports) {
