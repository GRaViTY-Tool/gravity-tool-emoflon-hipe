...
public class TypeToTAbstractType_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypeToTAbstractType>> ports;		
		
	public TypeToTAbstractType_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("ClassInnerAnonClassMember__Marker_CREATE__CORR__tInnerTypeToTInnerAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("NestedClassInType__Marker_CREATE__CORR__eNestedTypeToTNestedType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("PrimitiveTypeIsInt__Marker_CREATE__CORR__typeToTAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("TypeToTAbstractType_source_0_reference"), this::returnTrue));
	}
