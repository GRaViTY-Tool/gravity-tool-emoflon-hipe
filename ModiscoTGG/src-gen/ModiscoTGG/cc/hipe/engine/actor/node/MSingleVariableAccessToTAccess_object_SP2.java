...
public class MSingleVariableAccessToTAccess_object_SP2 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSingleVariableAccessToTAccess>> ports;		
		
	public MSingleVariableAccessToTAccess_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("FieldAccessReadWrite__Marker_CREATE__CORR__mAccessToTAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("FieldAccessRead__Marker_CREATE__CORR__mAccessToTAccess_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("FieldAccessWrite__Marker_CREATE__CORR__mAccessToTAccess_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
		for(Port<ModiscoTGG.MSingleVariableAccessToTAccess> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
		for(Port<ModiscoTGG.MSingleVariableAccessToTAccess> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MSingleVariableAccessToTAccess> message) {
		for(Port<?> port : ports) {
