...
public class MSingleVariableAccessToTAccess_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MSingleVariableAccessToTAccess>> ports;		
		
	public MSingleVariableAccessToTAccess_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_source_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_source_1_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MSingleVariableAccessToTAccess>(getSelf(), name2actor.get("MSingleVariableAccessToTAccess_source_2_reference"), this::returnTrue));
	}
