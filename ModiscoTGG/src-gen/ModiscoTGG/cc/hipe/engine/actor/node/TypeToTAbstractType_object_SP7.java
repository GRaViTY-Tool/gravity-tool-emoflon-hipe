...
public class TypeToTAbstractType_object_SP7 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypeToTAbstractType>> ports;		
		
	public TypeToTAbstractType_object_SP7() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("AnnotationTypeInAnonymous__Marker_CREATE__CORR__mNestedTypeToTNestedType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("Enum__Marker_CREATE__CORR__mTypeToTType_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("ParameterizedTypeToClass__Marker_CREATE__CORR__eParameterizedTypeToTAbstractType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypeToTAbstractType>(getSelf(), name2actor.get("TypeToTAbstractType_source_16_reference"), this::returnTrue));
	}
