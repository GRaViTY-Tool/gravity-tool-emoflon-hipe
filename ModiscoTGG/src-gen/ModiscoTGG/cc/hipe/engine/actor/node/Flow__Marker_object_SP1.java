...
public class Flow__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.Flow__Marker>> ports;		
		
	public Flow__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.Flow__Marker>(getSelf(), name2actor.get("Flow__Marker_CONTEXT__CORR__mOwnerToTOwner_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Flow__Marker>(getSelf(), name2actor.get("Flow__Marker_CONTEXT__TRG__tOwner_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.Flow__Marker>(getSelf(), name2actor.get("Flow__Marker_CREATE__SRC__mFlow_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.Flow__Marker> message) {
		for(Port<ModiscoTGG.Flow__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.Flow__Marker> message) {
		for(Port<ModiscoTGG.Flow__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.Flow__Marker> message) {
		for(Port<?> port : ports) {
