...
public class TypesVisibility__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TypesVisibility__Marker>> ports;		
		
	public TypesVisibility__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.TypesVisibility__Marker>(getSelf(), name2actor.get("TypesVisibility__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypesVisibility__Marker>(getSelf(), name2actor.get("TypesVisibility__Marker_CREATE__CORR__mModifierToTModifier_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TypesVisibility__Marker>(getSelf(), name2actor.get("TypesVisibility__Marker_CREATE__TRG__tModifier_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.TypesVisibility__Marker> message) {
		for(Port<ModiscoTGG.TypesVisibility__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.TypesVisibility__Marker> message) {
		for(Port<ModiscoTGG.TypesVisibility__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.TypesVisibility__Marker> message) {
		for(Port<?> port : ports) {
