...
public class TConstructor_object extends AbstractActor {
	
	private List<Port<org.gravity.typegraph.basic.TConstructor>> ports;		
		
	public TConstructor_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TConstructor>(getSelf(), name2actor.get("ConstructorDefinition__Marker_CREATE__TRG__tAnnotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.typegraph.basic.TConstructor>(getSelf(), name2actor.get("TMethodDefinition_tAnnotation_1_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.typegraph.basic.TConstructor> message) {
		for(Port<org.gravity.typegraph.basic.TConstructor> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.typegraph.basic.TConstructor> message) {
		for(Port<org.gravity.typegraph.basic.TConstructor> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.typegraph.basic.TConstructor> message) {
		for(Port<?> port : ports) {
