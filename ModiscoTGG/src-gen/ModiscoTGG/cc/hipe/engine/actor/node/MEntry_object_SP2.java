...
public class MEntry_object_SP2 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MEntry>> ports;		
		
	public MEntry_object_SP2() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MEntry>(getSelf(), name2actor.get("MMethodSignature_mEntrys_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MEntry>(getSelf(), name2actor.get("MethodParameterLinkFirst__Marker_CONTEXT__SRC__mEntry_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MEntry>(getSelf(), name2actor.get("MethodParameterLink__Marker_CONTEXT__SRC__mEntry_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MEntry>(getSelf(), name2actor.get("MethodParameter__Marker_CREATE__SRC__mEntry_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MEntry> message) {
		for(Port<org.gravity.modisco.MEntry> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MEntry> message) {
		for(Port<org.gravity.modisco.MEntry> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MEntry> message) {
		for(Port<?> port : ports) {
