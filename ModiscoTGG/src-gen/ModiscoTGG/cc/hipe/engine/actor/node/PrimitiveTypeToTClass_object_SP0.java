...
public class PrimitiveTypeToTClass_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.PrimitiveTypeToTClass>> ports;		
		
	public PrimitiveTypeToTClass_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeIsFloat__Marker_CREATE__CORR__ePrimitiveTypeToTPrimitiveClass_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_source_2_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.PrimitiveTypeToTClass>(getSelf(), name2actor.get("PrimitiveTypeToTClass_target_7_reference"), this::returnTrue));
	}
