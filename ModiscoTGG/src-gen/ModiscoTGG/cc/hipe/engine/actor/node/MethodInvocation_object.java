...
public class MethodInvocation_object extends AbstractActor {
	
	private List<Port<org.eclipse.modisco.java.MethodInvocation>> ports;		
		
	public MethodInvocation_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.eclipse.modisco.java.MethodInvocation>(getSelf(), name2actor.get("ArrayInitializerIgnoreExpressionsEdge__Marker_CONTEXT__SRC__invocation_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.eclipse.modisco.java.MethodInvocation>(getSelf(), name2actor.get("ArrayInitializer_expressions_5_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.eclipse.modisco.java.MethodInvocation> message) {
		for(Port<org.eclipse.modisco.java.MethodInvocation> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.eclipse.modisco.java.MethodInvocation> message) {
		for(Port<org.eclipse.modisco.java.MethodInvocation> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.eclipse.modisco.java.MethodInvocation> message) {
		for(Port<?> port : ports) {
