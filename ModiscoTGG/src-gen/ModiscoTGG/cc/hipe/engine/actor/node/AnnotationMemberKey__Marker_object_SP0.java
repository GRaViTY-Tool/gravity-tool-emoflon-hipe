...
public class AnnotationMemberKey__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.AnnotationMemberKey__Marker>> ports;		
		
	public AnnotationMemberKey__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationMemberKey__Marker>(getSelf(), name2actor.get("AnnotationMemberKey__Marker_CONTEXT__SRC__Annotation_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationMemberKey__Marker>(getSelf(), name2actor.get("AnnotationMemberKey__Marker_CREATE__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.AnnotationMemberKey__Marker>(getSelf(), name2actor.get("AnnotationMemberKey__Marker_CREATE__TRG__tAnnotationValue_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.AnnotationMemberKey__Marker> message) {
		for(Port<ModiscoTGG.AnnotationMemberKey__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.AnnotationMemberKey__Marker> message) {
		for(Port<ModiscoTGG.AnnotationMemberKey__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.AnnotationMemberKey__Marker> message) {
		for(Port<?> port : ports) {
