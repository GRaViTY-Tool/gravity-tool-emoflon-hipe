...
public class MFieldName_object_SP0 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MFieldName>> ports;		
		
	public MFieldName_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldName>(getSelf(), name2actor.get("FieldName__Marker_CREATE__SRC__mFieldName_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldName>(getSelf(), name2actor.get("FieldSignature__Marker_CONTEXT__SRC__mFieldName_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MFieldName>(getSelf(), name2actor.get("MGravityModel_mFieldNames_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MFieldName> message) {
		for(Port<org.gravity.modisco.MFieldName> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MFieldName> message) {
		for(Port<org.gravity.modisco.MFieldName> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MFieldName> message) {
		for(Port<?> port : ports) {
