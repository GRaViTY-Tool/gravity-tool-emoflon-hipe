...
public class ArrayInitializerIgnoreExpressionsEdge__Marker_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker>> ports;		
		
	public ArrayInitializerIgnoreExpressionsEdge__Marker_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker>(getSelf(), name2actor.get("ArrayInitializerIgnoreExpressionsEdge__Marker_CONTEXT__SRC__array_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker>(getSelf(), name2actor.get("ArrayInitializerIgnoreExpressionsEdge__Marker_CONTEXT__SRC__invocation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> message) {
		for(Port<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> message) {
		for(Port<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> message) {
		for(Port<?> port : ports) {
