...
public class ModelToPg__Marker_object extends AbstractActor {
	
	private List<Port<ModiscoTGG.ModelToPg__Marker>> ports;		
		
	public ModelToPg__Marker_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.ModelToPg__Marker>(getSelf(), name2actor.get("ModelToPg__Marker_CREATE__CORR__mModelToTPG_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ModelToPg__Marker>(getSelf(), name2actor.get("ModelToPg__Marker_CREATE__SRC__mModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.ModelToPg__Marker>(getSelf(), name2actor.get("ModelToPg__Marker_CREATE__TRG__tPG_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.ModelToPg__Marker> message) {
		for(Port<ModiscoTGG.ModelToPg__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.ModelToPg__Marker> message) {
		for(Port<ModiscoTGG.ModelToPg__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.ModelToPg__Marker> message) {
		for(Port<?> port : ports) {
