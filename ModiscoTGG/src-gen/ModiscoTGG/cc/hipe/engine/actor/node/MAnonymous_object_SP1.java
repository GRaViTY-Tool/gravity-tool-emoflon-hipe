...
public class MAnonymous_object_SP1 extends AbstractActor {
	
	private List<Port<org.gravity.modisco.MAnonymous>> ports;		
		
	public MAnonymous_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("AnonymousClassTypeInterface__Marker_CONTEXT__SRC__mAnonymous_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("AnonymousClass__Marker_CREATE__SRC__mAnonymous_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("LinkDefinitionToAnonymous__Marker_CONTEXT__SRC__mType_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<org.gravity.modisco.MAnonymous>(getSelf(), name2actor.get("MAnonymous_classInstanceCreation_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<org.gravity.modisco.MAnonymous> message) {
		for(Port<org.gravity.modisco.MAnonymous> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<org.gravity.modisco.MAnonymous> message) {
		for(Port<org.gravity.modisco.MAnonymous> port : ports) {
	...
	private void changeAttribute(AttributeChanged<org.gravity.modisco.MAnonymous> message) {
		for(Port<?> port : ports) {
