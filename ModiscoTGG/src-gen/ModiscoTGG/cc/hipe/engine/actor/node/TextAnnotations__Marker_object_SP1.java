...
public class TextAnnotations__Marker_object_SP1 extends AbstractActor {
	
	private List<Port<ModiscoTGG.TextAnnotations__Marker>> ports;		
		
	public TextAnnotations__Marker_object_SP1() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.TextAnnotations__Marker>(getSelf(), name2actor.get("TextAnnotations__Marker_CONTEXT__CORR__mMemberToTMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TextAnnotations__Marker>(getSelf(), name2actor.get("TextAnnotations__Marker_CONTEXT__TRG__tMember_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.TextAnnotations__Marker>(getSelf(), name2actor.get("TextAnnotations__Marker_CREATE__SRC__mComment_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.TextAnnotations__Marker> message) {
		for(Port<ModiscoTGG.TextAnnotations__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.TextAnnotations__Marker> message) {
		for(Port<ModiscoTGG.TextAnnotations__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.TextAnnotations__Marker> message) {
		for(Port<?> port : ports) {
