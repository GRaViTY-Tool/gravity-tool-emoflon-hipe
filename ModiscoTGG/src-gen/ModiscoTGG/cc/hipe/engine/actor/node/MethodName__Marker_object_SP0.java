...
public class MethodName__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MethodName__Marker>> ports;		
		
	public MethodName__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MethodName__Marker>(getSelf(), name2actor.get("MethodName__Marker_CONTEXT__SRC__mModel_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodName__Marker>(getSelf(), name2actor.get("MethodName__Marker_CREATE__CORR__mNameToTName_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodName__Marker>(getSelf(), name2actor.get("MethodName__Marker_CREATE__TRG__tName_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MethodName__Marker> message) {
		for(Port<ModiscoTGG.MethodName__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MethodName__Marker> message) {
		for(Port<ModiscoTGG.MethodName__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MethodName__Marker> message) {
		for(Port<?> port : ports) {
