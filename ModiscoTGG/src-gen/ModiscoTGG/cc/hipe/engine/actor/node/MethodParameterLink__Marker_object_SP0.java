...
public class MethodParameterLink__Marker_object_SP0 extends AbstractActor {
	
	private List<Port<ModiscoTGG.MethodParameterLink__Marker>> ports;		
		
	public MethodParameterLink__Marker_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLink__Marker>(getSelf(), name2actor.get("MethodParameterLink__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLink__Marker>(getSelf(), name2actor.get("MethodParameterLink__Marker_CONTEXT__SRC__mPrevEntry_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<ModiscoTGG.MethodParameterLink__Marker>(getSelf(), name2actor.get("MethodParameterLink__Marker_CONTEXT__TRG__tParameter_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<ModiscoTGG.MethodParameterLink__Marker> message) {
		for(Port<ModiscoTGG.MethodParameterLink__Marker> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<ModiscoTGG.MethodParameterLink__Marker> message) {
		for(Port<ModiscoTGG.MethodParameterLink__Marker> port : ports) {
	...
	private void changeAttribute(AttributeChanged<ModiscoTGG.MethodParameterLink__Marker> message) {
		for(Port<?> port : ports) {
