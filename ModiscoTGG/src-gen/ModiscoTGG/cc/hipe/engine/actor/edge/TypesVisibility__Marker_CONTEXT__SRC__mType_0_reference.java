...
public class TypesVisibility__Marker_CONTEXT__SRC__mType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.TypesVisibility__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.TypesVisibility__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AbstractTypeDeclaration, Set<ModiscoTGG.TypesVisibility__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TypesVisibility__Marker_CONTEXT__SRC__mType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("TypesVisibility__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.TypesVisibility__Marker source, org.eclipse.modisco.java.AbstractTypeDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.TypesVisibility__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.TypesVisibility__Marker>();
			pending.put((org.eclipse.modisco.java.AbstractTypeDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.TypesVisibility__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.TypesVisibility__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.TypesVisibility__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.TypesVisibility__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.TypesVisibility__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.TypesVisibility__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.TypesVisibility__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.TypesVisibility__Marker, org.eclipse.modisco.java.AbstractTypeDeclaration> message) {
		Set<ModiscoTGG.TypesVisibility__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.TypesVisibility__Marker) {
			leftChanged(message);
		}
		else {
