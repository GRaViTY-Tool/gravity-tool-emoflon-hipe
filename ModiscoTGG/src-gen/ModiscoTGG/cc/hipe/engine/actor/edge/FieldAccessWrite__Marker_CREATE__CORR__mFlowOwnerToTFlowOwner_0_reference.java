...
public class FieldAccessWrite__Marker_CREATE__CORR__mFlowOwnerToTFlowOwner_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.FieldAccessWrite__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.FieldAccessWrite__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.FieldAccessWrite__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldAccessWrite__Marker_CREATE__CORR__mFlowOwnerToTFlowOwner_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("FieldAccessWrite__CONSISTENCY_2217_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldAccessWrite__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.FieldAccessWrite__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.FieldAccessWrite__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.FieldAccessWrite__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldAccessWrite__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.FieldAccessWrite__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.FieldAccessWrite__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.FieldAccessWrite__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.FieldAccessWrite__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldAccessWrite__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldAccessWrite__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.FieldAccessWrite__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.FieldAccessWrite__Marker) {
			leftChanged(message);
		}
		else {
