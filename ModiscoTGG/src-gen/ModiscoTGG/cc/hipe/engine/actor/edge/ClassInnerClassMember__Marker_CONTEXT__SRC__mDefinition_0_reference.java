...
public class ClassInnerClassMember__Marker_CONTEXT__SRC__mDefinition_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassInnerClassMember__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassInnerClassMember__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MAbstractMethodDefinition, Set<ModiscoTGG.ClassInnerClassMember__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassInnerClassMember__Marker_CONTEXT__SRC__mDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassInnerClassMember__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassInnerClassMember__Marker source, org.gravity.modisco.MAbstractMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassInnerClassMember__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassInnerClassMember__Marker>();
			pending.put((org.gravity.modisco.MAbstractMethodDefinition) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassInnerClassMember__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassInnerClassMember__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MAbstractMethodDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassInnerClassMember__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassInnerClassMember__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassInnerClassMember__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassInnerClassMember__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassInnerClassMember__Marker, org.gravity.modisco.MAbstractMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassInnerClassMember__Marker, org.gravity.modisco.MAbstractMethodDefinition> message) {
		Set<ModiscoTGG.ClassInnerClassMember__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassInnerClassMember__Marker) {
			leftChanged(message);
		}
		else {
