...
public class ArrayTypeToClass__Marker_CREATE__CORR__eArrayTypeToTArrayClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ArrayTypeToClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ArrayTypeToClass__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.TypeToTAbstractType, Set<ModiscoTGG.ArrayTypeToClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ArrayTypeToClass__Marker_CREATE__CORR__eArrayTypeToTArrayClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ArrayTypeToClass__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ArrayTypeToClass__Marker source, ModiscoTGG.TypeToTAbstractType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ArrayTypeToClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ArrayTypeToClass__Marker>();
			pending.put((ModiscoTGG.TypeToTAbstractType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ArrayTypeToClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ArrayTypeToClass__Marker> message) {
		...
		ModiscoTGG.TypeToTAbstractType target = (ModiscoTGG.TypeToTAbstractType) message.input.getCREATE__CORR__eArrayTypeToTArrayClass();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.TypeToTAbstractType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ArrayTypeToClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ArrayTypeToClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ArrayTypeToClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ArrayTypeToClass__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ArrayTypeToClass__Marker, ModiscoTGG.TypeToTAbstractType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ArrayTypeToClass__Marker, ModiscoTGG.TypeToTAbstractType> message) {
		Set<ModiscoTGG.ArrayTypeToClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ArrayTypeToClass__Marker) {
			leftChanged(message);
		}
		else {
