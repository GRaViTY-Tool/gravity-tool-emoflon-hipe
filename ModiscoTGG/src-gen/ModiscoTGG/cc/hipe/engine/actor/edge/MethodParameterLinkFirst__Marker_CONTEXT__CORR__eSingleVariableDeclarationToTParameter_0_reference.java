...
public class MethodParameterLinkFirst__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MEntryToTParameter, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MEntryToTParameter, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MEntryToTParameter, Set<ModiscoTGG.MethodParameterLinkFirst__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLinkFirst__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLinkFirst__Marker source, ModiscoTGG.MEntryToTParameter target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MEntryToTParameter) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		ModiscoTGG.MEntryToTParameter target = (ModiscoTGG.MEntryToTParameter) message.input.getCONTEXT__CORR__eSingleVariableDeclarationToTParameter();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MEntryToTParameter> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MEntryToTParameter> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLinkFirst__Marker, ModiscoTGG.MEntryToTParameter> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker, ModiscoTGG.MEntryToTParameter> message) {
