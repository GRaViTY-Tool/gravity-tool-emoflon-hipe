...
public class StringLiteralToTTextNode_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.StringLiteralToTTextNode, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.StringLiteralToTTextNode, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TTextNode, Set<ModiscoTGG.StringLiteralToTTextNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public StringLiteralToTTextNode_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueStringArray__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueString__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.StringLiteralToTTextNode source, org.gravity.typegraph.basic.annotations.TTextNode target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.StringLiteralToTTextNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.StringLiteralToTTextNode>();
			pending.put((org.gravity.typegraph.basic.annotations.TTextNode) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.StringLiteralToTTextNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.StringLiteralToTTextNode> message) {
		...
		org.gravity.typegraph.basic.annotations.TTextNode target = (org.gravity.typegraph.basic.annotations.TTextNode) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TTextNode> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.StringLiteralToTTextNode> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.StringLiteralToTTextNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.StringLiteralToTTextNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.StringLiteralToTTextNode> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.StringLiteralToTTextNode, org.gravity.typegraph.basic.annotations.TTextNode> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.StringLiteralToTTextNode, org.gravity.typegraph.basic.annotations.TTextNode> message) {
		Set<ModiscoTGG.StringLiteralToTTextNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.StringLiteralToTTextNode) {
			leftChanged(message);
		}
		else {
