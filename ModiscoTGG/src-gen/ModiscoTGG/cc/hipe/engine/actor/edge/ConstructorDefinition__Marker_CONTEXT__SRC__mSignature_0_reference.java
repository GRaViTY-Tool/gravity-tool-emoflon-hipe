...
public class ConstructorDefinition__Marker_CONTEXT__SRC__mSignature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MMethodSignature, Set<ModiscoTGG.ConstructorDefinition__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ConstructorDefinition__Marker_CONTEXT__SRC__mSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ConstructorDefinition__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ConstructorDefinition__Marker source, org.gravity.modisco.MMethodSignature target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ConstructorDefinition__Marker> message) {
		...
		org.gravity.modisco.MMethodSignature target = (org.gravity.modisco.MMethodSignature) message.input.getCONTEXT__SRC__mSignature();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ConstructorDefinition__Marker, org.gravity.modisco.MMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ConstructorDefinition__Marker, org.gravity.modisco.MMethodSignature> message) {
