...
public class MemberInAnnotationVisibility__Marker_CONTEXT__TRG__tAnnotation_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MemberInAnnotationVisibility__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MemberInAnnotationVisibility__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<ModiscoTGG.MemberInAnnotationVisibility__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MemberInAnnotationVisibility__Marker_CONTEXT__TRG__tAnnotation_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInAnnotationVisibility__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MemberInAnnotationVisibility__Marker source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MemberInAnnotationVisibility__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MemberInAnnotationVisibility__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MemberInAnnotationVisibility__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MemberInAnnotationVisibility__Marker> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotationType target = (org.gravity.typegraph.basic.annotations.TAnnotationType) message.input.getCONTEXT__TRG__tAnnotation();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MemberInAnnotationVisibility__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MemberInAnnotationVisibility__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MemberInAnnotationVisibility__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MemberInAnnotationVisibility__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MemberInAnnotationVisibility__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MemberInAnnotationVisibility__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<ModiscoTGG.MemberInAnnotationVisibility__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MemberInAnnotationVisibility__Marker) {
			leftChanged(message);
		}
		else {
