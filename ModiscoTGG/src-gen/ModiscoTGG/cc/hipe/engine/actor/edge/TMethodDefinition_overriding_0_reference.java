...
public class TMethodDefinition_overriding_0_reference extends AbstractActor {
	...
	private Map<org.gravity.typegraph.basic.TMethodDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.typegraph.basic.TMethodDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.typegraph.basic.TMethodDefinition, Set<org.gravity.typegraph.basic.TMethodDefinition>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TMethodDefinition_overriding_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodOverriding__CC_3378_junction"), this::check_constraint_181));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("MethodOverriding__CONSISTENCY_3393_junction"), this::check_constraint_184));
	}	
	...
	private void addMatch(InputMessage message, org.gravity.typegraph.basic.TMethodDefinition source, org.gravity.typegraph.basic.TMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.typegraph.basic.TMethodDefinition) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<org.gravity.typegraph.basic.TMethodDefinition> message) {
		...
		org.gravity.typegraph.basic.TMethodDefinition target = (org.gravity.typegraph.basic.TMethodDefinition) message.input.getOverriding();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	private void addReference(ReferenceAdded<org.gravity.typegraph.basic.TMethodDefinition, org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<org.gravity.typegraph.basic.TMethodDefinition, org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	public boolean check_constraint_181(EdgeMatch edge) {
	...
	public boolean check_constraint_184(EdgeMatch edge) {
		org.gravity.typegraph.basic.TMethodDefinition tOverriding = (org.gravity.typegraph.basic.TMethodDefinition) edge.source();
		org.gravity.typegraph.basic.TMethodDefinition tMethod = (org.gravity.typegraph.basic.TMethodDefinition) edge.target();
		boolean predicate = !tMethod.equals(tOverriding);
		edge.setConstraintSatisfied(predicate);
