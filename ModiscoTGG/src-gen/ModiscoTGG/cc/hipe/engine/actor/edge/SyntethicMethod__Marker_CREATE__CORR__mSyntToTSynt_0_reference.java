...
public class SyntethicMethod__Marker_CREATE__CORR__mSyntToTSynt_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.SyntethicMethod__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.SyntethicMethod__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, Set<ModiscoTGG.SyntethicMethod__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SyntethicMethod__Marker_CREATE__CORR__mSyntToTSynt_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SyntethicMethod__Marker source, ModiscoTGG.MSyntethicMethodToTSyntethicMethod target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.SyntethicMethod__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.SyntethicMethod__Marker>();
			pending.put((ModiscoTGG.MSyntethicMethodToTSyntethicMethod) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.SyntethicMethod__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SyntethicMethod__Marker> message) {
		...
		ModiscoTGG.MSyntethicMethodToTSyntethicMethod target = (ModiscoTGG.MSyntethicMethodToTSyntethicMethod) message.input.getCREATE__CORR__mSyntToTSynt();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.SyntethicMethod__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.SyntethicMethod__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.SyntethicMethod__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.SyntethicMethod__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SyntethicMethod__Marker, ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SyntethicMethod__Marker, ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		Set<ModiscoTGG.SyntethicMethod__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.SyntethicMethod__Marker) {
			leftChanged(message);
		}
		else {
