...
public class ClassImplementsInterface__Marker_CONTEXT__SRC__eClassDeclaration_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassImplementsInterface__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassImplementsInterface__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.ClassDeclaration, Set<ModiscoTGG.ClassImplementsInterface__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassImplementsInterface__Marker_CONTEXT__SRC__eClassDeclaration_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassImplementsInterface__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassImplementsInterface__Marker source, org.eclipse.modisco.java.ClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassImplementsInterface__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassImplementsInterface__Marker>();
			pending.put((org.eclipse.modisco.java.ClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassImplementsInterface__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassImplementsInterface__Marker> message) {
		...
		org.eclipse.modisco.java.ClassDeclaration target = (org.eclipse.modisco.java.ClassDeclaration) message.input.getCONTEXT__SRC__eClassDeclaration();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.ClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassImplementsInterface__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassImplementsInterface__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassImplementsInterface__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassImplementsInterface__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassImplementsInterface__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassImplementsInterface__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
		Set<ModiscoTGG.ClassImplementsInterface__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassImplementsInterface__Marker) {
			leftChanged(message);
		}
		else {
