...
public class SuperMethodInvocation__Marker_CREATE__SRC__invocation_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MSuperMethodInvocation, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MSuperMethodInvocation, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MSuperMethodInvocation, Set<ModiscoTGG.SuperMethodInvocation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SuperMethodInvocation__Marker_CREATE__SRC__invocation_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SuperMethodInvocation__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SuperMethodInvocation__Marker source, org.gravity.modisco.MSuperMethodInvocation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MSuperMethodInvocation) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SuperMethodInvocation__Marker> message) {
		...
		org.gravity.modisco.MSuperMethodInvocation target = (org.gravity.modisco.MSuperMethodInvocation) message.input.getCREATE__SRC__invocation();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSuperMethodInvocation> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MSuperMethodInvocation> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SuperMethodInvocation__Marker, org.gravity.modisco.MSuperMethodInvocation> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SuperMethodInvocation__Marker, org.gravity.modisco.MSuperMethodInvocation> message) {
