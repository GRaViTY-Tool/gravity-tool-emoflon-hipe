...
public class NestedInterfaceInType__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.NestedInterfaceInType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.NestedInterfaceInType__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.InterfaceDeclarationToTInterface, Set<ModiscoTGG.NestedInterfaceInType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public NestedInterfaceInType__Marker_CREATE__CORR__mInterfaceDeclarationtoTInterface_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("NestedInterfaceInType__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.NestedInterfaceInType__Marker source, ModiscoTGG.InterfaceDeclarationToTInterface target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.NestedInterfaceInType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.NestedInterfaceInType__Marker>();
			pending.put((ModiscoTGG.InterfaceDeclarationToTInterface) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.NestedInterfaceInType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.NestedInterfaceInType__Marker> message) {
		...
		ModiscoTGG.InterfaceDeclarationToTInterface target = (ModiscoTGG.InterfaceDeclarationToTInterface) message.input.getCREATE__CORR__mInterfaceDeclarationtoTInterface();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.NestedInterfaceInType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.NestedInterfaceInType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.NestedInterfaceInType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.NestedInterfaceInType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.NestedInterfaceInType__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.NestedInterfaceInType__Marker, ModiscoTGG.InterfaceDeclarationToTInterface> message) {
		Set<ModiscoTGG.NestedInterfaceInType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.NestedInterfaceInType__Marker) {
			leftChanged(message);
		}
		else {
