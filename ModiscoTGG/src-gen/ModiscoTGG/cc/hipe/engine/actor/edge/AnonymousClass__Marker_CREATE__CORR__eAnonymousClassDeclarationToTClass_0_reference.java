...
public class AnonymousClass__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnonymousClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnonymousClass__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnonymousClassDeclarationToTClass, Set<ModiscoTGG.AnonymousClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnonymousClass__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnonymousClass__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnonymousClass__Marker source, ModiscoTGG.AnonymousClassDeclarationToTClass target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnonymousClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnonymousClass__Marker>();
			pending.put((ModiscoTGG.AnonymousClassDeclarationToTClass) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnonymousClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnonymousClass__Marker> message) {
		...
		ModiscoTGG.AnonymousClassDeclarationToTClass target = (ModiscoTGG.AnonymousClassDeclarationToTClass) message.input.getCREATE__CORR__eAnonymousClassDeclarationToTClass();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnonymousClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnonymousClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnonymousClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnonymousClass__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnonymousClass__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnonymousClass__Marker, ModiscoTGG.AnonymousClassDeclarationToTClass> message) {
		Set<ModiscoTGG.AnonymousClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnonymousClass__Marker) {
			leftChanged(message);
		}
		else {
