...
public class PrimitiveTypesIsVoid__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypesIsVoid__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypesIsVoid__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeVoid, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeVoid, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeVoid, Set<ModiscoTGG.PrimitiveTypesIsVoid__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypesIsVoid__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypesIsVoid__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypesIsVoid__Marker source, org.eclipse.modisco.java.PrimitiveTypeVoid target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypesIsVoid__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypesIsVoid__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeVoid) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypesIsVoid__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypesIsVoid__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeVoid target = (org.eclipse.modisco.java.PrimitiveTypeVoid) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypesIsVoid__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypesIsVoid__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypesIsVoid__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypesIsVoid__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypesIsVoid__Marker, org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypesIsVoid__Marker, org.eclipse.modisco.java.PrimitiveTypeVoid> message) {
		Set<ModiscoTGG.PrimitiveTypesIsVoid__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypesIsVoid__Marker) {
			leftChanged(message);
		}
		else {
