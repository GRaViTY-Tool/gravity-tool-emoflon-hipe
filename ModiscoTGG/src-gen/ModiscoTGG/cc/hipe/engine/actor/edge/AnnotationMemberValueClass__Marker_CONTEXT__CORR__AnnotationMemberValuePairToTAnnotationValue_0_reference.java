...
public class AnnotationMemberValueClass__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationMemberValueClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationMemberValueClass__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue, Set<ModiscoTGG.AnnotationMemberValueClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationMemberValueClass__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueClass__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationMemberValueClass__Marker source, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationMemberValueClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationMemberValueClass__Marker>();
			pending.put((ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationMemberValueClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationMemberValueClass__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationMemberValueClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationMemberValueClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationMemberValueClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationMemberValueClass__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationMemberValueClass__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationMemberValueClass__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		Set<ModiscoTGG.AnnotationMemberValueClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationMemberValueClass__Marker) {
			leftChanged(message);
		}
		else {
