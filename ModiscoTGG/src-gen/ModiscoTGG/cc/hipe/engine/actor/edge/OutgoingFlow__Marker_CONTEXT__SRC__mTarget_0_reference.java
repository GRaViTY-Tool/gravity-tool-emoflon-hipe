...
public class OutgoingFlow__Marker_CONTEXT__SRC__mTarget_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MAbstractFlowElement, Set<ModiscoTGG.OutgoingFlow__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public OutgoingFlow__Marker_CONTEXT__SRC__mTarget_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("OutgoingFlow__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.OutgoingFlow__Marker source, org.gravity.modisco.MAbstractFlowElement target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.OutgoingFlow__Marker> message) {
		...
		org.gravity.modisco.MAbstractFlowElement target = (org.gravity.modisco.MAbstractFlowElement) message.input.getCONTEXT__SRC__mTarget();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.OutgoingFlow__Marker, org.gravity.modisco.MAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.OutgoingFlow__Marker, org.gravity.modisco.MAbstractFlowElement> message) {
