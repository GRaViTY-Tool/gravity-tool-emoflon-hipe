...
public class FieldAccessReadWrite__Marker_CONTEXT__SRC__mField_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFieldDefinition, Set<ModiscoTGG.FieldAccessReadWrite__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldAccessReadWrite__Marker_CONTEXT__SRC__mField_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("FieldAccessReadWrite__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldAccessReadWrite__Marker source, org.gravity.modisco.MFieldDefinition target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldAccessReadWrite__Marker> message) {
		...
		org.gravity.modisco.MFieldDefinition target = (org.gravity.modisco.MFieldDefinition) message.input.getCONTEXT__SRC__mField();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldAccessReadWrite__Marker, org.gravity.modisco.MFieldDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldAccessReadWrite__Marker, org.gravity.modisco.MFieldDefinition> message) {
