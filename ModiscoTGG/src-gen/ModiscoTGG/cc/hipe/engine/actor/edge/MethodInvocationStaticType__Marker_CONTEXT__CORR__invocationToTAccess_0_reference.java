...
public class MethodInvocationStaticType__Marker_CONTEXT__CORR__invocationToTAccess_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodInvocationStaticType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodInvocationStaticType__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.AbstractMethodInvocationToTAccess, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.AbstractMethodInvocationToTAccess, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.AbstractMethodInvocationToTAccess, Set<ModiscoTGG.MethodInvocationStaticType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodInvocationStaticType__Marker_CONTEXT__CORR__invocationToTAccess_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodInvocationStaticType__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodInvocationStaticType__Marker source, ModiscoTGG.AbstractMethodInvocationToTAccess target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodInvocationStaticType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodInvocationStaticType__Marker>();
			pending.put((ModiscoTGG.AbstractMethodInvocationToTAccess) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodInvocationStaticType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodInvocationStaticType__Marker> message) {
		...
		ModiscoTGG.AbstractMethodInvocationToTAccess target = (ModiscoTGG.AbstractMethodInvocationToTAccess) message.input.getCONTEXT__CORR__invocationToTAccess();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AbstractMethodInvocationToTAccess> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodInvocationStaticType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodInvocationStaticType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodInvocationStaticType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodInvocationStaticType__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.AbstractMethodInvocationToTAccess> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodInvocationStaticType__Marker, ModiscoTGG.AbstractMethodInvocationToTAccess> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodInvocationStaticType__Marker, ModiscoTGG.AbstractMethodInvocationToTAccess> message) {
		Set<ModiscoTGG.MethodInvocationStaticType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodInvocationStaticType__Marker) {
			leftChanged(message);
		}
		else {
