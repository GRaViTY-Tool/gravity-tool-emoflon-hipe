...
public class FieldName__Marker_CREATE__CORR__eFieldDeclarationToTField_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.FieldName__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.FieldName__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MFieldNameToTField, Set<ModiscoTGG.FieldName__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldName__Marker_CREATE__CORR__eFieldDeclarationToTField_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("FieldName__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldName__Marker source, ModiscoTGG.MFieldNameToTField target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.FieldName__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.FieldName__Marker>();
			pending.put((ModiscoTGG.MFieldNameToTField) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.FieldName__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldName__Marker> message) {
		...
		ModiscoTGG.MFieldNameToTField target = (ModiscoTGG.MFieldNameToTField) message.input.getCREATE__CORR__eFieldDeclarationToTField();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MFieldNameToTField> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.FieldName__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.FieldName__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.FieldName__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.FieldName__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldName__Marker, ModiscoTGG.MFieldNameToTField> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldName__Marker, ModiscoTGG.MFieldNameToTField> message) {
		Set<ModiscoTGG.FieldName__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.FieldName__Marker) {
			leftChanged(message);
		}
		else {
