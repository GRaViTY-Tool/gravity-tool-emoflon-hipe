...
public class ParameterizedTypeToClass__Marker_CREATE__SRC__eParameterizedType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ParameterizedTypeToClass__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ParameterizedTypeToClass__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.ParameterizedType, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.ParameterizedType, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.ParameterizedType, Set<ModiscoTGG.ParameterizedTypeToClass__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ParameterizedTypeToClass__Marker_CREATE__SRC__eParameterizedType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ParameterizedTypeToClass__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ParameterizedTypeToClass__Marker source, org.eclipse.modisco.java.ParameterizedType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ParameterizedTypeToClass__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ParameterizedTypeToClass__Marker>();
			pending.put((org.eclipse.modisco.java.ParameterizedType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ParameterizedTypeToClass__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ParameterizedTypeToClass__Marker> message) {
		...
		org.eclipse.modisco.java.ParameterizedType target = (org.eclipse.modisco.java.ParameterizedType) message.input.getCREATE__SRC__eParameterizedType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.ParameterizedType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ParameterizedTypeToClass__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ParameterizedTypeToClass__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ParameterizedTypeToClass__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ParameterizedTypeToClass__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.ParameterizedType> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ParameterizedTypeToClass__Marker, org.eclipse.modisco.java.ParameterizedType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ParameterizedTypeToClass__Marker, org.eclipse.modisco.java.ParameterizedType> message) {
		Set<ModiscoTGG.ParameterizedTypeToClass__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ParameterizedTypeToClass__Marker) {
			leftChanged(message);
		}
		else {
