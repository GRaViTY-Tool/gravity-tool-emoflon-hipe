...
public class AnnotationMemberValueString__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationMemberValueString__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationMemberValueString__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue, Set<ModiscoTGG.AnnotationMemberValueString__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationMemberValueString__Marker_CONTEXT__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberValueString__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationMemberValueString__Marker source, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationMemberValueString__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationMemberValueString__Marker>();
			pending.put((ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationMemberValueString__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationMemberValueString__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationMemberValueString__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationMemberValueString__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationMemberValueString__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationMemberValueString__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationMemberValueString__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationMemberValueString__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		Set<ModiscoTGG.AnnotationMemberValueString__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationMemberValueString__Marker) {
			leftChanged(message);
		}
		else {
