...
public class FieldSignature__Marker_CREATE__SRC__mSignature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFieldSignature, Set<ModiscoTGG.FieldSignature__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldSignature__Marker_CREATE__SRC__mSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("FieldSignature__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldSignature__Marker source, org.gravity.modisco.MFieldSignature target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldSignature__Marker> message) {
		...
		org.gravity.modisco.MFieldSignature target = (org.gravity.modisco.MFieldSignature) message.input.getCREATE__SRC__mSignature();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldSignature__Marker, org.gravity.modisco.MFieldSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldSignature__Marker, org.gravity.modisco.MFieldSignature> message) {
