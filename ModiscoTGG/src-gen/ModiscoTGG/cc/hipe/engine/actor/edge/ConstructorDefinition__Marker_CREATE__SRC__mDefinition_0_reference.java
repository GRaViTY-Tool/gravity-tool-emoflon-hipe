...
public class ConstructorDefinition__Marker_CREATE__SRC__mDefinition_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MConstructorDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MConstructorDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MConstructorDefinition, Set<ModiscoTGG.ConstructorDefinition__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ConstructorDefinition__Marker_CREATE__SRC__mDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ConstructorDefinition__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ConstructorDefinition__Marker source, org.gravity.modisco.MConstructorDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MConstructorDefinition) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ConstructorDefinition__Marker> message) {
		...
		org.gravity.modisco.MConstructorDefinition target = (org.gravity.modisco.MConstructorDefinition) message.input.getCREATE__SRC__mDefinition();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MConstructorDefinition> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MConstructorDefinition> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ConstructorDefinition__Marker, org.gravity.modisco.MConstructorDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ConstructorDefinition__Marker, org.gravity.modisco.MConstructorDefinition> message) {
