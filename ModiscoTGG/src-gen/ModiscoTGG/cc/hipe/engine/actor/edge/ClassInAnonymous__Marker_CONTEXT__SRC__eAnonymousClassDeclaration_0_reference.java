...
public class ClassInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassInAnonymous__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassInAnonymous__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AnonymousClassDeclaration, Set<ModiscoTGG.ClassInAnonymous__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassInAnonymous__Marker_CONTEXT__SRC__eAnonymousClassDeclaration_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassInAnonymous__CONSISTENCY_triangle_7_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassInAnonymous__Marker source, org.eclipse.modisco.java.AnonymousClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassInAnonymous__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassInAnonymous__Marker>();
			pending.put((org.eclipse.modisco.java.AnonymousClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassInAnonymous__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassInAnonymous__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassInAnonymous__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassInAnonymous__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassInAnonymous__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassInAnonymous__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassInAnonymous__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		Set<ModiscoTGG.ClassInAnonymous__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassInAnonymous__Marker) {
			leftChanged(message);
		}
		else {
