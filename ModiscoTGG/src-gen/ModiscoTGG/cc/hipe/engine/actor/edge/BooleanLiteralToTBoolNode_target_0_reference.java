...
public class BooleanLiteralToTBoolNode_target_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.BooleanLiteralToTBoolNode, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.BooleanLiteralToTBoolNode, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TBoolNode, Set<ModiscoTGG.BooleanLiteralToTBoolNode>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public BooleanLiteralToTBoolNode_target_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueBooleanArray__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("AnnotationMemberValueBoolean__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.BooleanLiteralToTBoolNode source, org.gravity.typegraph.basic.annotations.TBoolNode target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.BooleanLiteralToTBoolNode> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.BooleanLiteralToTBoolNode>();
			pending.put((org.gravity.typegraph.basic.annotations.TBoolNode) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.BooleanLiteralToTBoolNode) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.BooleanLiteralToTBoolNode> message) {
		...
		org.gravity.typegraph.basic.annotations.TBoolNode target = (org.gravity.typegraph.basic.annotations.TBoolNode) message.input.getTarget();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TBoolNode> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.BooleanLiteralToTBoolNode> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.BooleanLiteralToTBoolNode source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.BooleanLiteralToTBoolNode> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.BooleanLiteralToTBoolNode> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.BooleanLiteralToTBoolNode, org.gravity.typegraph.basic.annotations.TBoolNode> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.BooleanLiteralToTBoolNode, org.gravity.typegraph.basic.annotations.TBoolNode> message) {
		Set<ModiscoTGG.BooleanLiteralToTBoolNode> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.BooleanLiteralToTBoolNode) {
			leftChanged(message);
		}
		else {
