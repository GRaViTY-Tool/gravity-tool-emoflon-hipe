...
public class MemberInClassVisibility__Marker_CONTEXT__SRC__mClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MemberInClassVisibility__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MemberInClassVisibility__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.ClassDeclaration, Set<ModiscoTGG.MemberInClassVisibility__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MemberInClassVisibility__Marker_CONTEXT__SRC__mClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInClassVisibility__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MemberInClassVisibility__Marker source, org.eclipse.modisco.java.ClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MemberInClassVisibility__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MemberInClassVisibility__Marker>();
			pending.put((org.eclipse.modisco.java.ClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MemberInClassVisibility__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MemberInClassVisibility__Marker> message) {
		...
		org.eclipse.modisco.java.ClassDeclaration target = (org.eclipse.modisco.java.ClassDeclaration) message.input.getCONTEXT__SRC__mClass();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.ClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MemberInClassVisibility__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MemberInClassVisibility__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MemberInClassVisibility__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MemberInClassVisibility__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MemberInClassVisibility__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MemberInClassVisibility__Marker, org.eclipse.modisco.java.ClassDeclaration> message) {
		Set<ModiscoTGG.MemberInClassVisibility__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MemberInClassVisibility__Marker) {
			leftChanged(message);
		}
		else {
