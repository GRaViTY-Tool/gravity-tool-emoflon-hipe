...
public class SuperConstructorInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.SuperConstructorInvocation__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.SuperConstructorInvocation__Marker, Set<EdgeMatch>>();
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement, Set<ModiscoTGG.SuperConstructorInvocation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SuperConstructorInvocation__Marker_CREATE__CORR__mFlowElementToTFlowElement_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_4767_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SuperConstructorInvocation__Marker source, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.SuperConstructorInvocation__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.SuperConstructorInvocation__Marker>();
			pending.put((ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.SuperConstructorInvocation__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SuperConstructorInvocation__Marker> message) {
		...
		ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement target = (ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement) message.input.getCREATE__CORR__mFlowElementToTFlowElement();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.SuperConstructorInvocation__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.SuperConstructorInvocation__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.SuperConstructorInvocation__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.SuperConstructorInvocation__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SuperConstructorInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SuperConstructorInvocation__Marker, ModiscoTGG.MAbstractFlowElementToTAbstractFlowElement> message) {
		Set<ModiscoTGG.SuperConstructorInvocation__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.SuperConstructorInvocation__Marker) {
			leftChanged(message);
		}
		else {
