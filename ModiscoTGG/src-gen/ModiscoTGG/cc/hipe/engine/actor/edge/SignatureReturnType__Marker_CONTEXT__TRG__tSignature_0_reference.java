...
public class SignatureReturnType__Marker_CONTEXT__TRG__tSignature_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.SignatureReturnType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.SignatureReturnType__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMethodSignature, Set<ModiscoTGG.SignatureReturnType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SignatureReturnType__Marker_CONTEXT__TRG__tSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SignatureReturnType__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SignatureReturnType__Marker source, org.gravity.typegraph.basic.TMethodSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.SignatureReturnType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.SignatureReturnType__Marker>();
			pending.put((org.gravity.typegraph.basic.TMethodSignature) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.SignatureReturnType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SignatureReturnType__Marker> message) {
		...
		org.gravity.typegraph.basic.TMethodSignature target = (org.gravity.typegraph.basic.TMethodSignature) message.input.getCONTEXT__TRG__tSignature();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodSignature> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.SignatureReturnType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.SignatureReturnType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.SignatureReturnType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.SignatureReturnType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SignatureReturnType__Marker, org.gravity.typegraph.basic.TMethodSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SignatureReturnType__Marker, org.gravity.typegraph.basic.TMethodSignature> message) {
		Set<ModiscoTGG.SignatureReturnType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.SignatureReturnType__Marker) {
			leftChanged(message);
		}
		else {
