...
public class MethodParameterLinkFirst__Marker_CONTEXT__CORR__mmethodSignatureToTMethodSignature_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MSignatureToTSignature, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MSignatureToTSignature, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MSignatureToTSignature, Set<ModiscoTGG.MethodParameterLinkFirst__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLinkFirst__Marker_CONTEXT__CORR__mmethodSignatureToTMethodSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLinkFirst__Marker source, ModiscoTGG.MSignatureToTSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MSignatureToTSignature) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		ModiscoTGG.MSignatureToTSignature target = (ModiscoTGG.MSignatureToTSignature) message.input.getCONTEXT__CORR__mmethodSignatureToTMethodSignature();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MSignatureToTSignature> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MSignatureToTSignature> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLinkFirst__Marker, ModiscoTGG.MSignatureToTSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker, ModiscoTGG.MSignatureToTSignature> message) {
