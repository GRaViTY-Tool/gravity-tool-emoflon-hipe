...
public class MethodParameterLink__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MethodParameterLink__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MethodParameterLink__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.MEntryToTParameter, Set<ModiscoTGG.MethodParameterLink__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLink__Marker_CONTEXT__CORR__eSingleVariableDeclarationToTParameter_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodParameterLink__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLink__Marker source, ModiscoTGG.MEntryToTParameter target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MethodParameterLink__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MethodParameterLink__Marker>();
			pending.put((ModiscoTGG.MEntryToTParameter) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MethodParameterLink__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLink__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MEntryToTParameter> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MethodParameterLink__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MethodParameterLink__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MethodParameterLink__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MethodParameterLink__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLink__Marker, ModiscoTGG.MEntryToTParameter> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLink__Marker, ModiscoTGG.MEntryToTParameter> message) {
		Set<ModiscoTGG.MethodParameterLink__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MethodParameterLink__Marker) {
			leftChanged(message);
		}
		else {
