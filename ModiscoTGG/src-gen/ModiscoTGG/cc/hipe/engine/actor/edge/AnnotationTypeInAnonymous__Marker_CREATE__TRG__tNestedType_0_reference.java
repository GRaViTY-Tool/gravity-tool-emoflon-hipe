...
public class AnnotationTypeInAnonymous__Marker_CREATE__TRG__tNestedType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationTypeInAnonymous__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationTypeInAnonymous__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<ModiscoTGG.AnnotationTypeInAnonymous__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationTypeInAnonymous__Marker_CREATE__TRG__tNestedType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationTypeInAnonymous__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationTypeInAnonymous__Marker source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationTypeInAnonymous__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationTypeInAnonymous__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationTypeInAnonymous__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationTypeInAnonymous__Marker> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotationType target = (org.gravity.typegraph.basic.annotations.TAnnotationType) message.input.getCREATE__TRG__tNestedType();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationTypeInAnonymous__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationTypeInAnonymous__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationTypeInAnonymous__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationTypeInAnonymous__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationTypeInAnonymous__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationTypeInAnonymous__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<ModiscoTGG.AnnotationTypeInAnonymous__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationTypeInAnonymous__Marker) {
			leftChanged(message);
		}
		else {
