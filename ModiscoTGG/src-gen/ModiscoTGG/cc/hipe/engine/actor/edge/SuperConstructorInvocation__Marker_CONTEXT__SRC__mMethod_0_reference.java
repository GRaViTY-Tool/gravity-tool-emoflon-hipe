...
public class SuperConstructorInvocation__Marker_CONTEXT__SRC__mMethod_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MAbstractMethodDefinition, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MAbstractMethodDefinition, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MAbstractMethodDefinition, Set<ModiscoTGG.SuperConstructorInvocation__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SuperConstructorInvocation__Marker_CONTEXT__SRC__mMethod_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("SuperConstructorInvocation__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SuperConstructorInvocation__Marker source, org.gravity.modisco.MAbstractMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MAbstractMethodDefinition) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SuperConstructorInvocation__Marker> message) {
		...
		org.gravity.modisco.MAbstractMethodDefinition target = (org.gravity.modisco.MAbstractMethodDefinition) message.input.getCONTEXT__SRC__mMethod();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MAbstractMethodDefinition> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MAbstractMethodDefinition> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SuperConstructorInvocation__Marker, org.gravity.modisco.MAbstractMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SuperConstructorInvocation__Marker, org.gravity.modisco.MAbstractMethodDefinition> message) {
