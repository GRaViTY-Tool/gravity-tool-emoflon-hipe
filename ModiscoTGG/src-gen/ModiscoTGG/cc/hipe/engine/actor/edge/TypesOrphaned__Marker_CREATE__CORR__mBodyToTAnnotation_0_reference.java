...
public class TypesOrphaned__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.TypesOrphaned__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.TypesOrphaned__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.ASTNodeToTAnnotatable, Set<ModiscoTGG.TypesOrphaned__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public TypesOrphaned__Marker_CREATE__CORR__mBodyToTAnnotation_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("TypesOrphaned__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.TypesOrphaned__Marker source, ModiscoTGG.ASTNodeToTAnnotatable target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.TypesOrphaned__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.TypesOrphaned__Marker>();
			pending.put((ModiscoTGG.ASTNodeToTAnnotatable) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.TypesOrphaned__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.TypesOrphaned__Marker> message) {
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.ASTNodeToTAnnotatable> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.TypesOrphaned__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.TypesOrphaned__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.TypesOrphaned__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.TypesOrphaned__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.TypesOrphaned__Marker, ModiscoTGG.ASTNodeToTAnnotatable> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.TypesOrphaned__Marker, ModiscoTGG.ASTNodeToTAnnotatable> message) {
		Set<ModiscoTGG.TypesOrphaned__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.TypesOrphaned__Marker) {
			leftChanged(message);
		}
		else {
