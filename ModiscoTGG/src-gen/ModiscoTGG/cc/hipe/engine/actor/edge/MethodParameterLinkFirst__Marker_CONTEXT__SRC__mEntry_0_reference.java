...
public class MethodParameterLinkFirst__Marker_CONTEXT__SRC__mEntry_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MEntry, Set<ModiscoTGG.MethodParameterLinkFirst__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodParameterLinkFirst__Marker_CONTEXT__SRC__mEntry_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MethodParameterLinkFirst__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodParameterLinkFirst__Marker source, org.gravity.modisco.MEntry target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodParameterLinkFirst__Marker> message) {
		...
		org.gravity.modisco.MEntry target = (org.gravity.modisco.MEntry) message.input.getCONTEXT__SRC__mEntry();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.modisco.MEntry> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodParameterLinkFirst__Marker, org.gravity.modisco.MEntry> message) {
