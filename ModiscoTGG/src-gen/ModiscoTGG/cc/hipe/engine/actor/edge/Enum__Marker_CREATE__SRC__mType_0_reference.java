...
public class Enum__Marker_CREATE__SRC__mType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.Enum__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.Enum__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.EnumDeclaration, Set<ModiscoTGG.Enum__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Enum__Marker_CREATE__SRC__mType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("Enum__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.Enum__Marker source, org.eclipse.modisco.java.EnumDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.Enum__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.Enum__Marker>();
			pending.put((org.eclipse.modisco.java.EnumDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.Enum__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.Enum__Marker> message) {
		...
		org.eclipse.modisco.java.EnumDeclaration target = (org.eclipse.modisco.java.EnumDeclaration) message.input.getCREATE__SRC__mType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.EnumDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.Enum__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.Enum__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.Enum__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.Enum__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.Enum__Marker, org.eclipse.modisco.java.EnumDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.Enum__Marker, org.eclipse.modisco.java.EnumDeclaration> message) {
		Set<ModiscoTGG.Enum__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.Enum__Marker) {
			leftChanged(message);
		}
		else {
