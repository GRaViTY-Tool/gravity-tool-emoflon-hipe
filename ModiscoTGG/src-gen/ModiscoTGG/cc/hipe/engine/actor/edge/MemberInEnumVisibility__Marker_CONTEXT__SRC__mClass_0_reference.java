...
public class MemberInEnumVisibility__Marker_CONTEXT__SRC__mClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MemberInEnumVisibility__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MemberInEnumVisibility__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.EnumDeclaration, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.EnumDeclaration, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.EnumDeclaration, Set<ModiscoTGG.MemberInEnumVisibility__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MemberInEnumVisibility__Marker_CONTEXT__SRC__mClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("MemberInEnumVisibility__CONSISTENCY_triangle_8_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MemberInEnumVisibility__Marker source, org.eclipse.modisco.java.EnumDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MemberInEnumVisibility__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MemberInEnumVisibility__Marker>();
			pending.put((org.eclipse.modisco.java.EnumDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MemberInEnumVisibility__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MemberInEnumVisibility__Marker> message) {
		...
		org.eclipse.modisco.java.EnumDeclaration target = (org.eclipse.modisco.java.EnumDeclaration) message.input.getCONTEXT__SRC__mClass();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.EnumDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MemberInEnumVisibility__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MemberInEnumVisibility__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MemberInEnumVisibility__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MemberInEnumVisibility__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.EnumDeclaration> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MemberInEnumVisibility__Marker, org.eclipse.modisco.java.EnumDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MemberInEnumVisibility__Marker, org.eclipse.modisco.java.EnumDeclaration> message) {
		Set<ModiscoTGG.MemberInEnumVisibility__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MemberInEnumVisibility__Marker) {
			leftChanged(message);
		}
		else {
