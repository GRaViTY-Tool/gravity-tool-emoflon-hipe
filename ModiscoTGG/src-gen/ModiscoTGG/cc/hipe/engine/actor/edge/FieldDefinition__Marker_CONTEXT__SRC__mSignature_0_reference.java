...
public class FieldDefinition__Marker_CONTEXT__SRC__mSignature_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MFieldSignature, Set<EdgeMatch>> targetElements = new HashMap<org.gravity.modisco.MFieldSignature, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.gravity.modisco.MFieldSignature, Set<ModiscoTGG.FieldDefinition__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public FieldDefinition__Marker_CONTEXT__SRC__mSignature_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("FieldDefinition__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.FieldDefinition__Marker source, org.gravity.modisco.MFieldSignature target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((org.gravity.modisco.MFieldSignature) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.FieldDefinition__Marker> message) {
		...
		org.gravity.modisco.MFieldSignature target = (org.gravity.modisco.MFieldSignature) message.input.getCONTEXT__SRC__mSignature();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MFieldSignature> message) {
	...
	private void rightRemoved(NodeDeletedRight<org.gravity.modisco.MFieldSignature> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.FieldDefinition__Marker, org.gravity.modisco.MFieldSignature> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.FieldDefinition__Marker, org.gravity.modisco.MFieldSignature> message) {
