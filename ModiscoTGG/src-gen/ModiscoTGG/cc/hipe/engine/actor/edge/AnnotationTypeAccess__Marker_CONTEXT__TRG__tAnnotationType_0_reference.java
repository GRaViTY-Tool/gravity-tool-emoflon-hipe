...
public class AnnotationTypeAccess__Marker_CONTEXT__TRG__tAnnotationType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationTypeAccess__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationTypeAccess__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotationType, Set<ModiscoTGG.AnnotationTypeAccess__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationTypeAccess__Marker_CONTEXT__TRG__tAnnotationType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationTypeAccess__CONSISTENCY_triangle_1_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationTypeAccess__Marker source, org.gravity.typegraph.basic.annotations.TAnnotationType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationTypeAccess__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationTypeAccess__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotationType) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationTypeAccess__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationTypeAccess__Marker> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotationType target = (org.gravity.typegraph.basic.annotations.TAnnotationType) message.input.getCONTEXT__TRG__tAnnotationType();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationTypeAccess__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationTypeAccess__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationTypeAccess__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationTypeAccess__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationTypeAccess__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationTypeAccess__Marker, org.gravity.typegraph.basic.annotations.TAnnotationType> message) {
		Set<ModiscoTGG.AnnotationTypeAccess__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationTypeAccess__Marker) {
			leftChanged(message);
		}
		else {
