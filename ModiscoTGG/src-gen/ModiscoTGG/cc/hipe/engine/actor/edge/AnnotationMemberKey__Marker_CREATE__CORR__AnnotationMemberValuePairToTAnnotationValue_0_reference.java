...
public class AnnotationMemberKey__Marker_CREATE__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationMemberKey__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationMemberKey__Marker, Set<EdgeMatch>>();
	...
	private Map<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue, Set<ModiscoTGG.AnnotationMemberKey__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationMemberKey__Marker_CREATE__CORR__AnnotationMemberValuePairToTAnnotationValue_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("AnnotationMemberKey__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationMemberKey__Marker source, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationMemberKey__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationMemberKey__Marker>();
			pending.put((ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationMemberKey__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationMemberKey__Marker> message) {
		...
		ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue target = (ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue) message.input.getCREATE__CORR__AnnotationMemberValuePairToTAnnotationValue();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationMemberKey__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationMemberKey__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationMemberKey__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationMemberKey__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationMemberKey__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationMemberKey__Marker, ModiscoTGG.AnnotationMemberValuePairToTAnnotationValue> message) {
		Set<ModiscoTGG.AnnotationMemberKey__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationMemberKey__Marker) {
			leftChanged(message);
		}
		else {
