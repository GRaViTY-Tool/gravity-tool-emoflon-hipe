...
public class MSyntethicMethodToTSyntethicMethod_source_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, Set<EdgeMatch>>();
	...
	private Map<org.gravity.modisco.MSyntheticMethodDefinition, Set<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MSyntethicMethodToTSyntethicMethod_source_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("SyntethicMethod__CONSISTENCY_triangle_9_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MSyntethicMethodToTSyntethicMethod source, org.gravity.modisco.MSyntheticMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.MSyntethicMethodToTSyntethicMethod>();
			pending.put((org.gravity.modisco.MSyntheticMethodDefinition) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.MSyntethicMethodToTSyntethicMethod) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		...
		org.gravity.modisco.MSyntheticMethodDefinition target = (org.gravity.modisco.MSyntheticMethodDefinition) message.input.getSource();
	...
	private void rightAdded(NodeAddedRight<org.gravity.modisco.MSyntheticMethodDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.MSyntethicMethodToTSyntethicMethod source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, org.gravity.modisco.MSyntheticMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MSyntethicMethodToTSyntethicMethod, org.gravity.modisco.MSyntheticMethodDefinition> message) {
		Set<ModiscoTGG.MSyntethicMethodToTSyntethicMethod> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.MSyntethicMethodToTSyntethicMethod) {
			leftChanged(message);
		}
		else {
