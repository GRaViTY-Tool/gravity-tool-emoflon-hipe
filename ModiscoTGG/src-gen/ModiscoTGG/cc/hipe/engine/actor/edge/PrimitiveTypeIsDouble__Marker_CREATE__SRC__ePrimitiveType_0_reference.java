...
public class PrimitiveTypeIsDouble__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeIsDouble__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeIsDouble__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeDouble, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeDouble, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeDouble, Set<ModiscoTGG.PrimitiveTypeIsDouble__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeIsDouble__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypeIsDouble__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeIsDouble__Marker source, org.eclipse.modisco.java.PrimitiveTypeDouble target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeIsDouble__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeIsDouble__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeDouble) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeIsDouble__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeIsDouble__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeDouble target = (org.eclipse.modisco.java.PrimitiveTypeDouble) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeDouble> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeIsDouble__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeIsDouble__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeIsDouble__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeIsDouble__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeDouble> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeIsDouble__Marker, org.eclipse.modisco.java.PrimitiveTypeDouble> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeIsDouble__Marker, org.eclipse.modisco.java.PrimitiveTypeDouble> message) {
		Set<ModiscoTGG.PrimitiveTypeIsDouble__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeIsDouble__Marker) {
			leftChanged(message);
		}
		else {
