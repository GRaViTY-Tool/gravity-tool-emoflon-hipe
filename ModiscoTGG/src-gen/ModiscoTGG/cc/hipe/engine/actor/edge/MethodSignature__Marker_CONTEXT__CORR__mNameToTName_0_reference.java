...
public class MethodSignature__Marker_CONTEXT__CORR__mNameToTName_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.MMethodNameToTMethod, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.MMethodNameToTMethod, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.MMethodNameToTMethod, Set<ModiscoTGG.MethodSignature__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public MethodSignature__Marker_CONTEXT__CORR__mNameToTName_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("MethodSignature__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.MethodSignature__Marker source, ModiscoTGG.MMethodNameToTMethod target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.MMethodNameToTMethod) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.MethodSignature__Marker> message) {
		...
		ModiscoTGG.MMethodNameToTMethod target = (ModiscoTGG.MMethodNameToTMethod) message.input.getCONTEXT__CORR__mNameToTName();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.MMethodNameToTMethod> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.MMethodNameToTMethod> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.MethodSignature__Marker, ModiscoTGG.MMethodNameToTMethod> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.MethodSignature__Marker, ModiscoTGG.MMethodNameToTMethod> message) {
