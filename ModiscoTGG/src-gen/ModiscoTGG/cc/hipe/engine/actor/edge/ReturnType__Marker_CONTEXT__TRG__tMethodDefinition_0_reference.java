...
public class ReturnType__Marker_CONTEXT__TRG__tMethodDefinition_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ReturnType__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ReturnType__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.TMethodDefinition, Set<ModiscoTGG.ReturnType__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ReturnType__Marker_CONTEXT__TRG__tMethodDefinition_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ReturnType__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ReturnType__Marker source, org.gravity.typegraph.basic.TMethodDefinition target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ReturnType__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ReturnType__Marker>();
			pending.put((org.gravity.typegraph.basic.TMethodDefinition) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ReturnType__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ReturnType__Marker> message) {
		...
		org.gravity.typegraph.basic.TMethodDefinition target = (org.gravity.typegraph.basic.TMethodDefinition) message.input.getCONTEXT__TRG__tMethodDefinition();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.TMethodDefinition> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ReturnType__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ReturnType__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ReturnType__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ReturnType__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ReturnType__Marker, org.gravity.typegraph.basic.TMethodDefinition> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ReturnType__Marker, org.gravity.typegraph.basic.TMethodDefinition> message) {
		Set<ModiscoTGG.ReturnType__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ReturnType__Marker) {
			leftChanged(message);
		}
		else {
