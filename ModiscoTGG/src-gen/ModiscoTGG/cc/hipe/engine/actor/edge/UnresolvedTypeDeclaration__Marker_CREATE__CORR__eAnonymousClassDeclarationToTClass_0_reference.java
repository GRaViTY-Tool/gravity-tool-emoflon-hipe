...
public class UnresolvedTypeDeclaration__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.TypeToTAbstractType, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.TypeToTAbstractType, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.TypeToTAbstractType, Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public UnresolvedTypeDeclaration__Marker_CREATE__CORR__eAnonymousClassDeclarationToTClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("UnresolvedTypeDeclaration__CONSISTENCY_triangle_3_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.UnresolvedTypeDeclaration__Marker source, ModiscoTGG.TypeToTAbstractType target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.TypeToTAbstractType) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		...
		ModiscoTGG.TypeToTAbstractType target = (ModiscoTGG.TypeToTAbstractType) message.input.getCREATE__CORR__eAnonymousClassDeclarationToTClass();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.TypeToTAbstractType> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.TypeToTAbstractType> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.UnresolvedTypeDeclaration__Marker, ModiscoTGG.TypeToTAbstractType> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.UnresolvedTypeDeclaration__Marker, ModiscoTGG.TypeToTAbstractType> message) {
