...
public class SubPackageToPGPackage__Marker_CONTEXT__CORR__mParentPackageToTParent_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.PackageToTPackage, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.PackageToTPackage, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.PackageToTPackage, Set<ModiscoTGG.SubPackageToPGPackage__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public SubPackageToPGPackage__Marker_CONTEXT__CORR__mParentPackageToTParent_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("SubPackageToPGPackage__CONSISTENCY_triangle_6_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.SubPackageToPGPackage__Marker source, ModiscoTGG.PackageToTPackage target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.PackageToTPackage) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.SubPackageToPGPackage__Marker> message) {
		...
		ModiscoTGG.PackageToTPackage target = (ModiscoTGG.PackageToTPackage) message.input.getCONTEXT__CORR__mParentPackageToTParent();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.PackageToTPackage> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.PackageToTPackage> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.SubPackageToPGPackage__Marker, ModiscoTGG.PackageToTPackage> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.SubPackageToPGPackage__Marker, ModiscoTGG.PackageToTPackage> message) {
