...
public class PrimitiveTypeIsFloat__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeIsFloat__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeIsFloat__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeFloat, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeFloat, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeFloat, Set<ModiscoTGG.PrimitiveTypeIsFloat__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeIsFloat__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypeIsFloat__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeIsFloat__Marker source, org.eclipse.modisco.java.PrimitiveTypeFloat target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeIsFloat__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeIsFloat__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeFloat) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeIsFloat__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeIsFloat__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeFloat target = (org.eclipse.modisco.java.PrimitiveTypeFloat) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeFloat> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeIsFloat__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeIsFloat__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeIsFloat__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeIsFloat__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeFloat> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeIsFloat__Marker, org.eclipse.modisco.java.PrimitiveTypeFloat> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeIsFloat__Marker, org.eclipse.modisco.java.PrimitiveTypeFloat> message) {
		Set<ModiscoTGG.PrimitiveTypeIsFloat__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeIsFloat__Marker) {
			leftChanged(message);
		}
		else {
