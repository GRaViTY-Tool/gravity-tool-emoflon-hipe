...
public class ArrayInitializerIgnoreExpressionsEdge__Marker_CONTEXT__SRC__invocation_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.MethodInvocation, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.MethodInvocation, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.MethodInvocation, Set<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ArrayInitializerIgnoreExpressionsEdge__Marker_CONTEXT__SRC__invocation_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("ArrayInitializerIgnoreExpressionsEdge__CONSISTENCY_triangle_0_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker source, org.eclipse.modisco.java.MethodInvocation target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker>();
			pending.put((org.eclipse.modisco.java.MethodInvocation) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> message) {
		...
		org.eclipse.modisco.java.MethodInvocation target = (org.eclipse.modisco.java.MethodInvocation) message.input.getCONTEXT__SRC__invocation();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.MethodInvocation> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.MethodInvocation> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker, org.eclipse.modisco.java.MethodInvocation> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker, org.eclipse.modisco.java.MethodInvocation> message) {
		Set<ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ArrayInitializerIgnoreExpressionsEdge__Marker) {
			leftChanged(message);
		}
		else {
