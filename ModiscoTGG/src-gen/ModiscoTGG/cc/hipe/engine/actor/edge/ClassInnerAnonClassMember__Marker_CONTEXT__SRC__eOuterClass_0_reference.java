...
public class ClassInnerAnonClassMember__Marker_CONTEXT__SRC__eOuterClass_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.ClassInnerAnonClassMember__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.ClassInnerAnonClassMember__Marker, Set<EdgeMatch>>();
	...
	private Map<org.eclipse.modisco.java.AnonymousClassDeclaration, Set<ModiscoTGG.ClassInnerAnonClassMember__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public ClassInnerAnonClassMember__Marker_CONTEXT__SRC__eOuterClass_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("ClassInnerAnonClassMember__CONSISTENCY_triangle_12_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.ClassInnerAnonClassMember__Marker source, org.eclipse.modisco.java.AnonymousClassDeclaration target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.ClassInnerAnonClassMember__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.ClassInnerAnonClassMember__Marker>();
			pending.put((org.eclipse.modisco.java.AnonymousClassDeclaration) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.ClassInnerAnonClassMember__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.ClassInnerAnonClassMember__Marker> message) {
		...
		org.eclipse.modisco.java.AnonymousClassDeclaration target = (org.eclipse.modisco.java.AnonymousClassDeclaration) message.input.getCONTEXT__SRC__eOuterClass();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.ClassInnerAnonClassMember__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.ClassInnerAnonClassMember__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.ClassInnerAnonClassMember__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.ClassInnerAnonClassMember__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.ClassInnerAnonClassMember__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.ClassInnerAnonClassMember__Marker, org.eclipse.modisco.java.AnonymousClassDeclaration> message) {
		Set<ModiscoTGG.ClassInnerAnonClassMember__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.ClassInnerAnonClassMember__Marker) {
			leftChanged(message);
		}
		else {
