...
public class AnnotationOnBodyDeclaration__Marker_CONTEXT__TRG__tAnnotable_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.AnnotationOnBodyDeclaration__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.AnnotationOnBodyDeclaration__Marker, Set<EdgeMatch>>();
	...
	private Map<org.gravity.typegraph.basic.annotations.TAnnotatable, Set<ModiscoTGG.AnnotationOnBodyDeclaration__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public AnnotationOnBodyDeclaration__Marker_CONTEXT__TRG__tAnnotable_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("AnnotationOnBodyDeclaration__CONSISTENCY_triangle_2_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.AnnotationOnBodyDeclaration__Marker source, org.gravity.typegraph.basic.annotations.TAnnotatable target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.AnnotationOnBodyDeclaration__Marker>();
			pending.put((org.gravity.typegraph.basic.annotations.TAnnotatable) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.AnnotationOnBodyDeclaration__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> message) {
		...
		org.gravity.typegraph.basic.annotations.TAnnotatable target = (org.gravity.typegraph.basic.annotations.TAnnotatable) message.input.getCONTEXT__TRG__tAnnotable();
	...
	private void rightAdded(NodeAddedRight<org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.AnnotationOnBodyDeclaration__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> sourcePending = pending.get(match.target());
	...
	private void addReference(ReferenceAdded<ModiscoTGG.AnnotationOnBodyDeclaration__Marker, org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.AnnotationOnBodyDeclaration__Marker, org.gravity.typegraph.basic.annotations.TAnnotatable> message) {
		Set<ModiscoTGG.AnnotationOnBodyDeclaration__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.AnnotationOnBodyDeclaration__Marker) {
			leftChanged(message);
		}
		else {
