...
public class PrimitiveTypeIsByte__Marker_CREATE__SRC__ePrimitiveType_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<ModiscoTGG.PrimitiveTypeIsByte__Marker, Set<EdgeMatch>> sourceElements = new HashMap<ModiscoTGG.PrimitiveTypeIsByte__Marker, Set<EdgeMatch>>();
	private Map<org.eclipse.modisco.java.PrimitiveTypeByte, Set<EdgeMatch>> targetElements = new HashMap<org.eclipse.modisco.java.PrimitiveTypeByte, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<org.eclipse.modisco.java.PrimitiveTypeByte, Set<ModiscoTGG.PrimitiveTypeIsByte__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public PrimitiveTypeIsByte__Marker_CREATE__SRC__ePrimitiveType_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("PrimitiveTypeIsByte__CONSISTENCY_triangle_5_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.PrimitiveTypeIsByte__Marker source, org.eclipse.modisco.java.PrimitiveTypeByte target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<ModiscoTGG.PrimitiveTypeIsByte__Marker> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<ModiscoTGG.PrimitiveTypeIsByte__Marker>();
			pending.put((org.eclipse.modisco.java.PrimitiveTypeByte) target, sourcePending);
		}
		sourcePending.add((ModiscoTGG.PrimitiveTypeIsByte__Marker) source);
	}
	
	private void leftAdded(NodeAddedLeft<ModiscoTGG.PrimitiveTypeIsByte__Marker> message) {
		...
		org.eclipse.modisco.java.PrimitiveTypeByte target = (org.eclipse.modisco.java.PrimitiveTypeByte) message.input.getCREATE__SRC__ePrimitiveType();
	...
	private void rightAdded(NodeAddedRight<org.eclipse.modisco.java.PrimitiveTypeByte> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<ModiscoTGG.PrimitiveTypeIsByte__Marker> pendingsSources = pending.get(message.input);
			for(ModiscoTGG.PrimitiveTypeIsByte__Marker source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<ModiscoTGG.PrimitiveTypeIsByte__Marker> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<ModiscoTGG.PrimitiveTypeIsByte__Marker> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<org.eclipse.modisco.java.PrimitiveTypeByte> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.PrimitiveTypeIsByte__Marker, org.eclipse.modisco.java.PrimitiveTypeByte> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.PrimitiveTypeIsByte__Marker, org.eclipse.modisco.java.PrimitiveTypeByte> message) {
		Set<ModiscoTGG.PrimitiveTypeIsByte__Marker> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof ModiscoTGG.PrimitiveTypeIsByte__Marker) {
			leftChanged(message);
		}
		else {
