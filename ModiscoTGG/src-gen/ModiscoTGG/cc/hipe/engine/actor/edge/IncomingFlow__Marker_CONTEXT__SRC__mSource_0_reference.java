...
public class IncomingFlow__Marker_CONTEXT__SRC__mSource_0_reference extends AbstractActor {
	...
	private Map<org.gravity.modisco.MAbstractFlowElement, Set<ModiscoTGG.IncomingFlow__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public IncomingFlow__Marker_CONTEXT__SRC__mSource_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("IncomingFlow__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.IncomingFlow__Marker source, org.gravity.modisco.MAbstractFlowElement target) {
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.IncomingFlow__Marker> message) {
		...
		org.gravity.modisco.MAbstractFlowElement target = (org.gravity.modisco.MAbstractFlowElement) message.input.getCONTEXT__SRC__mSource();
	...
	private void addReference(ReferenceAdded<ModiscoTGG.IncomingFlow__Marker, org.gravity.modisco.MAbstractFlowElement> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.IncomingFlow__Marker, org.gravity.modisco.MAbstractFlowElement> message) {
