...
public class UnresolvedTypeDeclaration__Marker_CONTEXT__CORR__eModelToPg_0_reference extends AbstractActor {
	...
	private Map<ModiscoTGG.ModelToTypeGraph, Set<EdgeMatch>> targetElements = new HashMap<ModiscoTGG.ModelToTypeGraph, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<ModiscoTGG.ModelToTypeGraph, Set<ModiscoTGG.UnresolvedTypeDeclaration__Marker>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public UnresolvedTypeDeclaration__Marker_CONTEXT__CORR__eModelToPg_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("UnresolvedTypeDeclaration__CONSISTENCY_triangle_4_triangleJunction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, ModiscoTGG.UnresolvedTypeDeclaration__Marker source, ModiscoTGG.ModelToTypeGraph target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((ModiscoTGG.ModelToTypeGraph) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<ModiscoTGG.UnresolvedTypeDeclaration__Marker> message) {
		...
		ModiscoTGG.ModelToTypeGraph target = (ModiscoTGG.ModelToTypeGraph) message.input.getCONTEXT__CORR__eModelToPg();
	...
	private void rightAdded(NodeAddedRight<ModiscoTGG.ModelToTypeGraph> message) {
	...
	private void rightRemoved(NodeDeletedRight<ModiscoTGG.ModelToTypeGraph> message) {
	...
	private void addReference(ReferenceAdded<ModiscoTGG.UnresolvedTypeDeclaration__Marker, ModiscoTGG.ModelToTypeGraph> message) {
	...
	private void removeReference(ReferenceDeleted<ModiscoTGG.UnresolvedTypeDeclaration__Marker, ModiscoTGG.ModelToTypeGraph> message) {
