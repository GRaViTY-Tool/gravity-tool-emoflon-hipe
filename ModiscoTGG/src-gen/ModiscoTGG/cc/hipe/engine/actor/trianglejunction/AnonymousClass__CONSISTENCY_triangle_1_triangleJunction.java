...
public class AnonymousClass__CONSISTENCY_triangle_1_triangleJunction extends GenericTriangleJunctionActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, AbstractJunctionNode node) {
		...
		ports.add(new PortJunctionRight(getSelf(), name2actor.get("AnonymousClass__CONSISTENCY_1042_junction"), this::check_constraint_51));
	}
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		Object[] matchObjects = match.getNodes();
		Collection<HMatch> matches_0 = tAbstractTypeAttrMap.get(matchObjects[5]);
		if(matches_0 != null) {
			matches.remove(match);
		}
		Collection<HMatch> matches_1 = tPackageAttrMap.get(matchObjects[1]);
		if(matches_1 != null) {
			matches.remove(match);
		}
	}
	...
	public boolean check_constraint_51(HMatch match) {
		...
		csp_29.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, "$1", "java.lang.String"));
		csp_29.getVariables().add(new org.emoflon.ibex.tgg.operational.csp.RuntimeTGGAttributeConstraintVariable(true, tNewPackage.getTName(), "java.lang.String"));
		csp_29.solve();
